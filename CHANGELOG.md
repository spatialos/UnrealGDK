...
- The initial connection from a worker will attempt to use relevant command line arguments (receptionistHost, locatorHost) to inform the connection. If these are not provided the standard connection flow will be followed. Subsiquent connections will not use command line arguments.
- The command "Open 0.0.0.0" can be used to connect a worker to using its command line arguments, simulating initial connection.
