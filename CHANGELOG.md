...
## [`x.y.z`] - Unreleased
- Spatial Position Updates are now sent based on a different logic: previously, a position update was sent to spatial if enough time has passed since the Actor's last update (computed using the SpatialUpdateFrequency) AND if the Actor has moved more than the Position Distance Threshold. This change makes the updates being sent if either of the following is true:
  1. The time since the last sent position update is greater than or equal to MinimumTimeThreshold AND the distance travelled since the last update is greater than or equal to MinimumDistanceThreshold.
  1. The time since tha last sent position update is greater than or equal to MaximumTimeThreshold AND the Actor has moved any non-null amount.
  1. The distance travelled since the last sent position update is greater than or equal to MaximumDistanceThreshold.
...
### Breaking changes:
- The logic for sending Spatial position updates has been changed, the `PositionUpdateFrequency` and the `PositionDistanceThreshold` have now been replaced in the GDK settings by 4 new variables. To update your project:
  1. Read the more detailed description of the change, you can find it by searching "Spatial Position Updates are now sent based on a different logic" inside this document.
  2. Find any project-specific usages of the removed variables mentioned above, and replace them according to the new logic.
