...
- Removed the `bEnableHandover` setting and the LoadBalancingStrategy `RequiresHandoverData` method is used instead. If this returns false, handover will never occur.
...
- Handover is now optional according to the load balancing strategy `RequiresHandoverData` method. 
