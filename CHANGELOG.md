...
- Removed the `bEnableHandover` setting and the LoadBalancingStrategy `RequiresHandover` method is used instead. If this returns false, handover will never occur.
...
- Handover is now optional according to the load balancing strategy `RequiresHandover` method. 
