...
- Changed how FGameplayAbilitySpecHandle are generated to account for multi-worker scenarios. Handles are now only generated as a result of an ability being given to an Ability System Component (ASC), and not when FGameplayAbilitySpecHandle is constructed. Handles are now also only unique among all other handles generated by the same ASC, while in native Unreal they are unique among all handles in the game.
