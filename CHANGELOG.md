...
### Breaking changes:
- [Schema descriptors](https://docs.improbable.io/reference/13.8/shared/flexible-project-layout/build-process/schema-descriptor-build-process#schema-descriptor-introduction) are now generated by the [schema_compiler tool](https://docs.improbable.io/reference/13.8/shared/schema/introduction#using-the-schema-compiler-directly). GDK specific schema are no longer copied to your project's `spatial` directory. If you maintain any `spatial local launch` scripts, you must update any calls to have the flag `--enable_pre_run_check=false`, this prevents `spatial local launch` from creating a potentially bad schema descriptor. Additionally, you must delete the `<ProjectRoot>\spatial\schema\unreal\gdk` folder as this is no longer required and will break schema descriptor generation. Finally, for the same reason as the `spatial local launch` flag, any calls to `spatial upload` must also now use the flag `--enable_pre_upload_check=false`.
