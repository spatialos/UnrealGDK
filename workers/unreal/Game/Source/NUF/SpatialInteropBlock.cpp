	...
	UWorld* World)
{
	...
	if(ActorStringRef.IsValid())
	{
		if (World)
		{
			...
			auto NewActor = World->SpawnActor<AActor>(ActorStringRef.TryLoad()->GetClass(), InitialTransform,
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("SpawnNewEntity() failed: received invalid path. EntityType(`%s`)"), *EntityTypeString);
		return nullptr;
	}
}
