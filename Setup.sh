...
spatial package retrieve tools       schema_compiler-x86_64-macos        ${DOMAIN_ENVIRONMENT_VAR}   "${PINNED_CORE_SDK_VERSION}"  "${CORE_SDK_DIR}"/tools/schema_compiler-x86_64-macos.zip
spatial package retrieve schema      standard_library                    ${DOMAIN_ENVIRONMENT_VAR}   "${PINNED_CORE_SDK_VERSION}"  "${CORE_SDK_DIR}"/schema/standard_library.zip
spatial package retrieve worker_sdk  c_headers                           ${DOMAIN_ENVIRONMENT_VAR}   "${PINNED_CORE_SDK_VERSION}"  "${CORE_SDK_DIR}"/worker_sdk/c_headers.zip
spatial package retrieve worker_sdk  c-dynamic-x86_64-clang-macos        ${DOMAIN_ENVIRONMENT_VAR}   "${PINNED_CORE_SDK_VERSION}"  "${CORE_SDK_DIR}"/worker_sdk/c-dynamic-x86_64-clang-macos.zip
spatial package retrieve worker_sdk  c-static-fullylinked-arm-clang-ios  ${DOMAIN_ENVIRONMENT_VAR}   "${PINNED_CORE_SDK_VERSION}"  "${CORE_SDK_DIR}"/worker_sdk/c-static-fullylinked-arm-clang-ios.zip
spatial package retrieve worker_sdk  csharp                              ${DOMAIN_ENVIRONMENT_VAR}   "${PINNED_CORE_SDK_VERSION}"  "${CORE_SDK_DIR}"/worker_sdk/csharp.zip
spatial package retrieve spot        spot-macos                          ${DOMAIN_ENVIRONMENT_VAR}   "${PINNED_SPOT_VERSION}"      "${BINARIES_DIR}"/Programs/spot
