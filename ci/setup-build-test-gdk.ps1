...
class TestSuite {
    ...
    TestSuite([string] $test_repo_url, [string] $test_repo_branch, [string] $test_repo_relative_uproject_path, [string] $test_repo_map,
              ...
              [string] $additional_gdk_editor_options, [bool] $run_with_spatial) {
...
[string] $user_gdk_editor_settings = ""
...
if (Test-Path env:GDK_EDITOR_SETTINGS) {
    $user_gdk_editor_settings = ";" + $env:GDK_EDITOR_SETTINGS
}
