...
if ((Test-Path env:TEST_CONFIG) -And ($env:TEST_CONFIG -eq "Native")) {
    ...
    $tests += [TestSuite]::new($gdk_test_project, "SpatialNetworkingMap", "SpatialOnlyTestResults", "SpatialGDK.+/Game/Maps/FunctionalTests/CI_Fast_Spatial_Only/", "$user_gdk_settings", $True, "$user_cmd_line_args")
    $tests += [TestSuite]::new($gdk_test_project, "SpatialNetworkingMap", "SlowSpatialOnlyTestResults", "SpatialGDK.+/Game/Maps/FunctionalTests/CI_Slow_Spatial_Only/", "$user_gdk_settings", $True, "$user_cmd_line_args")
