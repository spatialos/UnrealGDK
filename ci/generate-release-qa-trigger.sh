...
echo "steps:" | tee /dev/fd/2 | buildkite-agent ...
echo "  - trigger: "${REPO_NAME}-${TEST_NAME}"" | tee /dev/fd/2 | buildkite-agent ...
echo "    label: "Run ${REPO_NAME}-${TEST_NAME} at HEAD OF ${BRANCH_TO_TEST}"" | tee /dev/fd/2 | buildkite-agent ...
echo "    async: true" | tee /dev/fd/2 | buildkite-agent ...
echo "    build:" | tee /dev/fd/2 | buildkite-agent ...
echo "      branch: "${BRANCH_TO_TEST}"" | tee /dev/fd/2 | buildkite-agent ...
echo "      commit: "HEAD"" | tee /dev/fd/2 | buildkite-agent ...
echo "      env:" | tee /dev/fd/2 | buildkite-agent ...

for element in "${ENVIRONMENT_VARIABLES[@]}"
    do
        echo "        ${element}" | tee /dev/fd/2 | buildkite-agent ...
    done
}
