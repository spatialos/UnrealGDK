...
{GenerateTypeFunctions(type.Name, type, bundle)}");
            ...
            {
                builder.AppendLine(string.Join(Environment.NewLine, allNestedTypes.Select(nestedType => GenerateTypeFunctions(Types.GetTypeClassDefinitionName(nestedType.QualifiedName, bundle), types.Find(t => t.QualifiedName == nestedType.QualifiedName), bundle))));
            }
...
{Text.Indent(1, $"{string.Join($", {Environment.NewLine}", type.Fields.Select(f => $"{Types.GetFieldTypeAsCpp(f, bundle)} {Text.SnakeCaseToPascalCase(f.Identifier.Name)}"))})")}
...
{string.Join(Environment.NewLine, type.Fields.Select(field => $@"{Types.GetConstAccessorTypeModification(field, bundle)} {name}::Get{Text.SnakeCaseToPascalCase(field.Identifier.Name)}() const
{{
{Text.Indent(1, $"return _{Text.SnakeCaseToPascalCase(field.Identifier.Name)};")}
}}

{Types.GetFieldTypeAsCpp(field, bundle)}& {name}::Get{Text.SnakeCaseToPascalCase(field.Identifier.Name)}()
{{
{Text.Indent(1, $"return _{ Text.SnakeCaseToPascalCase(field.Identifier.Name)}; ")}
}}

{name}& {name}::Set{Text.SnakeCaseToPascalCase(field.Identifier.Name)}({Types.GetConstAccessorTypeModification(field, bundle)} Value)
{{
...
{Serialization.GetFieldSerialization(field, targetSchemaObject, $"_{Text.SnakeCaseToPascalCase(field.Identifier.Name)}", bundle)}")))}
}}
...
{Serialization.GetFieldDeserialization(field, targetSchemaObject, $"Data._{Text.SnakeCaseToPascalCase(field.Identifier.Name)}", bundle, true)}
"))}
return Data;")}
}}");
            }
...
");
            if (type.Fields.Count() > 0)
            {
                builder.AppendLine(string.Join(Environment.NewLine, type.Fields.Select(field => $@"const {Types.CollectionTypesToQualifiedTypes[Types.Collection.Option]}<{Types.GetFieldTypeAsCpp(field, bundle)}>& {name}::Update::Get{Text.SnakeCaseToPascalCase(field.Identifier.Name)}() const
{{
{Text.Indent(1, $"return _{Text.SnakeCaseToPascalCase(field.Identifier.Name)};")}
}}

{Types.CollectionTypesToQualifiedTypes[Types.Collection.Option]}<{Types.GetFieldTypeAsCpp(field, bundle)}>& {name}::Update::Get{Text.SnakeCaseToPascalCase(field.Identifier.Name)}()
{{
{Text.Indent(1, $"return _{Text.SnakeCaseToPascalCase(field.Identifier.Name)};")}
}}

{name}::Update& {name}::Update::Set{Text.SnakeCaseToPascalCase(field.Identifier.Name)}({Types.GetFieldTypeAsCpp(field, bundle)} value)
{{
{Text.Indent(1, $@"_{ Text.SnakeCaseToPascalCase(field.Identifier.Name)} = value;
return *this;")}
}}
...
{{
{Text.Indent(1, Serialization.GetFieldSerialization(field, updatesObjectName, $"(*_{Text.SnakeCaseToPascalCase(field.Identifier.Name)})", bundle))}
}}"
:
Serialization.GetFieldSerialization(field, updatesObjectName, $"(*_{Text.SnakeCaseToPascalCase(field.Identifier.Name)})", bundle))}
}}"))));
            }
...
{{
{Text.Indent(1, Serialization.GetFieldDeserialization(field, updatesObjectName, $"{deserializingTargetObjectName}._{Text.SnakeCaseToPascalCase(field.Identifier.Name)}", bundle, false, true))}
}}
