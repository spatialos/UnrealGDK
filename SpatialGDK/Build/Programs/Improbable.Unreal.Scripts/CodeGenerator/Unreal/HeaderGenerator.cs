...
namespace Improbable.CodeGen.Unreal
{
    ...
    { 
        ...
        {
            List<TypeDescription> allTopLevelTypes = Types.SortTopLevelTypesTopologically(type, types, bundle);
            string[] typeNamespaces = Text.GetNamespaceFromTypeName(type.QualifiedName);
            IEnumerable<string> requiredIncludes = Types.GetRequiredTypeIncludes(type, bundle).Select(inc => $"#include \"{string.Concat(Enumerable.Repeat("../", type.QualifiedName.Count(c => c == '.')))}{inc}\"");
            List<EnumDefinition> allNestedEnums = Types.GetRecursivelyNestedEnums(type);
            IEnumerable<string> enumDefs = allNestedEnums.Select(enumDef => EnumGenerator.GenerateEnum(Types.GetTypeClassName(enumDef.QualifiedName, bundle), enumDef, bundle).Replace($"enum {enumDef.Name}", $"class {enumDef.Name}_{enumDef.Name}"));
...
");
        ...
        {
            bool hasFields = type.Fields.Count > 0;
            string serializedArgType = type.ComponentId.HasValue ? "Schema_ComponentData" : "Schema_Object";
            string serializedArgName = type.ComponentId.HasValue ? "ComponentData" : "SchemaObject";
