...
exit /b !ERRORLEVEL!";
            ...
            else if (gameName == baseGameName)
            {
                ...
                var clientLaunchConfig = Path.Combine(Path.GetDirectoryName(projectFile), "../spatial", "workers", "unreal", "launcher_client_config.json");
                var copiedLaunchConfig = Path.Combine(windowsNoEditorPath, "launcher_client_config.json");
                if (File.Exists(clientLaunchConfig))
                {
                    File.Copy(clientLaunchConfig, copiedLaunchConfig);
                }
