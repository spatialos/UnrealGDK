...
2. **Unreal Linux cross-platform support**</br>  
    To build the server software for SpatialOS deployments, you need to build the Unreal Engine fork targeting Linux which requires cross-compilation of your SpatialOS project and the Unreal Engine fork. To do this, you need to download and unzip the Linux cross compilation toolchain.</br></br>
...
You need to add two [envionment variables](https://en.wikipedia.org/wiki/Environment_variable). Both are system variables; one to set the path to the Unreal Engine fork directory (`UNREAL_HOME`), and another to set the path to the Linux cross-compilation toolchain (`LINUX_MULTIARCH_ROOT`).
...
2. In the dialog box, select **New...** to create a new system variable named `UNREAL_HOME`.<br/>
Set the variable value as the path to the directory you cloned the Unreal Engine fork into.
3. Test the variable is set correctly: close and restart your terminal window and run `echo %UNREAL_HOME%` (Command Prompt) or `echo $Env:UNREAL_HOME` (PowerShell). </br> 
...
5. Back in the Environment Variables dialog box, create another system variable named `LINUX_MULTIARCH_ROOT`. </br>
...
7. Test the variable is set correctly: close and restart your terminal window and run `echo %LINUX_MULTIARCH_ROOT%` (Command Prompt) or `echo $Env:LINUX_MULTIARCH_ROOT` (PowerShell). </br>
