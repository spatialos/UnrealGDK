...
In the ThirdPersonShooter project each `TPSCharacter` has a variable tracking health called `CurrentHealth`. The health is deducted on the server when the character is being shot, however as the variable is not setup for replication it does not replicate to the client. Therefore the character's health meter does not update on the client even though the health variable is being modified on the server. 
