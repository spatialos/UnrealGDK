...
#### Generate snapshot 
Generate a snapshot for the map you intend to server transition to, and save it to `<GameRoot>\Content\Spatial\Snapshots\`. To do this, you can either:

* generate a snapshot (on the GDK toolbar, select **Snapshot**) and then copy the snapshot manually from `<ProjectRoot>\spatial\snapshots\default.snapshot`, or
* set up your project settings to generate the snapshot for your level into `<GameRoot>\Content\Spatial\Snapshots\`, and then generate the snapshot (on the GDK toolbar, select **Snapshot**). You can find these settings via **Edit** > **Project Settings** > **SpatialOS Unreal GDK** > **Snapshot path**.
