...
To use this tutorial, you must also install [Postgresql](https://www.postgresql.org/) - the database that the Database Sync Worker integrates with.
...
Then, in `spatial/workers/database-sync-worker/scripts` run `build-nuget-packages.ps1` (or `.sh`).
...
```../../Engine/Plugins/UnrealGDK/SpatialGDK/Build/Scripts/ExternalSchemaCodegen.bat spatial/schema/dbsync Game/Source/GDKShooter/ExternalSchemaCodegen```.
 For more information on this tool, including the parameters it accepts, see [this reference page]({{urlRoot}}/content/apis-and-helper-scripts/helper-scripts).
