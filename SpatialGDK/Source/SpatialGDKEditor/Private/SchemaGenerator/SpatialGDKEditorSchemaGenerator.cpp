...
void CreatePartitionAuthoritySet(FString SchemaOutputPath = FString())
{
	if (SchemaOutputPath == "")
	{
		SchemaOutputPath = GetDefault<USpatialGDKEditorSettings>()->GetGeneratedSchemaOutputFolder();
	...
	FString SchemaDir = FPaths::Combine(ContentDir, TEXT("Spatial"), PartitionDataFolderName);

	IPlatformFile& Filesystem = IPlatformFile::GetPlatformPhysical();

	if (!Filesystem.DirectoryExists(*SchemaDir))
	{
		return;
	}
	...
	Filesystem.IterateDirectory(*SchemaDir, [&SchemaFiles, &SchemaExtension](const TCHAR* Entry, bool bIsDirectory) {
	...
	if (!RunSchemaCompiler(SchemaJsonPath, SchemaDir))
	{
