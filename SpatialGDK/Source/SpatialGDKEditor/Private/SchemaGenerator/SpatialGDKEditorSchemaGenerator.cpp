...
bool SaveSchemaDatabase(const FString& PackagePath)
{
	...
	// Generate hash
	{
		SchemaDatabase->SchemaDescriptorHash = 0; // If we are in a unusual environment (test) these paths are incorrect 
		...
		if (FileHandle)
		{
			...
			{
				UE_LOG(LogSpatialGDKSchemaGenerator, Warning, TEXT("Failed to fully read schema.descriptor. Schema not saved."));
				return false;
			}
		}
		else
		{
			UE_LOG(LogSpatialGDKSchemaGenerator, Warning, TEXT("Failed to open schema.descriptor generated by the schema compiler!"));
			return false;
		}
	}
