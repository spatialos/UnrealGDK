...
bool SaveSchemaDatabase(const FString& PackagePath)
{
	...
	// Generate hash
	{
		...
		if (!FileHandle)
		{
			UE_LOG(LogSpatialGDKSchemaGenerator, Error, TEXT("Failed to open schema.descriptor generated by the schema compiler! Schema not saved."));
			FMessageDialog::Debugf(FText::FromString(TEXT("Failed to open schema.descriptor generated by the schema compiler!")));
			return false;
		}
		...
		if (!Result)
		{
			UE_LOG(LogSpatialGDKSchemaGenerator, Error, TEXT("Failed to fully read schema.descriptor. Schema not saved."));
			FMessageDialog::Debugf(FText::FromString(TEXT("Failed to fully read schema.descriptor.")));
			return false;
		}

		UE_LOG(LogSpatialGDKSchemaGenerator, Display, TEXT("Generating schema hash for file with %d bytes."), (int)FileSize); // Temporary debugging
