...
bool SaveSchemaDatabase(const FString& PackagePath)
{
	...
	for (int Retry = 0; Retry < 2; Retry++)
	{
		...
		if (FileHandle)
		{
			...
			if (FileHandle->Read(ByteArray.Get(), FileSize))
			{
		...
		if (!Result)
		{
			FFileStatData StatData = FPlatformFileManager::Get().GetPlatformFile().GetStatData(*DescriptorPath);
			const TCHAR* Reason = nullptr;

			UE_LOG(LogSpatialGDKSchemaGenerator, Warning, TEXT("StatData for %s. Valid: %s, IsDirectory: %s, ReadOnly: %s, FileSize: %llu, Access Time: %llu, Modification time: %llu"),
				*DescriptorPath,
				StatData.bIsValid ? TEXT("Yes") : TEXT("No"),
				StatData.bIsDirectory ? TEXT("Yes") : TEXT("No"),
				StatData.bIsReadOnly ? TEXT("Yes") : TEXT("No"),
				StatData.FileSize,
				StatData.AccessTime.ToUnixTimestamp(),
				StatData.ModificationTime.ToUnixTimestamp());
			UE_LOG(LogSpatialGDKSchemaGenerator, Warning, TEXT("Attempting again in 1 second"), *DescriptorPath);
...
bool RunSchemaCompiler()
{
	...
	UE_LOG(LogSpatialGDKSchemaGenerator, Display, TEXT("Starting '%s' with `%s` arguments."), *SpatialGDKServicesConstants::SchemaCompilerExe, *SchemaCompilerArgs);
	...
	if (ExitCode == 0)
	{
		UE_LOG(LogSpatialGDKSchemaGenerator, Display, TEXT("schema_compiler successfully generated compiled schema with arguments `%s`: %s"), *SchemaCompilerArgs, *SchemaCompilerOut);
		return true;
	}
	else
	{
