...
bool CreateSpawnerEntity(Worker_SnapshotOutputStream* OutputStream)
{
	...
	Components.Add(SpatialGDK::Metadata(TEXT("SpatialSpawner")).CreateMetadataData());
	Components.Add(SpatialGDK::Persistence().CreatePersistenceData());
	Components.Add(SpatialGDK::EntityAcl(SpatialConstants::ClientOrServerPermission, ComponentWriteAcl).CreateEntityAclData());
...
bool CreateGlobalStateManager(Worker_SnapshotOutputStream* OutputStream)
{
	...
	Components.Add(SpatialGDK::Position(Origin).CreatePositionData());
	Components.Add(SpatialGDK::Metadata(TEXT("GlobalStateManager")).CreateMetadataData());
	Components.Add(SpatialGDK::Persistence().CreatePersistenceData());
	...
	Components.Add(SpatialGDK::EntityAcl(SpatialConstants::UnrealServerPermission, ComponentWriteAcl).CreateEntityAclData());
...
bool CreatePlaceholders(Worker_SnapshotOutputStream* OutputStream)
{
	...
	for (int x = -PlaceholderCountAxis / 2; x < PlaceholderCountAxis / 2; x++)
	{
		...
		{
			const SpatialGDK::Coordinates PlaceholderPosition{ x * CHUNK_SIZE + CHUNK_SIZE * 0.5f, 0, y * CHUNK_SIZE + CHUNK_SIZE * 0.5f };
			...
			Components.Add(SpatialGDK::Position(PlaceholderPosition).CreatePositionData());
			Components.Add(SpatialGDK::Metadata(TEXT("Placeholder")).CreateMetadataData());
			Components.Add(SpatialGDK::Persistence().CreatePersistenceData());
			Components.Add(SpatialGDK::EntityAcl(SpatialConstants::UnrealServerPermission, ComponentWriteAcl).CreateEntityAclData());
...
bool CreateStartupActor(Worker_SnapshotOutputStream* OutputStream, AActor* Actor, Worker_EntityId EntityId, USpatialNetConnection* NetConnection, USpatialClassInfoManager* ClassInfoManager)
{
	...
	TArray<Worker_ComponentData> Components;
	Components.Add(SpatialGDK::Position(SpatialGDK::Coordinates::FromFVector(Channel->GetActorSpatialPosition(Actor))).CreatePositionData());
	Components.Add(SpatialGDK::Metadata(ActorClass->GetName()).CreateMetadataData());
	Components.Add(SpatialGDK::EntityAcl(SpatialConstants::ClientOrServerPermission, ComponentWriteAcl).CreateEntityAclData());
	Components.Add(SpatialGDK::Persistence().CreatePersistenceData());
	Components.Add(SpatialGDK::SpawnData(Actor).CreateSpawnDataData());
	Components.Add(SpatialGDK::UnrealMetadata({}, {}, ActorClass->GetPathName(), Actor->bNetLoadOnClient).CreateUnrealMetadataData());
	Components.Add(SpatialGDK::Interest().CreateInterestData());
