...
TMap<FName, uint32> GetLayerWorkerCountMappingFromWorldSettings(const UWorld& World)
{
	...
	return WorldSettings->MultiWorkerSettingsClass->GetDefaultObject<UAbstractSpatialMultiWorkerSettings>()->GetLayerWorkerCounts();
}

uint32 GetTotalWorkerCountFromWorldSettings(const UWorld& World)
{
	const ASpatialWorldSettings* WorldSettings = Cast<ASpatialWorldSettings>(World.GetWorldSettings());
	if (WorldSettings == nullptr)
	{
...
bool FillWorkerConfigurationFromCurrentMap(FWorkerTypeLaunchSection& OutWorker, FIntPoint& OutWorldDimensions)
{
	...
	OutWorker.NumEditorInstances = GetTotalWorkerCountFromWorldSettings(*EditorWorld);
