...
private:
  Worker_ComponentData StoreComponentData(ComponentData&& data);
  Worker_ComponentUpdate StoreComponentUpdate(ComponentUpdate&& update);
  Worker_CommandRequest StoreCommandRequest(CommandRequest&& request);
  Worker_CommandResponse StoreCommandResponse(CommandResponse&& response);
  const std::vector<Worker_Entity>& StoreQueryEntities(std::vector<EntitySnapshot> entities);
  const std::vector<Worker_ComponentData>& StoreSnapshotComponents(EntityState entity);
  const char* StoreString(const std::string& string);
  const char** StoreAttributeSet(const std::vector<std::string>& attributeSet);
