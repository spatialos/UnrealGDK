...
FVisibilityResult UInterestedServerLevelStreamingStrategy::GetVisibilityResultForLevel(const FVector& LevelPosition, const float LevelExtent, const FIntVector& OriginLocation) const
{
	const USpatialNetDriver* SpatialNetDriver = Cast<USpatialNetDriver>(GetWorld()->GetNetDriver());
	const UAbstractLBStrategy* LoadBalanceStrategy = SpatialNetDriver->LoadBalanceStrategy;
	...
	{
		UE_LOG(LogInterestedServerLevelStreamingStrategy, Error,
			   TEXT("No load balancing strategy set for InterestedServerLevelStreamingStrategy. No levels will be loaded."));
		return false;
	}
	...
	{
		UE_LOG(LogInterestedServerLevelStreamingStrategy, Error,
			   TEXT("Load balancing strategy is not ready for InterestedServerLevelStreamingStrategy. No levels will be loaded."));
		return false;
	}

	const SpatialGDK::QueryConstraint WorkerInterestQuery =
		LoadBalanceStrategy->GetWorkerInterestQueryConstraint(LoadBalanceStrategy->GetLocalVirtualWorkerId());
	const auto BoxConstraint = WorkerInterestQuery.BoxConstraint;
	...
	if (!BoxConstraint.IsSet())
	{
		UE_LOG(LogInterestedServerLevelStreamingStrategy, Error,
			   TEXT("Load balancing strategy does not use single box constraint for interest. Only load balancing strategies with single "
					"box constraints are currently supported. No levels will be loaded."));
		return false;
	}
	...
	// Calculate bounds of level in world coordinates
	FIntPoint WorldOriginLocation2D = FIntPoint(OriginLocation.X, OriginLocation.Y);
	FIntPoint LevelPosition2D = FIntPoint(LevelPosition.X, LevelPosition.Y);
	FIntPoint LevelOffset2D = LevelPosition2D - WorldOriginLocation2D;
	FBox2D TileBounds2D(FVector2D(LevelPosition2D.X - LevelExtent, LevelPosition2D.Y - LevelExtent),
						FVector2D(LevelPosition2D.X + LevelExtent, LevelPosition2D.Y + LevelExtent));
