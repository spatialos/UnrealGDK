...
SpatialVirtualWorkerTranslator::SpatialVirtualWorkerTranslator()
	: UnassignedVirtualWorkersCount(0)
...
void SpatialVirtualWorkerTranslator::AddVirtualWorkerIds(const TSet<VirtualWorkerId>& InVirtualWorkerIds)
{
	...
	for (VirtualWorkerId VirtualWorkerId : InVirtualWorkerIds)
	{
		...
		UnassignedVirtualWorkersCount++;
	}
}

const PhysicalWorkerName* SpatialVirtualWorkerTranslator::GetPhysicalWorkerForVirtualWorker(VirtualWorkerId id)
{
...
void SpatialVirtualWorkerTranslator::Tick(float DeltaTime)
{
...
bool SpatialVirtualWorkerTranslator::ConstructVirtualWorkerMappingFromQueryResponse(const Worker_EntityQueryResponseOp& Op)
{
	if ((int) Op.result_count < UnassignedVirtualWorkersCount)
	{
		return false;
	}
	...
	return UnassignedVirtualWorkers.IsEmpty();
}
...
void SpatialVirtualWorkerTranslator::WorkerEntityQueryDelegate(const Worker_EntityQueryResponseOp& Op)
{
	...
	else
	{
		...
		if (!ConstructVirtualWorkerMappingFromQueryResponse(Op))
		{
			FTimerHandle RetryTimer;
			UE_LOG(LogSpatialVirtualWorkerTranslator, Warning, TEXT("(%s) Retrying query for worker system entities in 5 seconds."), *WorkerId);
