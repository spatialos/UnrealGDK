...
void USpatialNetDriver::OnGSMQuerySuccess()
{
	...
	if (bWaitingToSpawn && ClientCanSendPlayerSpawnRequests())
	{
		...
		if (GetDefault<USpatialGDKSettings>()->bEnableSchemaValidationOnJoin)
		{
			...
			{
				UE_LOG(LogSpatialOSNetDriver, Warning, TEXT("Your schema hash does match the server, this may cause problems. Our hash: %u server hash: %u"), ClassInfoManager->SchemaDatabase->SchemaDescriptorHash, ServerHash);
