	...
	for (int32 j = 0; j < FinalSortedCount; j++)
	{
		...
		if (Channel == nullptr || Channel->Actor != nullptr) // Make sure didn't just close this channel.
		{
			...
			if (bIsRecentlyRelevant)
			{
				...
				{
					if (!Actor->GetClass()->HasAnySpatialClassFlags(SPATIALCLASS_ExplicitSpatialType))
					{
	...
	UObject* SubObject)
{
	...
	if (!CallingObject->GetClass()->HasAnySpatialClassFlags(SPATIALCLASS_ExplicitSpatialType))
	{
