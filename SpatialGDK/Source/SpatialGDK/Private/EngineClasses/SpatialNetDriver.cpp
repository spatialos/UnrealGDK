...
void USpatialNetDriver::InitiateConnectionToSpatialOS(const FURL& URL)
{
	...
	if (bConnectAsClient)
	{
		...
		GameInstance->bPersistSpatialConnection = URL.HasOption(*SpatialConstants::ClientsStayConnectedURLOption);
	}
	else
	{
		// Servers should never disconnect from a deployment.
		GameInstance->bPersistSpatialConnection = true;
	}

	if (!GameInstance->bPersistSpatialConnection)
	{
...
void USpatialNetDriver::HandleOngoingServerTravel()
{
	...
	TimerManager.SetTimer(TimerHandle, [this]()
	{
		if (Cast<USpatialGameInstance>(GetWorld()->GetGameInstance())->bResponsibleForSnapshotLoading)
		{
			...
			Worker_AuthorityChangeOp Op1;
			Op1.authority = WORKER_AUTHORITY_AUTHORITATIVE;
			Op1.component_id = SpatialConstants::STARTUP_ACTOR_MANAGER_COMPONENT_ID;
			Op1.entity_id = 2;

			Worker_AuthorityChangeOp Op3;
			Op3.authority = WORKER_AUTHORITY_AUTHORITATIVE;
			Op3.component_id = SpatialConstants::SINGLETON_MANAGER_COMPONENT_ID;
			Op3.entity_id = 2;

			Worker_AuthorityChangeOp Op4;
			Op4.authority = WORKER_AUTHORITY_AUTHORITATIVE;
			Op4.component_id = SpatialConstants::PLAYER_SPAWNER_COMPONENT_ID;
			Op4.entity_id = 1;

			// STAVKA - add Session entity and component id (see SpatialSessionStateComponent)
			Worker_AuthorityChangeOp Op5;
			Op5.authority = WORKER_AUTHORITY_AUTHORITATIVE;
			Op5.component_id = 1000; // ComponentId 1000 is the Session component, defined in session.schema
			Op5.entity_id = 3; // EntityId 3 is the session entity, written to the snapshot in DeploymentSnapshotTemplate.cpp

			//StaticComponentView->OnAuthorityChange(Op1);
			////StaticComponentView->OnAuthorityChange(Op2);
			//StaticComponentView->OnAuthorityChange(Op3);
			//StaticComponentView->OnAuthorityChange(Op4);

			GlobalStateManager->GlobalStateManagerEntityId = SpatialConstants::INITIAL_GLOBAL_STATE_MANAGER_ENTITY_ID;

			Receiver->HandleActorAuthority(Op4);
			Receiver->HandleActorAuthority(Op1);
			Receiver->HandleActorAuthority(Op3);
			Receiver->HandleActorAuthority(Op5);
			GlobalStateManager->TriggerBeginPlay();

			//GlobalStateManager->BecomeAuthoritativeOverAllActors();
			//GlobalStateManager->SetCanBeginPlay(true);
			//GlobalStateManager->ExecuteInitialSingletonActorReplication();

			FTimerHandle SetAcceptingPlayersTimerHandle;
			TimerManager.SetTimer(SetAcceptingPlayersTimerHandle, [this]()
			{
				Worker_AuthorityChangeOp Op2;
				Op2.authority = WORKER_AUTHORITY_AUTHORITATIVE;
				Op2.component_id = SpatialConstants::DEPLOYMENT_MAP_COMPONENT_ID;
				Op2.entity_id = 2;

				Receiver->HandleActorAuthority(Op2);
...
void USpatialNetDriver::OnMapLoaded(UWorld* LoadedWorld)
{
	...
	if (!IsServer())
	{
		...
		if (GlobalStateManager->bAcceptingPlayers && GetWorld()->RemovePIEPrefix(GlobalStateManager->DeploymentMapURL) == GetWorld()->RemovePIEPrefix(GetWorld()->URL.Map))
		{
			...
			bWaitingForAcceptingPlayersToSpawn = false;
			GameInstance->bPersistSpatialConnection = false;
		}
		else
		{
			checkNoEntry();
...
void USpatialNetDriver::SpatialProcessServerTravel(const FString& URL, bool bAbsolute, AGameModeBase* GameMode)
{
	...
	NetDriver->GlobalStateManager->SetCanBeginPlay(false);
	...
	AddStringToSchema(UpdateObject, SpatialConstants::DEPLOYMENT_MAP_MAP_URL_ID, TEXT(""));

	// Set the AcceptingPlayers state on the GSM
	Schema_AddBool(UpdateObject, SpatialConstants::DEPLOYMENT_MAP_ACCEPTING_PLAYERS_ID, false);

	// Component updates are short circuited so we set the updated state here and then send the component update.
	NetDriver->Connection->SendComponentUpdate(NetDriver->GlobalStateManager->GlobalStateManagerEntityId, &Update);
	...
	UObject* SubObject)
{
	...
	{
		UE_LOG(LogSpatialOSNetDriver, Warning, TEXT("No owning connection for actor %s. Function %s"), *Actor->GetName(), *Function->GetName());
	}
