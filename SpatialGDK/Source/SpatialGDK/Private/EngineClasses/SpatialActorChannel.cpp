...
FRepChangeState USpatialActorChannel::CreateInitialRepChangeState(TWeakObjectPtr<UObject> Object)
{
	...
	for (uint16 CmdIdx = 0; CmdIdx < CmdCount; ++CmdIdx)
	{
		...
		// decide whether to replicate physics. see: ComponentReader::ApplySchemaObject UNR-5843 TODO: fix this so we no longer need this
		// special handling, for instance by replicating bRepPhysics as a separate always replicated field.
