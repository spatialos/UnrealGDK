	...
	else
	{
		...
		if (NetGUID.IsValid())
		{
			...
			if (Value == nullptr)
			{
				...
				UE_LOG(LogSpatialNetBitReader, Warning, TEXT("Object ref did not map to valid object. Streaming level not loaded or actor deleted. Will be set to nullptr: %s %s"),
