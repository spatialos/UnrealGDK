...
TArray<Worker_Op> GetOpsFromEntityDeltas(const TArray<EntityDelta>& Deltas)
{
	...
	for (const EntityDelta& Entity : Deltas)
	{
		...
		if (Entity.Type == EntityDelta::ADD)
		{
			...
			Op.op.add_entity.entity_id = ToWorkerEntityId(Entity.EntityId);
			Ops.Push(Op);
		}
		...
		for (const ComponentChange& Change : Entity.ComponentsAdded)
		{
			...
			Op.op.add_component.entity_id = ToWorkerEntityId(Entity.EntityId);
		...
		for (const AuthorityChange& Change : Entity.AuthorityLost)
		{
			...
			Op.op.component_set_authority_change.entity_id = ToWorkerEntityId(Entity.EntityId);
		...
		for (const ComponentChange& Change : Entity.ComponentsRefreshed)
		{
			...
			AddOp.op.add_component.entity_id = ToWorkerEntityId(Entity.EntityId);
		...
		for (const ComponentChange& Change : Entity.ComponentUpdates)
		{
			...
			Op.op.component_update.entity_id = ToWorkerEntityId(Entity.EntityId);
		...
		for (const ComponentChange& Change : Entity.ComponentsRemoved)
		{
			...
			Op.op.remove_component.entity_id = ToWorkerEntityId(Entity.EntityId);
		...
		if (Entity.Type == EntityDelta::REMOVE)
		{
			...
			Op.op.remove_entity.entity_id = ToWorkerEntityId(Entity.EntityId);
			Ops.Push(Op);
		}
