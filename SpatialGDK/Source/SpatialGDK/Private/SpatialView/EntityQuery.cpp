...
int32 EntityQuery::GetNestedConstraintCount(const Worker_Constraint& Constraint)
{
	...
	case WORKER_CONSTRAINT_TYPE_AND: {
		auto& AndConstraint = Constraint.constraint.and_constraint;
		size_t Sum = 1;
	...
	{
		auto& OrConstraint = Constraint.constraint.or_constraint;
		size_t Sum = 1;
...
void EntityQuery::StoreChildConstraints(const Worker_Constraint& Constraint, int32 Index)
{
	...
	case WORKER_CONSTRAINT_TYPE_AND:
	{
		const auto& AndConstraint = Constraint.constraint.and_constraint;
	...
	case WORKER_CONSTRAINT_TYPE_OR:
	{
		const auto& OrConstraint = Constraint.constraint.or_constraint;
		// Appends children to the array.
	...
	case WORKER_CONSTRAINT_TYPE_NOT:
	{
		const auto& NotConstraint = Constraint.constraint.not_constraint;
