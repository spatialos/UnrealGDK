...
void USpatialReceiver::LeaveCriticalSection()
{
	...
	for (FEntityId& PendingAddEntity : PendingAddActors)
	{
...
void USpatialReceiver::OnRemoveEntity(const Worker_RemoveEntityOp& Op)
{
	...
	TWeakObjectPtr<UObject> RemovedObject = PackageMap->GetObjectFromEntityId(Op.entity_id);
	...
	if (NetDriver->IsServer())
	{
		...
		if (FString* WorkerName = WorkerConnectionEntities.Find(Op.entity_id))
		{
			...
			if (USpatialNetConnection* ClientConnection = ClientConnectionPtr.Get())
			{
				...
				{
					FEntityId PCEntity = PackageMap->GetEntityIdFromObject(Controller);
...
void USpatialReceiver::DropQueuedRemoveComponentOpsForEntity(FEntityId EntityId)
{
...
USpatialActorChannel* USpatialReceiver::GetOrRecreateChannelForDomantActor(AActor* Actor, FEntityId EntityID)
{
...
void USpatialReceiver::UpdateShadowData(FEntityId EntityId)
{
...
bool USpatialReceiver::IsReceivedEntityTornOff(FEntityId EntityId)
{
...
void USpatialReceiver::ReceiveActor(FEntityId EntityId)
{
...
void USpatialReceiver::RemoveActor(FEntityId EntityId)
{
...
void USpatialReceiver::DestroyActor(AActor* Actor, FEntityId EntityId)
{
...
void USpatialReceiver::ApplyComponentDataOnActorCreation(FEntityId EntityId, const Worker_ComponentData& Data,
													...
													TUniquePtr<SpatialGDK::DynamicComponent> Data)
{
	...
	PendingDynamicSubobjectComponents.Add(MakeTuple(static_cast<FEntityId>(EntityId), ComponentId),
	...
	ForAllSchemaComponentTypes([&](ESchemaComponentType Type) {
		...
		if (!PendingDynamicSubobjectComponents.Contains(MakeTuple(static_cast<FEntityId>(EntityId), SchemaComponentId)))
		{
...
void USpatialReceiver::AttachDynamicSubobject(AActor* Actor, FEntityId EntityId, const FClassInfo& Info)
{
	USpatialActorChannel* Channel = NetDriver->GetActorChannelByEntityId(EntityId);
	if (Channel == nullptr)
	{
...
void USpatialReceiver::OnComponentUpdate(const Worker_ComponentUpdateOp& Op)
{
	...
	FSpanId CauseSpanId = EventTracer->GetSpanId(EntityComponentId(Op.entity_id, Op.update.component_id));
...
void USpatialReceiver::HandleRPCLegacy(const Worker_ComponentUpdateOp& Op)
{
	...
	FEntityId EntityId = Op.entity_id;
...
void USpatialReceiver::OnCommandRequest(const Worker_Op& Op)
{
		...
		&& CommandIndex == SpatialConstants::PLAYER_SPAWNER_SPAWN_PLAYER_COMMAND_ID)
	{
		...
		EventTracer->TraceEvent(FSpatialTraceEventBuilder::ReceiveCommandRequest(TEXT("SPAWN_PLAYER_COMMAND"), RequestId), SpanId);
		return;
	}
			 ...
			 && CommandIndex == SpatialConstants::SERVER_WORKER_FORWARD_SPAWN_REQUEST_COMMAND_ID)
	{
		...
		EventTracer->TraceEvent(
			FSpatialTraceEventBuilder::ReceiveCommandRequest(TEXT("SERVER_WORKER_FORWARD_SPAWN_REQUEST_COMMAND"), RequestId), SpanId);
		return;
	}
	else if (ComponentId == SpatialConstants::RPCS_ON_ENTITY_CREATION_ID && CommandIndex == SpatialConstants::CLEAR_RPCS_ON_ENTITY_CREATION)
	{
		...
		TOptional<FSpanId> SpanId = EventTracer->CreateSpan(&Op.span_id, 1);
		EventTracer->TraceEvent(FSpatialTraceEventBuilder::ReceiveCommandRequest(TEXT("CLEAR_RPCS_ON_ENTITY_CREATION"), RequestId), SpanId);
		return;
	}
#if WITH_EDITOR
			 ...
			 && CommandIndex == SpatialConstants::SHUTDOWN_MULTI_PROCESS_REQUEST_ID)
	{
		...
		TOptional<FSpanId> SpanId = EventTracer->CreateSpan(&Op.span_id, 1);
		EventTracer->TraceEvent(FSpatialTraceEventBuilder::ReceiveCommandRequest(TEXT("SHUTDOWN_MULTI_PROCESS_REQUEST"), RequestId),
								SpanId);
		return;
	}
#endif // WITH_EDITOR
	...
	TOptional<FSpanId> SpanId = EventTracer->CreateSpan(&Op.span_id, 1);
	EventTracer->TraceEvent(FSpatialTraceEventBuilder::ReceiveCommandRequest("RPC_COMMAND_REQUEST", TargetActor, TraceTargetObject,
...
void USpatialReceiver::OnCommandResponse(const Worker_Op& Op)
{
	...
	if (ComponentId == SpatialConstants::PLAYER_SPAWNER_COMPONENT_ID)
	{
		...
		EventTracer->TraceEvent(FSpatialTraceEventBuilder::ReceiveCommandResponse(TEXT("SPAWN_PLAYER_COMMAND"), RequestId), Op.span_id);
		return;
	}
	else if (ComponentId == SpatialConstants::SERVER_WORKER_COMPONENT_ID)
	{
		...
		TOptional<FSpanId> SpanId = EventTracer->CreateSpan(&Op.span_id, 1);
		EventTracer->TraceEvent(
			FSpatialTraceEventBuilder::ReceiveCommandResponse(TEXT("SERVER_WORKER_FORWARD_SPAWN_REQUEST_COMMAND"), RequestId), SpanId);
		return;
	}
...
void USpatialReceiver::ReceiveCommandResponse(const Worker_Op& Op)
{
	...
	if (ReliableRPCPtr == nullptr)
	{
		...
		EventTracer->TraceEvent(FSpatialTraceEventBuilder::ReceiveCommandResponse(TargetActor, RequestId, false), Op.span_id);
		return;
	}
	...
	EventTracer->TraceEvent(FSpatialTraceEventBuilder::ReceiveCommandResponse(TargetActor, TargetObject, ReliableRPC->Function, RequestId,
...
FRPCErrorInfo USpatialReceiver::ApplyRPCInternal(UObject* TargetObject, UFunction* Function, const FPendingRPCParams& PendingRPCParams)
{
	...
	if (UnresolvedRefCount == 0 || SpatialSettings->QueuedIncomingRPCWaitTime < TimeQueued)
	{
		...
		const FEntityId EntityId = PendingRPCParams.ObjectRef.Entity;
		...
		else
		{
			if (EventTracer->IsEnabled() && RPCType != ERPCType::CrossServer)
			{
				...
				if (RPCId != 0)
				{
					...
					FSpanId CauseSpanId = EventTracer->GetSpanId(Id);
					TOptional<FSpanId> SpanId = EventTracer->CreateSpan(&CauseSpanId, 1);
...
void USpatialReceiver::OnCreateEntityResponse(const Worker_Op& Op)
{
	...
	if (Channel.IsValid())
	{
		...
		TOptional<FSpanId> SpanId = EventTracer->CreateSpan(&Op.span_id, 1);
		EventTracer->TraceEvent(FSpatialTraceEventBuilder::ReceiveCreateEntitySuccess(Channel->Actor, EntityId), SpanId);
	}
...
void USpatialReceiver::ProcessQueuedActorRPCsOnEntityCreation(FEntityId EntityId, RPCsOnEntityCreation& QueuedRPCs)
{
...
void USpatialReceiver::ClearPendingRPCs(FEntityId EntityId)
{
...
bool USpatialReceiver::OnExtractIncomingRPC(FEntityId EntityId, ERPCType RPCType, const SpatialGDK::RPCPayload& Payload)
{
...
void USpatialReceiver::CloseClientConnection(USpatialNetConnection* ClientConnection, FEntityId PlayerControllerEntityId)
{
...
void USpatialReceiver::StartAsyncLoadingClass(const FString& ClassPath, FEntityId EntityId)
{
	FString PackagePath = GetPackagePath(ClassPath);
	FName PackagePathName = *PackagePath;
...
bool USpatialReceiver::IsEntityWaitingForAsyncLoad(FEntityId Entity)
{
...
TArray<PendingAddComponentWrapper> USpatialReceiver::ExtractAddComponents(FEntityId Entity)
{
...
EntityComponentOpListBuilder USpatialReceiver::ExtractAuthorityOps(FEntityId Entity)
{
...
bool USpatialReceiver::HasEntityBeenRequestedForDelete(FEntityId EntityId)
{
...
void USpatialReceiver::HandleEntityDeletedAuthority(FEntityId EntityId)
{
