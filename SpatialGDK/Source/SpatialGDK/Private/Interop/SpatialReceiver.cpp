...
void USpatialReceiver::ReceiveActor(Worker_EntityId EntityId)
{
	...
	else
	{
		...
		{
			UE_LOG(LogSpatialReceiver, Log, TEXT("The received actor with entity id %lld was already torn off. The actor will not be spawned."), EntityId);
			return;
		}
...
void USpatialReceiver::RemoveActor(Worker_EntityId EntityId)
{
	...
	if (Actor->GetTearOff())
	{
		if (USpatialActorChannel* ActorChannel = NetDriver->GetActorChannelByEntityId(EntityId))
		{
			UE_LOG(LogSpatialActorChannel, Log, TEXT("Ben: Closing channel due to tearoff, for entity id %lld, channel %s."), EntityId, *GetPathNameSafe(ActorChannel));
#if ENGINE_MINOR_VERSION <= 20
			ActorChannel->ConditionalCleanUp();
#else
