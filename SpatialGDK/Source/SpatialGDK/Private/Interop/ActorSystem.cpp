...
namespace SpatialGDK
{
	...
	ActorSystem::ActorSystem(const FSubView& InSubView, const FSubView& InTombstoneSubView, USpatialNetDriver* InNetDriver, FTimerManager* InTimerManager,
	...
	void ActorSystem::Advance()
	{
		for (const EntityDelta& Delta : TombstoneSubView->GetViewDelta().EntityDeltas)
		{
			...
			{
				AActor* EntityActor = TryGetActor(UnrealMetadata(SubView->GetView()[Delta.EntityId].Components.FindByPredicate(ComponentIdEquality{ SpatialConstants::UNREAL_METADATA_COMPONENT_ID})->GetUnderlying()));
