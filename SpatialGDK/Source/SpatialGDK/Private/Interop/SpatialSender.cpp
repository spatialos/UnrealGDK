...
Worker_ComponentData USpatialSender::CreateLevelComponentData(AActor* Actor)
{
	...
	if (ActorWorld != NetDriver->World)
	{
		...
		{
			UE_LOG(LogSpatialSender, Verbose, TEXT("Could not find Streaming Level Component for Level %s, processing Actor %s. Have you generated schema?"),
...
void USpatialSender::ResetOutgoingUpdate(USpatialActorChannel* DependentChannel, UObject* ReplicatedObject, int16 Handle, bool bIsHandover)
{
	...
	{
		FChannelToHandleToUnresolved* ChannelToUnresolved = ObjectToUnresolved.Find(UnresolvedObject);
		if (ChannelToUnresolved == nullptr)
		{
			continue;
		}

		FHandleToUnresolved* OtherHandleToUnresolved = ChannelToUnresolved->Find(ChannelObjectPair);
		if (OtherHandleToUnresolved == nullptr)
		{
			continue;
		}
