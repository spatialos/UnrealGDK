...
void USnapshotManager::DeleteEntities(const Worker_EntityQueryResponseOp& Op)
{
	...
	{
		if (Op.results[i].entity_id != SpatialConstants::INITIAL_SPAWNER_ENTITY_ID && Op.results[i].entity_id != SpatialConstants::INITIAL_GLOBAL_STATE_MANAGER_ENTITY_ID && Op.results[i].entity_id != 3)
		{
