...
ActorLockToken UOwnershipLockingPolicy::AcquireLock(AActor* Actor, FString DebugString)
{
	...
	else
	{
		...
		AActor* OwnershipHierarchyRoot = SpatialGDK::GetOutermostReplicatedOwner(Actor);
...
bool UOwnershipLockingPolicy::IsLocked(const AActor* Actor) const
{
	...
	if (AActor* HierarchyRoot = SpatialGDK::GetOutermostReplicatedOwner(Actor))
	{
...
void UOwnershipLockingPolicy::OnOwnerUpdated(const AActor* Actor, const AActor* OldOwner)
{
	...
	{
		const AActor* OldHierarchyRoot = OldOwner->GetOwner() != nullptr ? SpatialGDK::GetOutermostReplicatedOwner(OldOwner) : OldOwner;
...
void UOwnershipLockingPolicy::RecalculateLockedActorOwnershipHierarchyInformation(const AActor* ExplicitlyLockedActor)
{
	...
	AActor* NewOwnershipHierarchyRoot = SpatialGDK::GetOutermostReplicatedOwner(ExplicitlyLockedActor);
