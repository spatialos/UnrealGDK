...
void FPartitionManager::UpdatePartitionMetadata(FPartitionHandle Partition, TArray<ComponentUpdate> Updates)
{
	...
	{
		// if(PartitionState.CurrentMetadataComponents.Contains(Update.GetComponentId()))
		...
		// else
		//{
		//	ComponentData* PendingCreation = PartitionState.PendingMetadataCreation.FindByPredicate([&Update](const ComponentData&
		// InitialData) 	{return Update.GetComponentId() == InitialData.GetComponentId(); }); 	if(PendingCreation != nullptr)
		//	{
		//		PendingCreation->ApplyUpdate(MoveTemp(Update));
		//	}
		//	else
		//	{
		//		ComponentData NewComponent(Update.GetComponentId());
		//		NewComponent.ApplyUpdate(MoveTemp(Update));
		//		PartitionState.PendingMetadataCreation.Add(MoveTemp(NewComponent));
		//	}
		//}
	}
}
