...
{
	return CompareSchemaObjectToSendAndPayload(Schema_GetComponentUpdateFields(Update.UpdateObj.schema_type), EntityPayloadItem.Payload, RPCType, RPCId) &&
...
Worker_ComponentData GetComponentDataOnEntityCreationFromRPCService(SpatialGDK::SpatialRPCService& RPCService, Worker_EntityId EntityID, ERPCType RPCType)
{
...
RPC_SERVICE_TEST(GIVEN_authority_over_client_endpoint_WHEN_push_server_unreliable_rpcs_to_the_service_THEN_payloads_are_writen_correctly_to_component_updates)
{
	...
	{
		RPCService.PushRPC(EntityPayloadItem.EntityId, ERPCType::ServerUnreliable, EntityPayloadItem.Payload);
	}
...
RPC_SERVICE_TEST(GIVEN_receiving_an_rpc_WHEN_return_false_from_extract_callback_THEN_some_rpcs_persist_on_component)
{
	...
	SpatialGDK::SpatialRPCService::UpdateToSend* Update = UpdateToSendArray.FindByPredicate([](const SpatialGDK::SpatialRPCService::UpdateToSend& Update) {
		return (Update.UpdateObj.component_id == SpatialConstants::SERVER_ENDPOINT_COMPONENT_ID);
	});

	if (Update != nullptr)
	{
		const Schema_Object* ComponentObject = Schema_GetComponentUpdateFields(Update->UpdateObj.schema_type);
		uint64 Ack = 0;
