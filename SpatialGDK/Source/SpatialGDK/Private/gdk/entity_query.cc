...
void EntityQuery::StoreConstraint(const Worker_Constraint& constraint) {
  ...
  case WORKER_CONSTRAINT_TYPE_AND:
    for (std::uint32_t i = 0; i < constraint.constraint.and_constraint.constraint_count; ++i) {
      StoreConstraint(constraint.constraint.and_constraint.constraints[i]);
    }
    // Adjust pointers to point at the child constraint.
    constraints[index].constraint.and_constraint.constraints = &constraints[index + 1];
    break;
  case WORKER_CONSTRAINT_TYPE_OR:
    for (std::uint32_t i = 0; i < constraint.constraint.or_constraint.constraint_count; ++i) {
      StoreConstraint(constraint.constraint.or_constraint.constraints[i]);
    }
    // Adjust pointers to point at the child constraint.
    constraints[index].constraint.or_constraint.constraints = &constraints[index + 1];
    break;
  case WORKER_CONSTRAINT_TYPE_NOT:
    StoreConstraint(*constraint.constraint.not_constraint.constraint);
    // Adjust pointers to point at the child constraint.
    constraints[index].constraint.not_constraint.constraint = &constraints[index + 1];
    break;
  }
}
