...
void SpatialOsWorker::FlushMessagesToSend() {
...
void SpatialOsWorker::SendLogMessage(Worker_LogLevel level, EntityId entityId,
                                     std::string loggerName, std::string message) {
...
void SpatialOsWorker::SendAddComponent(EntityId entityId, ComponentData&& data) {
  workerView.SendAddComponent(EntityComponentData{entityId, std::move(data)});
}

void SpatialOsWorker::SendRemoveComponent(EntityId entityId, ComponentId componentId) {
  workerView.SendRemoveComponent(EntityComponentId{entityId, componentId});
}

void SpatialOsWorker::SendCommandRequest(EntityId entityId, RequestId requestId,
                                         CommandRequest&& request, std::uint32_t timeout) {
...
void SpatialOsWorker::SendReserveEntityIdsRequest(RequestId requestId,
...
void SpatialOsWorker::SendCreateEntityRequest(RequestId requestId, EntityState&& entity,
                                              EntityId entityId, std::uint32_t timeoutMillis) {
...
const std::vector<EntityId>& SpatialOsWorker::GetEntitiesAdded() const {
...
SpatialOsWorker::GetComponentsAdded(ComponentId componentId) const {
  return currentViewDelta->GetComponentsAdded(componentId);
}

const std::vector<EntityComponentId>&
SpatialOsWorker::GetComponentsRemoved(ComponentId componentId) const {
  return currentViewDelta->GetComponentsRemoved(componentId);
}

const std::vector<EntityComponentUpdate>&
SpatialOsWorker::GetComponentUpdates(ComponentId componentId) const {
  return currentViewDelta->GetComponentUpdates(componentId);
}

const std::vector<EntityComponentId>&
SpatialOsWorker::GetAuthorityGained(ComponentId componentId) const {
  return currentViewDelta->GetAuthorityGained(componentId);
}

const std::vector<EntityComponentId>&
SpatialOsWorker::GetAuthorityLost(ComponentId componentId) const {
  return currentViewDelta->GetAuthorityLost(componentId);
}

const std::vector<EntityComponentId>&
SpatialOsWorker::GetAuthorityLossImminent(ComponentId componentId) const {
...
SpatialOsWorker::GetCommandRequests(ComponentId componentId) const {
...
const std::vector<EntityComponentData>&
SpatialOsWorker::GetCompleteUpdates(ComponentId componentId) const {
  return currentViewDelta->GetComponentCompleteUpdates(componentId);
}

const std::vector<EntityComponentUpdate>&
SpatialOsWorker::GetEvents(ComponentId componentId) const {
