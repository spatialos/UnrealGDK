...
#include "gdk/op_lists/received_op_list.h"
...
void SpatialOsConnectionHandler::PollOps() {
  ...
  for (std::uint32_t i = 0; i < opList->op_count; ++i) {
    ...
    case WORKER_OP_TYPE_RESERVE_ENTITY_IDS_RESPONSE:
      requestId = &op.op.reserve_entity_ids_response.request_id;
      isResponse = true;
      break;
    case WORKER_OP_TYPE_CREATE_ENTITY_RESPONSE:
      requestId = &op.op.create_entity_response.request_id;
      isResponse = true;
      break;
    case WORKER_OP_TYPE_DELETE_ENTITY_RESPONSE:
      requestId = &op.op.delete_entity_response.request_id;
      isResponse = true;
      break;
    case WORKER_OP_TYPE_ENTITY_QUERY_RESPONSE:
      requestId = &op.op.entity_query_response.request_id;
      isResponse = true;
      break;
    case WORKER_OP_TYPE_COMMAND_RESPONSE:
      requestId = &op.op.command_response.request_id;
      isResponse = true;
      break;
    default:
      requestId = nullptr;
...
void SpatialOsConnectionHandler::SendAndClearMessages(MessagesToSend* messages) {
  ...
  for (auto& update : messages->Updates) {
    ...
    Worker_Connection_SendComponentUpdate(connection.get(), update.EntityId, &u, &loopback);
  }
  ...
  for (auto& request : messages->CommandRequests) {
    auto r = Worker_CommandRequest{nullptr, request.Request.GetComponentId(),
                                   request.Request.GetCommandIndex(),
                                   std::move(request.Request).Release(), nullptr};
    const auto id = Worker_Connection_SendCommandRequest(connection.get(), request.EntityId, &r,
                                                         &request.Timeout, &shortCircuit);
  ...
  for (auto& response : messages->CommandResponse) {
    auto r = Worker_CommandResponse{nullptr, response.Response.GetComponentId(),
                                    response.Response.GetCommandIndex(),
