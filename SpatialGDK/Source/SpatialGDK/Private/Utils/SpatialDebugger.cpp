...
void ASpatialDebugger::OnEntityAdded(const Worker_EntityId EntityId)
{
	...
	if (AActor* Actor = Cast<AActor>(NetDriver->PackageMap->GetObjectFromEntityId(EntityId).Get()))
	{
		...
		if (Actor->IsA<APlayerController>())
		{
			...
			{
				LocalPlayerController->InputComponent->BindKey(ConfigUIToggleKey, IE_Pressed, this, &ASpatialDebugger::OnToggleConfigUI).bConsumeInput = false;
				LocalPlayerController->InputComponent->BindKey(SelectActorKey, IE_Pressed, this, &ASpatialDebugger::OnSelectActor).bConsumeInput = false;
				LocalPlayerController->InputComponent->BindKey(HighlightActorKey, IE_Pressed, this, &ASpatialDebugger::OnHighlightActor).bConsumeInput = false;
			}
		}
	}
}
