...
FPendingRPCParams CreateMockParameters(UObject* TargetObject, ERPCType Type)
{
	...
	return FPendingRPCParams{ ObjectRef, Type, MoveTemp(Payload), FSpatialGDKSpanId() };
}
} // anonymous namespace
...
RPCCONTAINER_TEST(GIVEN_a_container_WHEN_one_value_has_been_added_THEN_it_is_queued)
{
	...
	RPCs.ProcessOrQueueRPC(Params.ObjectRef, Params.Type, MoveTemp(Params.Payload), FSpatialGDKSpanId{});
...
RPCCONTAINER_TEST(GIVEN_a_container_WHEN_multiple_values_of_same_type_have_been_added_THEN_they_are_queued)
{
	...
	RPCs.ProcessOrQueueRPC(Params1.ObjectRef, Params1.Type, MoveTemp(Params1.Payload), FSpatialGDKSpanId{});
	RPCs.ProcessOrQueueRPC(Params2.ObjectRef, Params2.Type, MoveTemp(Params2.Payload), FSpatialGDKSpanId{});
...
RPCCONTAINER_TEST(GIVEN_a_container_WHEN_multiple_values_of_different_type_have_been_added_THEN_they_are_queued)
{
	...
	RPCs.ProcessOrQueueRPC(ParamsUnreliable.ObjectRef, ParamsUnreliable.Type, MoveTemp(ParamsUnreliable.Payload), FSpatialGDKSpanId{});
	RPCs.ProcessOrQueueRPC(ParamsReliable.ObjectRef, ParamsReliable.Type, MoveTemp(ParamsReliable.Payload), FSpatialGDKSpanId{});
...
RPCCONTAINER_TEST(GIVEN_a_container_storing_multiple_values_of_different_type_WHEN_processed_once_THEN_values_have_been_processed_in_order)
{
	...
	for (int i = 0; i < 4; ++i)
	{
		...
		RPCs.ProcessOrQueueRPC(ObjectRef, ParamsUnreliable.Type, MoveTemp(ParamsUnreliable.Payload), FSpatialGDKSpanId{});
		RPCs.ProcessOrQueueRPC(ObjectRef, ParamsReliable.Type, MoveTemp(ParamsReliable.Payload), FSpatialGDKSpanId{});
	}
