...
DEFINE_LATENT_AUTOMATION_COMMAND_ONE_PARAMETER(FCleanupWorld, TSharedPtr<TestData>, Data);
bool FCleanupWorld::Update()
{
	for (auto Pair : Data->TestActors)
	{
		Data->TestWorld->DestroyActor(Pair.Value, true);
	}
...
OWNERSHIPLOCKINGPOLICY_TEST(GIVEN_an_actor_has_not_been_locked_WHEN_IsLocked_is_called_THEN_returns_false_with_no_lock_tokens)
{
	...
	ADD_LATENT_AUTOMATION_COMMAND(FCleanupWorld(Data));
