...
bool FSpatialGDKEditorToolbarModule::ValidateGeneratedLaunchConfig() const
{
	...
	{
		const EAppReturnType::Type Result = FMessageDialog::Open(EAppMsgType::YesNo, FText::FromString(FString::Printf(TEXT("The autogenerated launch config does not have a worker section for the UnrealWorker. This is required to launch a deployment. \n\nDo you want to configure your launch config settings now?"))));
	...
	// Ensure that there are no additional server workers in the launch config.
	if(!SpatialGDKRuntimeSettings->bUsingOffloading && LaunchConfigDescription.Workers.Num() > 1)
	{
		const EAppReturnType::Type Result = FMessageDialog::Open(EAppMsgType::YesNo, FText::FromString(FString::Printf(TEXT("Offloading is currently disabled but the autogenerated lauch config contains worker sections for offloaded workers. Either enable offloading or remove the worker sections for the offloaded workers.\n\nDo you want to configure your launch config settings now?"))));
