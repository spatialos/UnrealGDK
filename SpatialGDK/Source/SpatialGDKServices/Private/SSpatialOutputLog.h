...
struct FSpatialLogMessage
{
	...
	FSpatialLogMessage(const TSharedRef<FString>& NewMessage, FName NewCategory, FName NewStyle = NAME_None)
		: Message(NewMessage)
	...
	FSpatialLogMessage(const TSharedRef<FString>& NewMessage, ELogVerbosity::Type NewVerbosity, FName NewCategory, FName NewStyle = NAME_None)
		: Message(NewMessage)
...
struct FSpatialLogFilter
{
	...
	FSpatialLogFilter() : TextFilterExpressionEvaluator(ETextFilterExpressionEvaluatorMode::BasicString)
	{
	...
	bool IsMessageAllowed(const TSharedPtr<FSpatialLogMessage>& Message);
...
class FSpatialOutputLogTextLayoutMarshaller : public FBaseTextLayoutMarshaller
{
public:

	static TSharedRef< FSpatialOutputLogTextLayoutMarshaller > Create(TArray< TSharedPtr<FSpatialLogMessage> > InMessages, FSpatialLogFilter* InFilter);

	virtual ~FSpatialOutputLogTextLayoutMarshaller();
...
protected:
	...
	FSpatialOutputLogTextLayoutMarshaller(TArray< TSharedPtr<FSpatialLogMessage> > InMessages, FSpatialLogFilter* InFilter);

	void AppendMessageToTextLayout(const TSharedPtr<FSpatialLogMessage>& InMessage);
	void AppendMessagesToTextLayout(const TArray<TSharedPtr<FSpatialLogMessage>>& InMessages);
...
public:
	SLATE_BEGIN_ARGS( SSpatialOutputLog )
		...
		SLATE_ARGUMENT( TArray< TSharedPtr<FSpatialLogMessage> >, Messages )
	 ...
	 */
	static bool CreateLogMessages(const TCHAR* V, ELogVerbosity::Type Verbosity, const class FName& Category, TArray< TSharedPtr<FSpatialLogMessage> >& OutMessages);
...
protected:
	...
	TSharedPtr< FSpatialOutputLogTextLayoutMarshaller > MessagesTextMarshaller;
