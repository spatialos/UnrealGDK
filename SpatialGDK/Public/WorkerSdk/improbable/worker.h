...
enum class ComponentUpdateLoopback { kNone = 0, kShortCircuited = 1 };
...
/** Update parameters. Used to modify the behaviour of sending a component update. */
struct UpdateParameters {
  /**
   * Controls how the update is sent back to the worker from which it was sent.
   */
  ComponentUpdateLoopback Loopback;
};
 ...
 */
class IMPROBABLE_DLL_API Locator {
public:
  /**
 ...
 */
class IMPROBABLE_DLL_API Connection {
public:
  /**
  ...
  void SendComponentUpdate(EntityId entity_id, const typename T::Update& update,
                           const UpdateParameters& parameters = {
                               ComponentUpdateLoopback::kShortCircuited});
   ...
   *
   * Note that the sent component update is added as an operation to the operation list and will be
   * returned by a subsequent call to GetOpList() unless Loopback is set to
   * ComponentUpdateLoopback::kNone.
   */
  template <typename T>
 ...
 */
class IMPROBABLE_DLL_API OpList {
public:
  // Noncopyable, but movable.
 ...
 */
class IMPROBABLE_DLL_API HistogramMetric {
public:
  /** A histogram bucket. */
  struct Bucket {
    /** The upper bound. */
 ...
 */
struct IMPROBABLE_DLL_API Metrics {
 ...
 */
class IMPROBABLE_DLL_API Entity {
public:
  Entity() {}
 ...
 * This object should not be modified concurrently by multiple threads.
 */
class IMPROBABLE_DLL_API Dispatcher {
public:
  template <typename T>
...
class IMPROBABLE_DLL_API SnapshotInputStream {
public:
...
private:
  const detail::ComponentInfo& component_info;
  std::unique_ptr<WorkerProtocol_SnapshotInputStream, void (*)(WorkerProtocol_SnapshotInputStream*)>
      input_stream;
};

/** A stream for writing snapshots one entity at a time. */
class IMPROBABLE_DLL_API SnapshotOutputStream {
public:
...
private:
  const detail::ComponentInfo& component_info;
  std::unique_ptr<WorkerProtocol_SnapshotOutputStream,
                  void (*)(WorkerProtocol_SnapshotOutputStream*)>
      output_stream;
};
...
#include <improbable/detail/worker.i.h>
