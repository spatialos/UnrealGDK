...
typedef enum WorkerProtocol_ComponentUpdateLoopback {
  WORKER_PROTOCOL_COMPONENT_UPDATE_LOOPBACK_NONE = 0,
  WORKER_PROTOCOL_COMPONENT_UPDATE_LOOPBACK_SHORT_CIRCUITED = 1,
} WorkerProtocol_ComponentUpdateLoopback;
...
typedef struct WorkerProtocol_UpdateParameters {
  /**
   * Controls how the update is sent back to the worker from which it was sent. Defined in the
   * WorkerProtocol_ComponentUpdateLoopback enumeration.
   */
  uint8_t Loopback;
} WorkerProtocol_UpdateParameters;
...
CORE_SDK_API void WorkerProtocol_Connection_SendComponentUpdate(
    WorkerProtocol_Connection* connection, WorkerProtocol_EntityId entity_id,
    const WorkerProtocol_ComponentHandle* component_update,
    const WorkerProtocol_UpdateParameters* update_parameters);
/**
