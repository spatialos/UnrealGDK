...
void FSpatialOutputDevice::Serialize(const TCHAR *InData,
  ...
  if (Connection.IsValid()) {
    Connection->SendLogMessage(ConvertLogLevelToSpatial(Verbosity),
                               TCHAR_TO_UTF8(*Name), TCHAR_TO_UTF8(InData));
  }
}
...
void FSpatialOutputDevice::SetVerbosityFilterLevel(
    ELogVerbosity::Type Verbosity) {
  FilterLevel = Verbosity;
}

worker::LogLevel
FSpatialOutputDevice::ConvertLogLevelToSpatial(ELogVerbosity::Type Verbosity) {
  switch (Verbosity) {
