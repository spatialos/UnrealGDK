						   ...
						   FTimerManager* InTimerManager)
{
	...
	for (UClass* TypeBindingClass : TypeBindingClasses)
	{
		...
		UE_LOG(LogSpatialOSInterop, Log,
			   TEXT("Registered type binding class %s handling replicated properties of %s."),
	...
	const TArray<uint16>& RepChanged, const TArray<uint16>& MigChanged)
{
	...
	if (PinnedConnection.IsValid())
	{
		...
		else
		{
			...
			auto Entity =
				improbable::unreal::FEntityBuilder::Begin()
					...
					// For now, just a dummy component we add to every such entity to make sure
					// client has
		...
		UE_LOG(
			LogSpatialOSInterop, Log,
			TEXT(
				"%s: Creating entity for actor %s (%lld) using initial changelist. Request ID: %d"),
										...
										const TArray<uint16>& MigChanged)
{
	...
	if (!Binding)
	{
		// UE_LOG(LogSpatialOSInterop, Warning, TEXT("SpatialUpdateInterop: Trying to send Spatial
								...
								FFrame* const Frame)
{
	...
	{
		UE_LOG(LogSpatialOSInterop, Warning,
			   TEXT("SpatialUpdateInterop: Trying to send RPC on unsupported class %s."),
...
{
	UE_LOG(LogSpatialOSInterop, Log,
		   TEXT("Resolving pending object refs and RPCs which depend on object: %s %s."),
...
{
	// Attempt to trigger command request by calling the passed RPC request function. This function
	// is
	// generated in the type binding
	// classes which capture the target actor and arguments of the RPC (unpacked from the FFrame) by
	// value, and attempts to serialize
	// the RPC arguments to a command request.
	//
	// The generated function tries to first resolve the target actor using the package map, then
	// any
	// UObject arguments. If any of these
	// fail, `Function()` will return with Result.UnresolvedObject being set to the object which was
	// unable to be resolved by the package
	// map. We then queue this RPC until that object becomes resolved. If all objects are resolved
	...
	{
		// Add to outgoing RPCs if reliable. Otherwise, do nothing, as we don't bother retrying if
		// unreliable.
		if (bReliable)
		{
	...
	if (StatusCode != worker::StatusCode::kSuccess)
	{
		if (RetryContext->NumAttempts < SpatialConstants::MAX_NUMBER_COMMAND_ATTEMPTS)
		{
			...
			TimerCallback.BindLambda([this, RetryContext]() {
				...
				// As it's possible for an entity to leave a workers checkout radius (and thus
				// become
				// unresolved again), we cannot
				// take for granted that retrying the command will mean that the object is always
				// resolved.
				// In that case, just
															...
															uint16 Handle)
{
	...
	UE_LOG(
		LogSpatialOSPackageMap, Log,
		TEXT("Added pending outgoing object ref depending on object: %s, channel: %s, handle: %d."),
...
void USpatialInterop::ResolvePendingOutgoingRPCs(UObject* Object)
{
	...
	if (RPCList)
	{
		...
		{
			// We can guarantee that SendCommandRequest won't populate PendingOutgoingRPCs[Object]
			// whilst
