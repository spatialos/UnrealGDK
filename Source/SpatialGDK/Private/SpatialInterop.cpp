	...
	for (UClass* TypeBindingClass : TypeBindingClasses)
	{
		UClass* BoundClass =
			TypeBindingClass->GetDefaultObject<USpatialTypeBinding>()
				->GetBoundClass();
		UE_LOG(LogSpatialOSInterop, Log, TEXT("Registered type binding class %s handling replicated "
											  "properties of %s."),
			   *TypeBindingClass->GetName(),
			   *BoundClass->GetName());
	...
	if (PinnedConnection.IsValid())
	{
		...
		if (TypeBinding)
		{
			auto Entity = TypeBinding->CreateActorEntity(
				PlayerWorkerId, Location, PathStr, Channel->GetChangeState(RepChanged, MigChanged), Channel);
								...
								FFrame* const Frame)
{
	...
	if (!Binding)
	{
		UE_LOG(LogSpatialOSInterop, Warning, TEXT("SpatialUpdateInterop: Trying to send RPC on unsupported class "
												  "%s."),
...
void USpatialInterop::HandleCommandResponse_Internal(
	const FString& RPCName, FUntypedRequestId RequestId, const FEntityId&, const worker::StatusCode& StatusCode, const FString& Message)
{
	...
	if (StatusCode != worker::StatusCode::kSuccess)
	{
		...
		{
			UE_LOG(LogSpatialOSInterop, Error, TEXT("%s: failed too many times, giving up (%u "
...
void USpatialInterop::QueueOutgoingObjectRepUpdate_Internal(
	UObject* UnresolvedObject, USpatialActorChannel* DependentChannel, uint16 Handle)
{
	check(UnresolvedObject);
	check(DependentChannel);
	check(Handle > 0);

	UE_LOG(LogSpatialOSPackageMap, Log, TEXT("Added pending outgoing object ref depending on object: %s, "
											 "channel: %s, handle: %d."),
		   *UnresolvedObject->GetName(),
		   *DependentChannel->GetName(),
		   Handle);
...
void USpatialInterop::QueueOutgoingRPC_Internal(
	UObject* UnresolvedObject, FRPCCommandRequestFunc CommandSender, bool bReliable)
{
	check(UnresolvedObject);
	...
	const FRepHandleData* RepHandleData)
{
	check(DependentChannel);
	check(RepHandleData);
	UE_LOG(LogSpatialOSPackageMap, Log, TEXT("Added pending incoming object ref depending on object "
