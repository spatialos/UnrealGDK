...
FPlayerSpawnRequestSender::FPlayerSpawnRequestSender()
    : TimerManager(nullptr), Connection(nullptr), View(nullptr),
      NumberOfAttempts(0u) {}
...
void FPlayerSpawnRequestSender::SendPlayerSpawnRequest() {
  Connection->SendCommandRequest<SpawnPlayerCommand>(
      SpatialConstants::SPAWNER_ENTITY_ID, Request, 0);
  ++NumberOfAttempts;

  ResponseCallbackKey = View->OnCommandResponse<SpawnPlayerCommand>(
      std::bind(&FPlayerSpawnRequestSender::HandlePlayerSpawnResponse, this,
                std::placeholders::_1));
}
...
void FPlayerSpawnRequestSender::HandlePlayerSpawnResponse(
  ...
  if (Op.StatusCode == worker::StatusCode::kSuccess) {
    UE_LOG(LogSpatialOSPlayerSpawner, Display,
           TEXT("Player spawned sucessfully"));
  } else if (NumberOfAttempts < SpatialConstants::MAX_NUMBER_COMMAND_ATTEMPTS) {
    UE_LOG(LogSpatialOSPlayerSpawner, Warning,
           TEXT("Player spawn request failed: \"%s\""),
           *FString(Op.Message.c_str()));

    FTimerHandle RetryTimer;
    FTimerDelegate TimerCallback;
    TimerCallback.BindLambda(
        [this, RetryTimer]() { SendPlayerSpawnRequest(); });

    TimerManager->SetTimer(
        RetryTimer, TimerCallback,
        SpatialConstants::GetCommandRetryWaitTimeSeconds(NumberOfAttempts),
        false);
  } else {
    UE_LOG(LogSpatialOSPlayerSpawner, Fatal,
           TEXT("Player spawn request failed too many times. (%u attempts)"),
