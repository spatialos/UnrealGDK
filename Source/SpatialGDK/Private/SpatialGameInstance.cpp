...
bool USpatialGameInstance::StartGameInstance_SpatialGDKClient(FString& Error)
{
	...
	if (!WorldContext->PendingNetGame->NetDriver)
	{
		...
		GetEngine()->BroadcastTravelFailure(
			WorldContext->World(), ETravelFailure::PendingNetGameCreateFailure, WorldContext->PendingNetGame->ConnectionError);
...
#if WITH_EDITOR
FGameInstancePIEResult USpatialGameInstance::StartPlayInEditorGameInstance(
	ULocalPlayer* LocalPlayer, const FGameInstancePIEParameters& Params)
{
	...
	ULevelEditorPlaySettings const* PlayInSettings =
		GetDefault<ULevelEditorPlaySettings>();
	const EPlayNetMode PlayNetMode = [&PlayInSettings] {
		EPlayNetMode NetMode(PIE_Standalone);
		return (PlayInSettings->GetPlayNetMode(NetMode) ? NetMode : PIE_Standalone);
	}();
...
void USpatialGameInstance::StartGameInstance()
{
	...
	{
		UE_LOG(
			LogSpatialGDK, Fatal, TEXT("Unable to browse to starting map: %s. Application will now exit."), *Error);
