...
FNetworkGUID USpatialPackageMapClient::ResolveEntityActor(
	AActor* Actor, FEntityId EntityId, const SubobjectToOffsetMap& SubobjectToOffset)
{
	...
	for (UObject* Subobject : ActorSubobjects)
	{
		...
		if (OffsetIterator != SubobjectToOffset.end())
		{
			...
			UE_LOG(LogSpatialOSPackageMap, Log, TEXT("Registered new object ref for subobject %s inside "
				...
				*ObjectRefToString(SubobjectRef));
	...
	const improbable::unreal::UnrealLevelData& LevelData)
{
	...
	for (TActorIterator<AActor> Itr(World); Itr; ++Itr)
	{
		...
		UE_LOG(
			LogSpatialOSPackageMap, Log, TEXT("%s: Static object in world. Name: %s. Path: %s. Replicated: %d"), *Interop->GetSpatialOS()->GetWorkerId(), *Actor->GetName(), *PathName, (int)Actor->GetIsReplicated());
	}
