...
{
  // todo-giray: Uncomment the rest when worker sdk finishes the FR that gracefully handles removing
...
bool USpatialActorChannel::ReplicateActor()
{
  ...
  if (RepFlags.bNetInitial && OpenedLocally)
  {
    ...
    // NetGUID/Class/etc.
    // We don't care about this as we can distinguish this already based on the components in the
    // entity,
  ...
  // However, we have many player controllers sharing one connection, so we do it at the actor level
  // before replication.
  ...
  if (RepFlags.bNetInitial || RepChanged.Num() > 0 || MigratableChanged.Num() > 0)
  {
    ...
    {
      // When a player is connected, a FUniqueNetIdRepl is created with the players worker ID. This
      // eventually gets stored
      // inside APlayerState::UniqueId when UWorld::SpawnPlayActor is called. If this actor channel
      ...
      else
      {
        UE_LOG(LogSpatialGDKActorChannel, Log,
               TEXT("Unable to find PlayerState for %s, this usually means that this actor is not "
                    "owned by a player."),
               *Actor->GetClass()->GetName());
      }

      // Ensure that the initial changelist contains _every_ property. This ensures that the default
...
void USpatialActorChannel::SetChannelActor(AActor* InActor)
{
  ...
  {
    UE_LOG(LogSpatialGDKActorChannel, Log,
           TEXT("Opened channel for actor %s with existing entity ID %lld."), *InActor->GetName(),
...
void USpatialActorChannel::OnReserveEntityIdResponse(const worker::ReserveEntityIdResponseOp& Op)
{
  if (Op.StatusCode != worker::StatusCode::kSuccess)
  {
    ...
    // todo: From now on, this actor channel will be useless. We need better error handling, or a
...
void USpatialActorChannel::UpdateSpatialPosition()
{
  ...
  // If we're a pawn and are controlled by a player controller, update the player controller and the
