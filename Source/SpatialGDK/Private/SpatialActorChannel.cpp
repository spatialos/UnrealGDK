...
void USpatialActorChannel::SetChannelActor(AActor* InActor)
{
	...
	if (ActorEntityId == 0)
	{
		...
		UE_LOG(LogSpatialGDKActorChannel, Log, TEXT("Opened channel for actor %s with no entity ID. Initiated reserve entity ID. Request id: %d"), *InActor->GetName(), ReserveEntityIdRequestId.Id);
	}
	else
	{
