...
USpatialActorChannel::USpatialActorChannel(
    ...
    : Super(ObjectInitializer), ActorEntityId(0), ReserveEntityIdRequestId(-1),
      CreateEntityRequestId(-1), SpatialNetDriver(nullptr) {
  bCoreActor = true;
  bCreatingNewEntity = false;
}

void USpatialActorChannel::Init(UNetConnection *Connection, int32 ChannelIndex,
                                bool bOpenedLocally) {
...
void USpatialActorChannel::BindToSpatialView() {
  ...
  if (PinnedView.IsValid()) {
    ...
    CreateEntityCallback = PinnedView->OnCreateEntityResponse(
        [this](const worker::CreateEntityResponseOp &Op) {
...
bool USpatialActorChannel::ReplicateActor() {
  ...
  if (OpenPacketId.First == INDEX_NONE) {
    ...
    Bunch.bReliable = true; // Net temporary sends need to be reliable as well
  ...
  RepFlags.bReplay =
      ActorWorld && (ActorWorld->DemoNetDriver == Connection->GetDriver());
  RepFlags.bNetInitial = RepFlags.bNetInitial;

  UE_LOG(LogNetTraffic, Log,
         TEXT("Replicate %s, bNetInitial: %d, bNetOwner: %d"),
  ...
  FRepChangelistState *ChangelistState =
      ActorReplicator->ChangelistMgr->GetRepChangelistState();
  bool bWroteSomethingImportant = false;
  ActorReplicator->ChangelistMgr->Update(
      Actor, Connection->Driver->ReplicationFrame,
  ...
  // all together into a
  // single CL
  for (int32 i = ActorReplicator->RepState->LastChangelistIndex;
       i < ChangelistState->HistoryEnd; i++) {
    const int32 HistoryIndex = i % FRepChangelistState::MAX_CHANGE_HISTORY;
    FRepChangedHistory &HistoryItem =
        ChangelistState->ChangeHistory[HistoryIndex];
    TArray<uint16> Temp = RepChanged;
    ActorReplicator->RepLayout->MergeChangeList(
        (uint8 *)Actor, HistoryItem.Changed, Temp, RepChanged);
  }

  const bool bCompareIndexSame = ActorReplicator->RepState->LastCompareIndex ==
                                 ChangelistState->CompareIndex;
    ...
    for (auto &PropertyInfo : Binding->GetMigratableHandlePropertyMap()) {
      ...
      if (RepFlags.bNetInitial ||
          !PropertyInfo.Value.Property->Identical(
              MigratablePropertyShadowData.GetData() + ShadowDataOffset,
              Data)) {
  ...
  if (RepFlags.bNetInitial || RepChanged.Num() > 0 ||
      MigratableChanged.Num() > 0) {
    if (RepFlags.bNetInitial && bCreatingNewEntity) {
      ...
      CreateEntityRequestId = Interop->SendCreateEntityRequest(
          this, LastSpatialPosition, PlayerWorkerId, InitialRepChanged,
          MigratableChanged);
      bCreatingNewEntity = false;
    } else {
...
void USpatialActorChannel::SetChannelActor(AActor *InActor) {
  ...
  check(Interop);
  USpatialTypeBinding *Binding =
      Interop->GetTypeBindingByClass(InActor->GetClass());
  if (Binding) {
    ...
    uint32 Offset = 0;
    for (auto &Property : MigratableProperties) {
      Property.Value.Property->InitializeValue(
          MigratablePropertyShadowData.GetData() + Offset);
  ...
  ActorEntityId = SpatialNetDriver->GetEntityRegistry()
                      ->GetEntityIdFromActor(InActor)
                      .ToSpatialEntityId();

  // If the entity registry has no entry for this actor, this means we need to
  // create it.
  if (ActorEntityId == 0) {
    USpatialNetConnection *SpatialConnection =
        Cast<USpatialNetConnection>(Connection);
    ...
    if (PinnedConnection.IsValid()) {
      ReserveEntityIdRequestId =
          PinnedConnection->SendReserveEntityIdRequest(0);
    }
    UE_LOG(LogSpatialGDKActorChannel, Log,
...
void USpatialActorChannel::PreReceiveSpatialUpdate() { Actor->PreNetReceive(); }
...
void USpatialActorChannel::OnReserveEntityIdResponse(
    const worker::ReserveEntityIdResponseOp &Op) {
  if (Op.StatusCode != worker::StatusCode::kSuccess) {
    UE_LOG(LogSpatialGDKActorChannel, Error,
           TEXT("Failed to reserve entity id. Reason: %s"),
  ...
  UE_LOG(LogSpatialGDKActorChannel, Log,
         TEXT("Received entity id (%d) for: %s. Request id: %d"),
         Op.EntityId.value_or(0), *Actor->GetName(),
...
void USpatialActorChannel::OnCreateEntityResponse(
  ...
  if (Op.StatusCode != worker::StatusCode::kSuccess) {
    UE_LOG(LogSpatialGDKActorChannel, Error,
           TEXT("Failed to create entity for actor %s: %s"), *Actor->GetName(),
           UTF8_TO_TCHAR(Op.Message.c_str()));
    // todo: From now on, this actor channel will be useless. We need better
    // error handling, or
    // a
  ...
  UE_LOG(LogSpatialGDKActorChannel, Log,
         TEXT("Created entity (%lld) for: %s. Request id: %d"),
         ActorEntityId.ToSpatialEntityId(), *Actor->GetName(),
         ReserveEntityIdRequestId.Id);
  ...
  UE_LOG(LogSpatialGDKActorChannel, Log,
         TEXT("Received create entity response op for %lld"),
...
void USpatialActorChannel::UpdateSpatialPosition() {
  ...
  if (FVector::DistSquared(ActorSpatialPosition, LastSpatialPosition) <
      SpatialPositionThreshold) {
    return;
  }
  ...
  if (APawn *Pawn = Cast<APawn>(Actor)) {
    if (APlayerController *PlayerController =
      ...
      USpatialActorChannel *ControllerActorChannel = Cast<USpatialActorChannel>(
          Connection->ActorChannels.FindRef(PlayerController));
      if (ControllerActorChannel) {
        Interop->SendSpatialPositionUpdate(
            ControllerActorChannel->GetEntityId(), LastSpatialPosition);
      }
      USpatialActorChannel *PlayerStateActorChannel =
          Cast<USpatialActorChannel>(
              Connection->ActorChannels.FindRef(PlayerController->PlayerState));
      if (PlayerStateActorChannel) {
        Interop->SendSpatialPositionUpdate(
            PlayerStateActorChannel->GetEntityId(), LastSpatialPosition);
      }
    }
  }
}
