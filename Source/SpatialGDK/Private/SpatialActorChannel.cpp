...
namespace
{
// This is a bookkeeping function that is similar to the one in RepLayout.cpp, modified for our
// needs (e.g. no NaKs)
// We can't use the one in RepLayout.cpp because it's private and it cannot account for our
// approach.
// In this function, we poll for any changes in Unreal properties compared to the last time we
...
{
	// todo-giray: Uncomment the rest when worker sdk finishes the FR that gracefully handles
	// removing
...
bool USpatialActorChannel::ReplicateActor()
{
	...
	if (OpenPacketId.First == INDEX_NONE)
	{
		...
		Bunch.bReliable =
			true;  // Net temporary sends need to be reliable as well to force them to retry
	}

	// Here, Unreal would have determined if this connection belongs to this actor's Outer.
	// We don't have this concept when it comes to connections, our ownership-based logic is in the
	// interop layer.
	...
	{
		// TODO(David): Note that initial actor data is stored in the header to encode the
		// NetGUID/Class/etc.
		// We don't care about this as we can distinguish this already based on the components in
		// the
		// entity,
	...
	FMemMark MemMark(FMemStack::Get());  // The calls to ReplicateProperties will allocate memory on
	...
	// However, we have many player controllers sharing one connection, so we do it at the actor
	// level
	// before replication.
	...
	// Gather all change lists that are new since we last looked, and merge them all together into a
	// single CL
	...
	// todo-giray: We currently don't take replication of custom delta properties into account here
	...
	if (RepFlags.bNetInitial || RepChanged.Num() > 0 || MigratableChanged.Num() > 0)
	{
		...
		{
			// When a player is connected, a FUniqueNetIdRepl is created with the players worker ID.
			// This
			// eventually gets stored
			// inside APlayerState::UniqueId when UWorld::SpawnPlayActor is called. If this actor
			// channel
			...
			{
				UE_LOG(LogSpatialGDKActorChannel, Log, TEXT("Unable to find PlayerState for %s, this usually means that this actor "
															"is not "
															"owned by a player."),
					   *Actor->GetClass()->GetName());
			}

			// Ensure that the initial changelist contains _every_ property. This ensures that the
			// default
			// properties are written to the entity template.
			// Otherwise, there will be a mismatch between the rep state shadow data used by
			...
			// Calculate initial spatial position (but don't send component update) and create the
			// entity.
	...
	ActorReplicator->RepState->LastChangelistIndex = ChangelistState->HistoryEnd;
	// todo-giray: The rest of this function is taken from Unreal's own implementation. It is mostly
	// redundant in our case,
	// but keeping it here for now to give us a chance to investigate if we need to write our own
...
void USpatialActorChannel::SetChannelActor(AActor* InActor)
{
	...
	// Set up the shadow data for the migratable properties. This is used later to compare the
	...
	if (ActorEntityId == 0)
	{
		...
		// Mark this channel as being responsible for creating this entity once we have an entity
		// ID.
...
void USpatialActorChannel::OnReserveEntityIdResponse(const worker::ReserveEntityIdResponseOp& Op)
{
	if (Op.StatusCode != worker::StatusCode::kSuccess)
	{
		...
		// todo: From now on, this actor channel will be useless. We need better error handling, or
		// a
...
{
	// PlayerController's and PlayerState's are a special case here. To ensure that they and their
	// associated pawn are
	// migrated between workers at the same time (which is not guaranteed), we ensure that we update
	...
	// If we're a pawn and are controlled by a player controller, update the player controller and
	// the
