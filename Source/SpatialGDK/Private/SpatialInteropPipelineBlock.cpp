...
void USpatialInteropPipelineBlock::Init(UEntityRegistry *Registry,
  ...
  for (TObjectIterator<UClass> It; It; ++It) {
    if (It->IsChildOf(USpatialOsComponent::StaticClass()) &&
      ...
      USpatialOsComponent *CDO =
          Cast<USpatialOsComponent>((*It)->GetDefaultObject());
      KnownComponents.Emplace(CDO->GetComponentId(), *It);
    }
  }
}

void USpatialInteropPipelineBlock::AddEntity(
    const worker::AddEntityOp &AddEntityOp) {
  UE_LOG(LogSpatialGDKInteropPipelineBlock, Verbose,
         TEXT("USpatialInteropPipelineBlock: worker::AddEntityOp %lld"),
...
void USpatialInteropPipelineBlock::RemoveEntity(
...
void USpatialInteropPipelineBlock::AddComponent(
    UAddComponentOpWrapperBase *AddComponentOp) {
  UE_LOG(LogSpatialGDKInteropPipelineBlock, Verbose,
         TEXT("USpatialInteropPipelineBlock: worker::AddComponentOp component "
              "ID: %u entity ID: "
              "%lld inCriticalSection: %d"),
         AddComponentOp->ComponentId, AddComponentOp->EntityId,
         (int)bInCriticalSection);

  if (bInCriticalSection) {
    FPendingAddComponentWrapper Wrapper;
    Wrapper.EntityComponent = FComponentIdentifier{AddComponentOp->EntityId,
    ...
    InitialiseNewComponentImpl(
        FComponentIdentifier{AddComponentOp->EntityId,
                             AddComponentOp->ComponentId},
        AddComponentOp);
  }
...
void USpatialInteropPipelineBlock::RemoveComponent(
  ...
  UE_LOG(LogSpatialGDKInteropPipelineBlock, Verbose,
         TEXT("USpatialInteropPipelineBlock: worker::RemoveComponentOp "
...
void USpatialInteropPipelineBlock::ChangeAuthority(
    const worker::ComponentId ComponentId,
    const worker::AuthorityChangeOp &AuthChangeOp) {
  UE_LOG(LogSpatialGDKInteropPipelineBlock, Verbose,
         TEXT("USpatialInteropPipelineBlock: worker::ChangeAuthorityOp "
              "component ID: %u entity ID: "
              "%lld inCriticalSection: %d"),
  ...
  if (bInCriticalSection &&
      PendingAddEntities.Contains(FEntityId(AuthChangeOp.EntityId))) {
    PendingAuthorityChanges.Emplace(
...
void USpatialInteropPipelineBlock::LeaveCriticalSection() {
  ...
  TSharedPtr<worker::View> LockedView =
      NetDriver->GetSpatialOS()->GetView().Pin();
...
void USpatialInteropPipelineBlock::InitialiseNewComponentImpl(
  ...
  UClass *ComponentClass =
      KnownComponents.FindRef(FComponentId{ComponentIdentifier.ComponentId});
  ...
  // required components.
  AActor *Actor =
      EntityRegistry->GetActorFromEntityId(ComponentIdentifier.EntityId);
  if (Actor) {
    USpatialOsComponent *Component =
        Cast<USpatialOsComponent>(Actor->GetComponentByClass(ComponentClass));
    if (Component) {
      Component->Init(LockedConnection, LockedView,
    ...
    if (LevelAddComponentOp) {
      check(Actor);
      USpatialNetDriver *Driver =
          Cast<USpatialNetDriver>(Actor->GetWorld()->GetNetDriver());
      UNetConnection *Connection = Driver->GetSpatialOSNetConnection();
      USpatialPackageMapClient *PMC =
          Cast<USpatialPackageMapClient>(Connection->PackageMap);
...
void USpatialInteropPipelineBlock::RemoveEntityImpl(const FEntityId &EntityId) {
  ...
  NetDriver->GetSpatialInterop()->RemoveActorChannel(
      EntityId.ToSpatialEntityId());
  auto *PackageMap = Cast<USpatialPackageMapClient>(
      NetDriver->GetSpatialOSNetConnection()->PackageMap);
  PackageMap->RemoveEntityActor(EntityId);
}

void USpatialInteropPipelineBlock::ProcessOps(
    const TWeakPtr<SpatialOSView> &, const TWeakPtr<SpatialOSConnection> &,
    UWorld *, UCallbackDispatcher *) {}

AActor *USpatialInteropPipelineBlock::GetOrCreateActor(
    TSharedPtr<worker::Connection> LockedConnection,
  ...
  UE_LOG(LogSpatialGDKInteropPipelineBlock, Log,
         TEXT("Checked out entity with entity ID %lld"),
         EntityId.ToSpatialEntityId());
  ...
  if (EntityActor) {
    // Option 1
    UE_LOG(LogSpatialGDKInteropPipelineBlock, Log,
           TEXT("Entity for core actor %s has been checked out on the worker "
                "which spawned it."),
           *EntityActor->GetName());
    ...
    improbable::unreal::UnrealMetadataData *UnrealMetadataComponent =
        GetComponentDataFromView<improbable::unreal::UnrealMetadata>(LockedView,
                                                                     EntityId);
    check(UnrealMetadataComponent);

    USpatialPackageMapClient *PackageMap = Cast<USpatialPackageMapClient>(
        NetDriver->GetSpatialOSNetConnection()->PackageMap);
    check(PackageMap);

    FNetworkGUID NetGUID = PackageMap->ResolveEntityActor(
        EntityActor, EntityId,
        UnrealMetadataComponent->subobject_name_to_offset());
    UE_LOG(LogSpatialGDKInteropPipelineBlock, Log,
           TEXT("Received create entity response op for %d"),
           EntityId.ToSpatialEntityId());
  } else {
    ...
    if ((ActorClass = GetRegisteredEntityClass(MetadataComponent)) != nullptr) {
      // Option 2
      UE_LOG(LogSpatialGDKInteropPipelineBlock, Log,
             TEXT("Spawning a registered %s"), *ActorClass->GetName());
      EntityActor = SpawnNewEntity(PositionComponent, ActorClass);
      EntityRegistry->AddToRegistry(EntityId, EntityActor);
    } else if ((ActorClass = GetNativeEntityClass(MetadataComponent)) !=
               nullptr) {
      // Option 3
      UNetConnection *Connection = nullptr;
      improbable::unreal::UnrealMetadataData *UnrealMetadataComponent =
          GetComponentDataFromView<improbable::unreal::UnrealMetadata>(
              LockedView, EntityId);
      check(UnrealMetadataComponent);
      ...
      if (NetDriver->IsServer() &&
          ActorClass == APlayerController::StaticClass()) {
        checkf(
            !UnrealMetadataComponent->owner_worker_id().empty(),
            TEXT("A player controller entity must have an owner worker ID."));
        FString URLString = FURL().ToString();
        FString OwnerWorkerId = UTF8_TO_TCHAR(
            UnrealMetadataComponent->owner_worker_id().data()->c_str());
        URLString += TEXT("?workerId=") + OwnerWorkerId;
        Connection = NetDriver->AcceptNewPlayer(
            FURL(nullptr, *URLString, TRAVEL_Absolute), true);
        check(Connection);
        ...
        if (UnrealMetadataComponent->static_path().empty()) {
          UE_LOG(LogSpatialGDKInteropPipelineBlock, Log,
                 TEXT("Spawning a native dynamic %s whilst checking out an "
                      "entity."),
                 *ActorClass->GetFullName());
          EntityActor = SpawnNewEntity(PositionComponent, ActorClass);
        } else {
          FString FullPath = UTF8_TO_TCHAR(
              UnrealMetadataComponent->static_path().data()->c_str());
          UE_LOG(
      ...
      PackageMap->ResolveEntityActor(
          EntityActor, EntityId,
          UnrealMetadataComponent->subobject_name_to_offset());
      Channel->SetChannelActor(EntityActor);

      // Inform USpatialInterop of this new actor channel.
      NetDriver->GetSpatialInterop()->AddActorChannel(
          EntityId.ToSpatialEntityId(), Channel);
      ...
      if (NetDriver->ServerConnection) {
        if (EntityActor->IsA(APlayerController::StaticClass())) {
          uint8 PlayerIndex = 0;
          FInBunch Bunch(NetDriver->ServerConnection, &PlayerIndex,
                         sizeof(PlayerIndex));
...
AActor *USpatialInteropPipelineBlock::SpawnNewEntity(
    improbable::PositionData *PositionComponent, UClass *ActorClass) {
  FVector InitialLocation = SpatialConstants::SpatialOSCoordinatesToLocation(
      PositionComponent->coords());
  ...
  if (ActorClass) {
    ...
    FVector SpawnLocation = FRepMovement::RebaseOntoLocalOrigin(
        InitialLocation, World->OriginLocation);

    NewActor = World->SpawnActorAbsolute(
        ActorClass, FTransform(FRotator::ZeroRotator, InitialLocation),
        SpawnInfo);
    check(NewActor);
  }
...
UClass *USpatialInteropPipelineBlock::GetRegisteredEntityClass(
    improbable::MetadataData *MetadataComponent) {
  FString EntityTypeString =
      UTF8_TO_TCHAR(MetadataComponent->entity_type().c_str());
  ...
  if (!RegisteredClass) {
    ...
    if (EntityTypeString.FindLastChar('/', LastSlash)) {
      RegisteredClass = EntityRegistry->GetRegisteredEntityClass(
          EntityTypeString.RightChop(LastSlash));
    }
  }
...
// clients.
UClass *USpatialInteropPipelineBlock::GetNativeEntityClass(
    improbable::MetadataData *MetadataComponent) {
...
void USpatialInteropPipelineBlock::SetupComponentInterests(
    AActor *Actor, const FEntityId &EntityId,
