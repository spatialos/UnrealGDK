                                                   ...
                                                   const worker::AuthorityChangeOp& AuthChangeOp)
{
  ...
  // When a component is initialised, the callback dispatcher will automatically deal with authority
  // changes. Therefore, we need
  // to only queue changes if the entity itself has been queued for addition, which can only happen
    ...
    const FComponentIdentifier& ComponentIdentifier, UAddComponentOpWrapperBase* AddComponentOp)
{
  ...
  // An actor might not be created for a particular entity ID if that entity doesn't have all of the
  // required components.
                                               ...
                                               const FEntityId& EntityId)
{
  ...
  // 3) A SpawnActor() call that was initiated from a different worker, which means we need to find
    ...
    else if ((ActorClass = GetNativeEntityClass(MetadataComponent)) != nullptr)
    {
      ...
      else
      {
        ...
        else
        {
          ...
          UE_LOG(LogSpatialGDKInteropPipelineBlock, Log,
                 TEXT("Searching for a native static actor %s of class %s in the persistent level "
        ...
        {
          // TODO(David): Currently, we just create an actor channel on the "catch-all" connection,
          // then create a new actor channel once we check out the player controller
          // and create a new connection. This is fine due to lazy actor channel creation in
          // USpatialNetDriver::ServerReplicateActors. However, the "right" thing to do
          // would be to make sure to create anything which depends on the PlayerController _after_
      ...
      {
        NetDriver->GetSpatialInterop()->ReceiveAddComponent(Channel,
                                                            PendingAddComponent.AddComponentOp);
      }
