...
void USpatialNetDriver::PostInitProperties()
{
	...
	{
		// GuidCache will be allocated as an FNetGUIDCache above. To avoid an engine code change, we
...
void USpatialNetDriver::OnMapLoaded(UWorld* LoadedWorld)
{
	...
	{
		// In PIE, if we have more than 2 clients, then OnMapLoaded is going to be triggered once
		// each
		// client loads the world.
		// As the delegate is a global variable, it triggers all 3 USpatialNetDriver::OnMapLoaded
	...
	checkf(
		...
		TEXT("SpatialOS should not be connected already. This is probably because we attempted to "
...
void USpatialNetDriver::OnSpatialOSConnected()
{
	...
	// We currently don't make use of any of these as some are meaningless in a SpatialOS world, and
	...
	// If we're the server, we will spawn the special Spatial connection that will route all updates
	// to SpatialOS.
	// There may be more than one of these connections in the future for different replication
	// conditions.
	if (ServerConnection)
	{
	...
	else
	{
		USpatialNetConnection* Connection =
			NewObject<USpatialNetConnection>(GetTransientPackage(), NetConnectionClass);
		check(Connection);
		...
		// Since this is not a "real" client connection, we immediately pretend that it is fully
		// logged
		// on.
...
bool USpatialNetDriver::IsLevelInitializedForActor(const AActor* InActor,
												   const UNetConnection* InConnection) const
{
	// In our case, the connection is not specific to a client. Thus, it's not relevant whether the
	// level is initialized.
	return true;
}

// SpatialGDK: Functions in the ifdef block below are modified versions of the UNetDriver::
// implementations.
#if WITH_SERVER_CODE

// Returns true if this actor should replicate to *any* of the passed in connections
static FORCEINLINE_DEBUGGABLE bool
IsActorRelevantToConnection(const AActor* Actor, const TArray<FNetViewer>& ConnectionViewers)
{
	// SpatialGDK: Currently we're just returning true as a worker replicates all the known actors
	// in
	// our design.
...
// Returns true if this actor is considered dormant (and all properties caught up) to the current
// connection
static FORCEINLINE_DEBUGGABLE bool IsActorDormant(FNetworkObjectInfo* ActorInfo,
...
int32 USpatialNetDriver::ServerReplicateActors_PrepConnections(const float DeltaSeconds)
{
	...
	for (int32 ConnIdx = 0; ConnIdx < ClientConnections.Num(); ConnIdx++)
	{
		...
		//@note: we cannot add Connection->IsNetReady(0) here to check for saturation, as if that's
		//the
		// case we still want to figure out the list of relevant actors
		//			to reset their NetUpdateTime so that they will get sent as soon as the connection
		//is
		// no
		// longer saturated
		AActor* OwningActor = Connection->OwningActor;

		// SpatialGDK: We allow a connection without an owner to process if it's meant to be the
			...
			(Connection->Driver->Time - Connection->LastReceiveTime < 1.5f))
		{
			...
			// the view target is what the player controller is looking at OR the owning actor
			// itself when
			// using beacons
	...
	UNetConnection* Connection, const TArray<FNetViewer>& ConnectionViewers, const TArray<FNetworkObjectInfo*> ConsiderList, const bool bCPUSaturated, FActorPriority*& OutPriorityList, FActorPriority**& OutPriorityActors)
{
	...
	if (MaxSortedActors > 0)
	{
		...
		for (FNetworkObjectInfo* ActorInfo : ConsiderList)
		{
			...
			{
				// Channel is marked to go dormant now once all properties have been replicated (but
				// is not
				// dormant yet)
				Channel->StartBecomingDormant();
			}

			// SpatialGDK: This actor should only be replicated if GetNetworkConnection() matches
			// this
			// connection. However, if this actor doesn't have a connection
			// (which implies that it's owned by the server rather than a client), then it should
			// fall
			// back to the "catch all" SpatialOS connection which is
			// ClientConnections[0]. The below condition means that each actor should only be
			// replicated
			...
			// NOTE - We use NetTag to make sure SentTemporaries didn't already mark this actor to
			// be
			// skipped
			if (Actor->NetTag != NetTag)
			{
				...
				OutPriorityList[FinalSortedCount] = FActorPriority(
					PriorityConnection, Channel, ActorInfo, ConnectionViewers, bLowNetBandwidth);
	...
	UNetConnection* Connection, const TArray<FNetViewer>& ConnectionViewers, FActorPriority** PriorityActors, const int32 FinalSortedCount, int32& OutUpdated)
{
	...
	for (int32 j = 0; j < FinalSortedCount; j++)
	{
		...
		if (PriorityActors[j]->ActorInfo == NULL && PriorityActors[j]->DestructionInfo)
		{
			...
			{
				// This deletion entry is for an actor in a streaming level the connection doesn't
				// have
				// loaded, so skip it
				continue;
			}
		...
		if (!Channel || Channel->Actor)  // make sure didn't just close this channel
		{
			...
			// SpatialGDK: Here, Unreal would check (again) whether an actor is relevant. Removed
			// such
			// checks.
			...
			const bool bIsRecentlyRelevant =
				bIsRelevant || (Channel && Time - Channel->RelevantTime < RelevantTimeout);
			...
			if (bIsRecentlyRelevant)
			{
				...
				// or the package map doesn't support the actor's class/archetype (or the actor
				// itself in
				// the case of serializable actors)
				// or it's an editor placed actor and the client hasn't initialized the level it's
				// in
				if (Channel == NULL && GuidCache->SupportsObject(Actor->GetClass()) &&
					GuidCache->SupportsObject(Actor->IsNetStartupActor() ? Actor
																		 : Actor->GetArchetype()))
				{
					...
					// if we couldn't replicate it for a reason that should be temporary, and this
					// Actor is
				...
				if (Channel)
				{
					// if it is relevant then mark the channel as relevant for a short amount of
					// time
					if (bIsRelevant)
					{
					...
					if (Channel->IsNetReady(0))
					{
						...
						if (Channel->ReplicateActor())
						{
							...
							// Choose an optimal time, we choose 70% of the actual rate to allow
							// frequency to go
							// up if needed
							PriorityActors[j]->ActorInfo->OptimalNetUpdateDelta = FMath::Clamp(
								DeltaBetweenReplications * 0.7f, MinOptimalDelta, MaxOptimalDelta);
					...
					if (!Connection->IsNetReady(0))
					{
						// We can bail out now since this connection is saturated, we'll return how
						// far we got
						// though
						return j;
					}
			...
			// If the actor wasn't recently relevant, or if it was torn off, close the actor channel
			// if it
			// exists for this connection
			if ((!bIsRecentlyRelevant || Actor->bTearOff) && Channel != NULL)
			{
				// Non startup (map) actors have their channels closed immediately, which destroys
				// them.
...
// In our implementation, connections on the server do not represent clients. They represent direct
// connections to SpatialOS.
// For this reason, things like ready checks, acks, throttling based on number of updated
...
int32 USpatialNetDriver::ServerReplicateActors(float DeltaSeconds)
{
	...
	// Bump the ReplicationFrame value to invalidate any properties marked as "unchanged" for this
	// frame.
	ReplicationFrame++;

	const int32 NumClientsToTick = ServerReplicateActors_PrepConnections(DeltaSeconds);

	// SpatialGDK: This is a formality as there is at least one "perfect" Spatial connection in our
	// design.
	...
	for (int32 i = 0; i < ClientConnections.Num(); i++)
	{
		...
		{
			// SpatialGDK: This should not really happen in our case because we only replicate to
			// SpatialOS and not to individual clients. Leaving the code here just in case.

			// UE_LOG(LogNet, Log, TEXT("skipping update to %s"),*Connection->GetName());
			// then mark each considered actor as bPendingNetUpdate so that they will be considered
			// again
			...
			for (int32 ConsiderIdx = 0; ConsiderIdx < ConsiderList.Num(); ConsiderIdx++)
			{
				...
				if (Actor != NULL && !ConsiderList[ConsiderIdx]->bPendingNetUpdate)
				{
					// find the channel
					UActorChannel* Channel = Connection->ActorChannels.FindRef(Actor);
					// and if the channel last update time doesn't match the last net update time
					// for the
					// actor
					if (Channel != NULL &&
						Channel->LastUpdateTime < ConsiderList[ConsiderIdx]->LastNetUpdateTime)
					{
		...
		{
			// Make a list of viewers this connection should consider (this connection and children
			// of
			// this connection)
			...
			// relevant actors that could not be processed this frame are marked to be considered
			// for next
			// frame
			for (int32 k = LastProcessedActor; k < FinalSortedCount; k++)
			{
				...
				{
					// A deletion entry, skip it because we dont have anywhere to store a 'better
					// give higher
					// priority next time'
					continue;
				}
	...
	if (NumClientsToTick < ClientConnections.Num())
	{
		...
		{
			// move all the ticked connections to the end of the list so that the other connections
			// are
...
void USpatialNetDriver::ProcessRemoteFunction(AActor* Actor, UFunction* Function, void* Parameters, FOutParmRec* OutParms, FFrame* Stack, UObject* SubObject)
{
	...
	// The RPC might have been called by an actor directly, or by a subobject on that actor (e.g.
	// UCharacterMovementComponent).
	UObject* CallingObject = SubObject ? SubObject : Actor;

	// Reading properties from an FFrame changes the FFrame (internal pointer to the last property
	...
	// Shouldn't need to call Super here as we've replaced pretty much all the functionality in
	// UIpNetDriver
...
{
// Super::TickFlush() will not call ReplicateActors() because Spatial connections have InternalAck
// set to true.
// In our case, our Spatial actor interop is triggered through ReplicateActors() so we want to call
// it regardless.
...
USpatialNetConnection* USpatialNetDriver::AcceptNewPlayer(const FURL& InUrl, bool bExistingPlayer)
{
	...
	// We create a "dummy" connection that corresponds to this player. This connection won't
	// transmit
	// any data.
	// We may not need to keep it in the future, but for now it looks like path of least resistance
	// is
	...
	FUniqueNetIdRepl WorkerId(
		TSharedPtr<FSpatialWorkerUniqueNetId>(new FSpatialWorkerUniqueNetId(ClientWorkerId)));

	// We will now ask GameMode/GameSession if it's ok for this user to join.
	// Note that in the initial implementation, we carry over no data about the user here (such as a
	...
	if (bOk)
	{
		...
		{
			// Most of this is taken from "World->SpawnPlayActor", excluding the logic to spawn a
			// pawn
			...
			// We explicitly don't call GameMode->PostLogin(NewPlayerController) here, to avoid the
			// engine
...
void USpatialPendingNetGame::InitNetDriver()
{
	...
	// This is a trimmed down version of UPendingNetGame::InitNetDriver(). We don't send any Unreal
	...
	if (!GDisallowNetworkTravel)
	{
		...
		if (!NetDriver->InitConnect(this, URL, ConnectionError))
		{
			...
			// ConnectionError should be set by calling InitConnect...however, if we set NetDriver
			// to NULL
			// without setting a
			...
			{
				ConnectionError =
					NSLOCTEXT("Engine", "NetworkInit", "Error initializing network layer.")
						.ToString();
			}
		}
	}
