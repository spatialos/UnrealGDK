...
void USpatialNetDriver::OnMapLoaded(UWorld* LoadedWorld)
{
  ...
  {
    // In PIE, if we have more than 2 clients, then OnMapLoaded is going to be triggered once each
...
void USpatialNetDriver::OnSpatialOSConnected()
{
  ...
  else
  {
    ...
    // Since this is not a "real" client connection, we immediately pretend that it is fully logged
    // on.
...
{
  // SpatialGDK: Currently we're just returning true as a worker replicates all the known actors in
  // our design.
...
int32 USpatialNetDriver::ServerReplicateActors_PrepConnections(const float DeltaSeconds)
{
  ...
  for (int32 ConnIdx = 0; ConnIdx < ClientConnections.Num(); ConnIdx++)
  {
    ...
    //@note: we cannot add Connection->IsNetReady(0) here to check for saturation, as if that's the
    //case we still want to figure out the list of relevant actors
    //			to reset their NetUpdateTime so that they will get sent as soon as the connection is no
    //longer saturated
        ...
        (Connection->Driver->Time - Connection->LastReceiveTime < 1.5f))
    {
      ...
      // the view target is what the player controller is looking at OR the owning actor itself when
      // using beacons
    ...
    FActorPriority*& OutPriorityList, FActorPriority**& OutPriorityActors)
{
  ...
  if (MaxSortedActors > 0)
  {
    ...
    for (FNetworkObjectInfo* ActorInfo : ConsiderList)
    {
      ...
      {
        // Channel is marked to go dormant now once all properties have been replicated (but is not
        // dormant yet)
        Channel->StartBecomingDormant();
      }

      // SpatialGDK: This actor should only be replicated if GetNetworkConnection() matches this
      // connection. However, if this actor doesn't have a connection
      // (which implies that it's owned by the server rather than a client), then it should fall
      // back to the "catch all" SpatialOS connection which is
      // ClientConnections[0]. The below condition means that each actor should only be replicated
      ...
      // NOTE - We use NetTag to make sure SentTemporaries didn't already mark this actor to be
      // skipped
    ...
    FActorPriority** PriorityActors, const int32 FinalSortedCount, int32& OutUpdated)
{
  ...
  for (int32 j = 0; j < FinalSortedCount; j++)
  {
    ...
    if (PriorityActors[j]->ActorInfo == NULL && PriorityActors[j]->DestructionInfo)
    {
      ...
      {
        // This deletion entry is for an actor in a streaming level the connection doesn't have
        // loaded, so skip it
        continue;
      }
      ...
      if (Channel)
      {
        ...
        UE_LOG(
            LogNetTraffic, Log,
            TEXT(
                "Server replicate actor creating destroy channel for NetGUID <%s,%s> Priority: %d"),
    ...
    if (!Channel || Channel->Actor)  // make sure didn't just close this channel
    {
      ...
      // SpatialGDK: Here, Unreal would check (again) whether an actor is relevant. Removed such
      // checks.
      ...
      if (bIsRecentlyRelevant)
      {
        ...
        // or the package map doesn't support the actor's class/archetype (or the actor itself in
            ...
            GuidCache->SupportsObject(Actor->IsNetStartupActor() ? Actor : Actor->GetArchetype()))
        {
          ...
          // if we couldn't replicate it for a reason that should be temporary, and this Actor is
        ...
        if (Channel)
        {
          ...
          if (Channel->IsNetReady(0))
          {
            ...
            if (Channel->ReplicateActor())
            {
              ...
              // Calculate min delta (max rate actor will upate), and max delta (slowest rate actor
              // will update)
              ...
              // Choose an optimal time, we choose 70% of the actual rate to allow frequency to go
              // up if needed
          ...
          if (!Connection->IsNetReady(0))
          {
            // We can bail out now since this connection is saturated, we'll return how far we got
            // though
            return j;
          }
        }
      ...
      // If the actor wasn't recently relevant, or if it was torn off, close the actor channel if it
...
int32 USpatialNetDriver::ServerReplicateActors(float DeltaSeconds)
{
  ...
  for (int32 i = 0; i < ClientConnections.Num(); i++)
  {
    ...
    if (i >= NumClientsToTick)
    {
      ...
      // then mark each considered actor as bPendingNetUpdate so that they will be considered again
      ...
      for (int32 ConsiderIdx = 0; ConsiderIdx < ConsiderList.Num(); ConsiderIdx++)
      {
        ...
        if (Actor != NULL && !ConsiderList[ConsiderIdx]->bPendingNetUpdate)
        {
          // find the channel
          UActorChannel* Channel = Connection->ActorChannels.FindRef(Actor);
          // and if the channel last update time doesn't match the last net update time for the
          // actor
          if (Channel != NULL &&
    ...
    {
      // Make a list of viewers this connection should consider (this connection and children of
      // this connection)
      ...
      for (int32 k = LastProcessedActor; k < FinalSortedCount; k++)
      {
        ...
        {
          // A deletion entry, skip it because we dont have anywhere to store a 'better give higher
          // priority next time'
          continue;
        }
        ...
        {
          // If this actor was relevant but didn't get processed, force another update for next
          // frame
          UE_LOG(LogNetTraffic, Log,
                 TEXT(" Saturated. Mark %s NetUpdateTime to be checked for next tick"),
                 *Actor->GetName());
  ...
  if (NumClientsToTick < ClientConnections.Num())
  {
    ...
    {
      // move all the ticked connections to the end of the list so that the other connections are
...
USpatialNetConnection* USpatialNetDriver::AcceptNewPlayer(const FURL& InUrl, bool bExistingPlayer)
{
  ...
  // We create a "dummy" connection that corresponds to this player. This connection won't transmit
  // any data.
  // We may not need to keep it in the future, but for now it looks like path of least resistance is
  ...
  if (bOk)
  {
    ...
    {
      // Most of this is taken from "World->SpawnPlayActor", excluding the logic to spawn a pawn
      ...
      // We explicitly don't call GameMode->PostLogin(NewPlayerController) here, to avoid the engine
...
void USpatialPendingNetGame::InitNetDriver()
{
  ...
  if (!GDisallowNetworkTravel)
  {
    ...
    if (!NetDriver->InitConnect(this, URL, ConnectionError))
    {
      ...
      // ConnectionError should be set by calling InitConnect...however, if we set NetDriver to NULL
      // without setting a
