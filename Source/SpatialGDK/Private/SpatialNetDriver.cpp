...
static FORCEINLINE_DEBUGGABLE bool
ShouldActorGoDormant(AActor* Actor, const TArray<FNetViewer>& ConnectionViewers, UActorChannel* Channel, const float Time, const bool bLowNetBandwidth)
{
	...
	if (MaxSortedActors > 0)
	{
		...
		for (FNetworkObjectInfo* ActorInfo : ConsiderList)
		{
			...
			if (ActorConnection != Connection)
			{
				...
				{
					UE_LOG(
						LogSpatialOSNetDriver, Verbose, TEXT("Actor %s will be replicated on the catch-all connection"), *Actor->GetName());
				}
				else
				{
					continue;
...
void USpatialNetDriver::ProcessRemoteFunction(
	AActor* Actor, UFunction* Function, void* Parameters, FOutParmRec* OutParms, FFrame* Stack, UObject* SubObject)
{
	USpatialNetConnection* Connection =
...
void USpatialPendingNetGame::InitNetDriver()
{
	...
	if (!GDisallowNetworkTravel)
	{
		...
		if (!NetDriver->InitConnect(this, URL, ConnectionError))
		{
			...
			{
				ConnectionError = NSLOCTEXT("Engine", "NetworkInit", "Error initializing network layer.")
									  .ToString();
			}
		}
	}
