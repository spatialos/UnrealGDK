...
FSpatialGDKWorkerConfiguration::FSpatialGDKWorkerConfiguration(
  ...
  if (AppConfig.ProjectName.IsEmpty()) {
    UE_LOG(LogSpatialOS, Error,
           TEXT("The project's name must be set from code, or on the "
                "command line with +projectName."))
  }

  FParse::Value(commandLine, *FString("assemblyName"), AppConfig.AssemblyName);

  FParse::Value(commandLine, *FString("deploymentName"),
  ...
  FParse::Value(commandLine, *FString("deploymentTag"),
                AppConfig.DeploymentTag);
  ...
  if (AppConfig.WorkerId.IsEmpty()) {
    AppConfig.WorkerId =
        GenerateRandomWorkerIdFromTypeName(AppConfig.WorkerPlatform);
  }

  FParse::Value(commandLine, *FString("loginToken"), AppConfig.LoginToken);

  auto &NetworkConfig = WorkerConfigurationData.Networking;
  FParse::Value(commandLine, *FString("locatorHost"),
                NetworkConfig.LocatorHost);
  FParse::Value(commandLine, *FString("receptionistHost"),
                NetworkConfig.ReceptionistHost);
  ...
  FParse::Value(commandLine, *FString("receptionistPort"),
                NetworkConfig.ReceptionistPort);
  FParse::Value(commandLine, *FString("raknetHeartbeatTimeoutMs"),
                NetworkConfig.RaknetHeartbeatTimeoutMillis);
  FParse::Value(commandLine, *FString("receiveQueueCapacity"),
                NetworkConfig.ReceiveQueueCapacity);
  FParse::Value(commandLine, *FString("sendQueueCapacity"),
                NetworkConfig.SendQueueCapacity);
  FParse::Value(commandLine, *FString("steamToken"), NetworkConfig.SteamToken);
  FParse::Value(commandLine, *FString("tcpMultiplexLevel"),
                NetworkConfig.TcpMultiplexLevel);
  FParse::Bool(commandLine, *FString("tcpNoDelay"), NetworkConfig.TcpNoDelay);
  FParse::Value(commandLine, *FString("tcpReceiveBufferSize"),
                NetworkConfig.TcpReceiveBufferSize);
  FParse::Value(commandLine, *FString("tcpSendBufferSize"),
                NetworkConfig.TcpSendBufferSize);
  FParse::Bool(commandLine, *FString("useExternalIpForBridge"),
               NetworkConfig.UseExternalIp);

  auto &DebuggingConfig = WorkerConfigurationData.Debugging;
  FParse::Value(commandLine, *FString("infraServicesUrl"),
                DebuggingConfig.InfraServiceUrl);
  ...
  FParse::Value(commandLine, *FString("protocolLogPrefix"),
                DebuggingConfig.ProtocolLogPrefix);
  FParse::Value(commandLine, *FString("protocolLogMaxFileBytes"),
                DebuggingConfig.ProtocolLogMaxFileBytes);
  FParse::Value(commandLine, *FString("protocolLogMaxFiles"),
                DebuggingConfig.ProtocolLogMaxFiles);
  FParse::Bool(commandLine, *FString("showDebugTraces"),
               DebuggingConfig.ShowDebugTraces);
  FParse::Bool(commandLine, *FString("useInstrumentation"),
               DebuggingConfig.UseInstrumentation);

  FParse::Bool(commandLine, *FString("logDebugToSpatialOs"),
               DebuggingConfig.LogDebugToSpatialOs);
  FParse::Bool(commandLine, *FString("logAssertToSpatialOs"),
               DebuggingConfig.LogAssertToSpatialOs);
  FParse::Bool(commandLine, *FString("logWarningToSpatialOs"),
               DebuggingConfig.LogWarningToSpatialOs);
  FParse::Bool(commandLine, *FString("logErrorToSpatialOs"),
               DebuggingConfig.LogErrorToSpatialOs);
...
const worker::NetworkConnectionType
FSpatialGDKWorkerConfiguration::GetLinkProtocol() const {
...
const uint32
FSpatialGDKWorkerConfiguration::GetRaknetHeartbeatTimeoutMillis() const {
...
const uint32
FSpatialGDKWorkerConfiguration::GetBuiltInMetricsReportPeriodMillis() const {
  return WorkerConfigurationData.Debugging.BuiltInMetricsReportPeriodMillis;
}

const uint32
FSpatialGDKWorkerConfiguration::GetLogMessageQueueCapacity() const {
...
const uint32
FSpatialGDKWorkerConfiguration::GetProtocolLogMaxFileBytes() const {
...
void FSpatialGDKWorkerConfiguration::LogWorkerConfiguration() const {
  ...
  UE_LOG(LogSpatialOS, Display, TEXT("assemblyName = %s"),
         *AppConfig.AssemblyName);
  UE_LOG(LogSpatialOS, Display, TEXT("deploymentName = %s"),
         *AppConfig.DeploymentName);
  UE_LOG(LogSpatialOS, Display, TEXT("deploymentTag = %s"),
         *AppConfig.DeploymentTag);
  UE_LOG(LogSpatialOS, Display, TEXT("projectName = %s"),
         *AppConfig.ProjectName);
  UE_LOG(LogSpatialOS, Display, TEXT("workerType = %s"),
         *AppConfig.WorkerPlatform);
  ...
  UE_LOG(LogSpatialOS, Display, TEXT("locatorHost = %s"),
         *NetworkConfig.LocatorHost);
  UE_LOG(LogSpatialOS, Display, TEXT("receptionistHost = %s"),
         *NetworkConfig.ReceptionistHost);
  UE_LOG(LogSpatialOS, Display, TEXT("linkProtocol = %s"),
         NetworkConfig.LinkProtocol == worker::NetworkConnectionType::kTcp
             ? TEXT("Tcp")
             : TEXT("Raknet"));
  UE_LOG(LogSpatialOS, Display, TEXT("receptionistPort = %d"),
         NetworkConfig.ReceptionistPort);
  UE_LOG(LogSpatialOS, Display, TEXT("raknetHeartbeatTimeoutMs = %d"),
         NetworkConfig.RaknetHeartbeatTimeoutMillis);
  UE_LOG(LogSpatialOS, Display, TEXT("receiveQueueCapacity = %d"),
         NetworkConfig.ReceiveQueueCapacity);
  UE_LOG(LogSpatialOS, Display, TEXT("sendQueueCapacity = %d"),
         NetworkConfig.SendQueueCapacity);
  UE_LOG(LogSpatialOS, Display, TEXT("steamToken = %s"),
         *NetworkConfig.SteamToken);
  UE_LOG(LogSpatialOS, Display, TEXT("tcpMultiplexLevel = %d"),
         NetworkConfig.TcpMultiplexLevel);
  ...
  UE_LOG(LogSpatialOS, Display, TEXT("tcpReceiveBufferSize = %d"),
         NetworkConfig.TcpReceiveBufferSize);
  UE_LOG(LogSpatialOS, Display, TEXT("tcpSendBufferSize = %d"),
         NetworkConfig.TcpSendBufferSize);
  ...
  UE_LOG(LogSpatialOS, Display, TEXT("infraServicesUrl = %s"),
         *DebuggingConfig.InfraServiceUrl);
  ...
  UE_LOG(LogSpatialOS, Display, TEXT("protocolLoggingOnStartup = %s"),
         DebuggingConfig.ProtocolLoggingOnStartup ? TEXT("true")
                                                  : TEXT("false"));
  UE_LOG(LogSpatialOS, Display, TEXT("protocolLogPrefix = %s"),
         *DebuggingConfig.ProtocolLogPrefix);
