...
USpatialOS::USpatialOS()
    : WorkerConfiguration(), WorkerConnection(),
      bConnectionWasSuccessful(false) {
  EntityPipeline =
      CreateDefaultSubobject<UEntityPipeline>(TEXT("EntityPipeline"));
  CallbackDispatcher =
      CreateDefaultSubobject<UCallbackDispatcher>(TEXT("CallbackDispatcher"));
}
...
void USpatialOS::ApplyEditorWorkerConfiguration(FWorldContext &InWorldContext) {
  ...
  if (SpatialGDKSettings == nullptr ||
      !SpatialGDKSettings->bUseUserWorkerConfigurations) {
    return;
  }

  // InWorldContext.PIEInstance is 0 if one PIE instance is run otherwise [1,
  // ..., n] if n PIE
  // instances are run. See Engine\Source\Editor\UnrealEd\Private\PlayLevel.cpp,
  // line 2528.
  const int32 EditorConfigurationArrayIndex =
      FMath::Max(0, InWorldContext.PIEInstance - 1);
  const int32 NumWorkerConfigurations =
      SpatialGDKSettings->WorkerConfigurations.Num();
  if (EditorConfigurationArrayIndex < NumWorkerConfigurations) {
    ...
    WorkerConfiguration =
        FSpatialGDKWorkerConfiguration(WorkerConfig.WorkerConfigurationData);
    ...
    if (InWorldContext.GameViewport != nullptr) {
      InWorldContext.GameViewport->bDisableWorldRendering =
          WorkerConfig.bDisableRendering;
    }
  }
}

bool USpatialOS::IsConnected() const { return WorkerConnection.IsConnected(); }
...
void USpatialOS::Connect() {
  ...
  if (LockedView.IsValid()) {
    Callbacks.Add(LockedView->OnDisconnect(
        std::bind(&USpatialOS::OnDisconnectDispatcherCallback, this,
                  std::placeholders::_1)));
  }
  ...
  Params.EnableProtocolLoggingAtStartup =
      WorkerConfiguration.GetProtocolLoggingOnStartup();
  Params.LogMessageQueueCapacity =
      WorkerConfiguration.GetLogMessageQueueCapacity();
  ...
  Params.Network.Tcp.MultiplexLevel =
      WorkerConfiguration.GetTcpMultiplexLevel();
  Params.Network.Tcp.NoDelay = WorkerConfiguration.GetTcpNoDelay();
  Params.Network.Tcp.ReceiveBufferSize =
      WorkerConfiguration.GetTcpReceiveBufferSize();
  Params.Network.Tcp.SendBufferSize =
      WorkerConfiguration.GetTcpSendBufferSize();
  Params.Network.UseExternalIp = WorkerConfiguration.GetUseExternalIp();
  Params.ProtocolLogging.LogPrefix =
      TCHAR_TO_UTF8(*WorkerConfiguration.GetProtocolLogPrefix());
  Params.ProtocolLogging.MaxLogFiles =
      WorkerConfiguration.GetProtocolLogMaxFiles();
  Params.ProtocolLogging.MaxLogFileSizeBytes =
      WorkerConfiguration.GetProtocolLogMaxFileBytes();
  ...
  OnQueueStatus.BindLambda([this](const worker::QueueStatus &Status) {
    if (Status.Error) {
      ...
      UE_LOG(LogSpatialOS, Display, TEXT("Position in queue: %u"),
             Status.PositionInQueue);
      return true;
    }
  });

  const bool ShouldConnectViaLocator =
      !WorkerConfiguration.GetLoginToken().IsEmpty();
  if (ShouldConnectViaLocator) {
    WorkerConnection.ConnectToLocatorAsync(
        WorkerConfiguration.GetProjectName(),
        WorkerConfiguration.GetLocatorHost(),
        WorkerConfiguration.GetDeploymentName(),
        WorkerConfiguration.GetLoginToken(), Params, OnQueueStatus,
        OnConnected);
  } else {
...
void USpatialOS::Disconnect() {
  if (IsConnected()) {
    ...
    // Manually broadcast OnDisconnected callbacks as Dispatcher->OnDisconnected
    // will not be
    // called.
...
void USpatialOS::ProcessOps() { WorkerConnection.ProcessOps(); }

const FSpatialGDKWorkerConfiguration
USpatialOS::GetWorkerConfiguration() const {
...
void USpatialOS::OnDisconnectDispatcherCallback(
    const worker::DisconnectOp &Op) {
