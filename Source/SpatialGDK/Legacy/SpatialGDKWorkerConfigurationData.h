...
struct SPATIALGDK_API FSpatialGDKNetworkingConfiguration
{
	...
	: LocatorHost("locator.improbable.io"), ReceptionistHost("127.0.0.1"), LinkProtocol(worker::NetworkConnectionType::kRaknet), ReceptionistPort(7777), RaknetHeartbeatTimeoutMillis(worker::defaults::kRakNetHeartbeatTimeoutMillis), ReceiveQueueCapacity(worker::defaults::kReceiveQueueCapacity), SendQueueCapacity(worker::defaults::kSendQueueCapacity), SteamToken(""), TcpMultiplexLevel(worker::defaults::kTcpMultiplexLevel), TcpNoDelay(worker::defaults::kTcpNoDelay), TcpReceiveBufferSize(worker::defaults::kTcpReceiveBufferSize), TcpSendBufferSize(worker::defaults::kTcpSendBufferSize), UseExternalIp(worker::defaults::kUseExternalIp)
	{
	}
...
struct SPATIALGDK_API FSpatialGDKDebuggingConfig
{
	...
	: InfraServiceUrl("https://api.spatial.improbable.io"), LogDebugToSpatialOs(false), LogAssertToSpatialOs(false), LogWarningToSpatialOs(true), LogErrorToSpatialOs(true), BuiltInMetricsReportPeriodMillis(worker::defaults::kBuiltInMetricsReportPeriodMillis), LogMessageQueueCapacity(worker::defaults::kLogMessageQueueCapacity), ProtocolLoggingOnStartup(false), ProtocolLogPrefix(worker::defaults::kLogPrefix.c_str()), ProtocolLogMaxFileBytes(100U * worker::defaults::kMaxLogFileSizeBytes), ProtocolLogMaxFiles(worker::defaults::kMaxLogFiles), ShowDebugTraces(false), UseInstrumentation(true)
	{
	}
   ...
   * behaviour.
    */
	UPROPERTY(EditAnywhere, config, Category = WorkerConfiguration, meta = (DeprecatedFunction, DeprecationMessage = "This field is deprecated and will be "
...
struct SPATIALGDK_API FSpatialGDKApplicationConfig
{
	...
	: AppName(""), AssemblyName(""), DeploymentName(""), DeploymentTag("prod"), ProjectName(""), WorkerPlatform("UnrealClient"), WorkerId(""), LoginToken("")
	{
	}
