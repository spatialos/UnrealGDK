...
USpatialOSBlueprintLibrary::USpatialOSBlueprintLibrary(
...
FString
USpatialOSBlueprintLibrary::EntityIdToString(const FEntityId& EntityId)
{
	return ToString(EntityId.ToSpatialEntityId());
}

FEntityIdQueryConstraint
USpatialOSBlueprintLibrary::MakeEntityIdQueryConstraintFromEntityId(
	const FEntityId& InEntityId)
{
...
FComponentIdQueryConstraint
USpatialOSBlueprintLibrary::MakeComponentIdQueryConstraintFromInt(
	int InComponentId)
{
	return FComponentIdQueryConstraint(InComponentId);
}
	...
	const TArray<FSphereQueryConstraint>& Spheres)
{
	...
	for (const FComponentIdQueryConstraint& constraint : ComponentIds)
	{
		...
		ComponentConstraint.ComponentId =
			constraint.Underlying.ToSpatialComponentId();
...
bool USpatialOSBlueprintLibrary::IsSuccessful_Command(
	const FSpatialOSCommandResult& CommandResult)
{
	return CommandResult.StatusCode == ECommandResponseCode::Success;
}

ECommandResponseCode USpatialOSBlueprintLibrary::GetCommandStatusCode(
	const FSpatialOSCommandResult& CommandResult)
{
	return CommandResult.StatusCode;
}

FString USpatialOSBlueprintLibrary::GetCommandErrorMessage(
	const FSpatialOSCommandResult& CommandResult)
{
...
bool USpatialOSBlueprintLibrary::IsValid_FRequestId(
	const FRequestId& RequestId)
{
