...
USpatialOsComponent::USpatialOsComponent()
    : MaxUpdatesPerSecond(30), Connection(nullptr), View(nullptr), EntityId(0),
      Callbacks(nullptr), TimeUntilNextUpdate(0),
      Authority(EAuthority::NotAuthoritative), bIsComponentReady(false),
      bHasEventQueued(false), Commander(nullptr) {}

void USpatialOsComponent::Init(
  ...
  Callbacks.Reset(
      new improbable::unreal::callbacks::FScopedViewCallbacks(View));
}
...
void USpatialOsComponent::ApplyInitialAuthority(
    const worker::AuthorityChangeOp &AuthChangeOp) {
  OnAuthorityChangeDispatcherCallback(AuthChangeOp);
}

worker::EntityId USpatialOsComponent::GetEntityId() { return EntityId; }

bool USpatialOsComponent::HasAuthority() {
  return Authority == EAuthority::Authoritative ||
         Authority == EAuthority::AuthorityLossImminent;
}

EAuthority USpatialOsComponent::GetAuthority() { return Authority; }

bool USpatialOsComponent::IsComponentReady() { return bIsComponentReady; }
...
void USpatialOsComponent::OnAuthorityChangeDispatcherCallback(
...
void USpatialOsComponent::OnRemoveComponentDispatcherCallback(
    const worker::RemoveComponentOp &op) {
  if (op.EntityId != EntityId) {
    return;
  }
