...
enum class ECommandDelivery : uint8
{
	...
	ROUNDTRIP = 0,
	// Do not send commands over the network if the target entity is on the same worker as the
	// command
	// originator and the worker is authoritative over the target entity's component.
	// Faster, but less reliable as SpatialOS will not guarantee successful execution in certain
	// edge
	// cases.
	SHORT_CIRCUIT = 1,
};
