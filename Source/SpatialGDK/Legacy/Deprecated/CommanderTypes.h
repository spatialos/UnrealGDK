...
{
	// Default behavior, always sends commands over the network. A successful command response
	// guarantees that the command was fully executed on the target worker.
	ROUNDTRIP = 0,
	// Do not send commands over the network if the target entity is on the same worker as the command
	// originator and the worker is authoritative over the target entity's component.
	// Faster, but less reliable as SpatialOS will not guarantee successful execution in certain edge
	// cases.
	SHORT_CIRCUIT = 1,
};
