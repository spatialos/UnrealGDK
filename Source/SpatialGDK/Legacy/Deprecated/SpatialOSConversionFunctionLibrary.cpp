...
FRotator
USpatialOSConversionFunctionLibrary::GetSpatialOsToUnrealCoordinateSpace() {
...
USpatialOSConversionFunctionLibrary::UnrealCoordinatesToSpatialOsCoordinates(
  ...
  return GetSpatialOsToUnrealCoordinateSpace().GetInverse().RotateVector(
             unrealCoordinates) /
         GetSpatialOsToUnrealScale();
}

improbable::Coordinates USpatialOSConversionFunctionLibrary::
    UnrealCoordinatesToSpatialOsCoordinatesCast(
        const FVector &unrealCoordinates) {
  FVector spatialCoords =
      UnrealCoordinatesToSpatialOsCoordinates(unrealCoordinates);
  return improbable::Coordinates{spatialCoords.X, spatialCoords.Y,
                                 spatialCoords.Z};
}
...
USpatialOSConversionFunctionLibrary::SpatialOsCoordinatesToUnrealCoordinates(
  ...
  return GetSpatialOsToUnrealCoordinateSpace().RotateVector(
             spatialOsCoordinates) *
         GetSpatialOsToUnrealScale();
}

FQuat USpatialOSConversionFunctionLibrary::UnrealRotationToSpatialOsRotation(
    const FQuat &unrealRotation) {
  return GetSpatialOsToUnrealCoordinateSpace().GetInverse().Quaternion() *
         unrealRotation;
}
