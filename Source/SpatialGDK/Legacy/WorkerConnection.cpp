...
void LogMessage(const worker::LogMessageOp &Message) {
  ...
  case worker::LogLevel::kDebug:
    UE_LOG(LogSpatialOS, Log, TEXT("[debug] %s"),
           UTF8_TO_TCHAR(Message.Message.c_str()));
    break;
  case worker::LogLevel::kInfo:
    UE_LOG(LogSpatialOS, Log, TEXT("[info] %s"),
           UTF8_TO_TCHAR(Message.Message.c_str()));
    break;
  case worker::LogLevel::kWarn:
    UE_LOG(LogSpatialOS, Warning, TEXT("%s"),
           UTF8_TO_TCHAR(Message.Message.c_str()));
    break;
  case worker::LogLevel::kError:
    UE_LOG(LogSpatialOS, Error, TEXT("%s"),
           UTF8_TO_TCHAR(Message.Message.c_str()));
    break;
  case worker::LogLevel::kFatal:
    UE_LOG(LogSpatialOS, Error, TEXT("[fatal] %s"),
           UTF8_TO_TCHAR(Message.Message.c_str()));
  default:
    break;
  }
...
FWorkerConnection::FWorkerConnection()
    : bIsConnecting(false), View(nullptr), Connection(nullptr),
      Metrics(new worker::Metrics{}) {
  UE_LOG(LogSpatialOS, Log, TEXT("Initializing SpatialOS..."));

  View = TSharedPtr<SpatialOSView>(
      new SpatialOSView(improbable::unreal::Components{}));
  View->OnLogMessage(LogMessage);
  View->OnMetrics(
      std::bind(&FWorkerConnection::OnMetrics, this, std::placeholders::_1));
}

void FWorkerConnection::GetDeploymentListAsync(
    const FString &ProjectName, const FString &LocatorHost,
    const FString &LoginToken,
    FOnDeploymentsFoundDelegate OnDeploymentsFoundCallback,
    std::uint32_t TimeoutMillis) {
  AsyncTask(ENamedThreads::GameThread, [ProjectName, LocatorHost, LoginToken,
                                        TimeoutMillis,
...
void FWorkerConnection::ConnectToReceptionistAsync(
    const FString &Hostname, std::uint16_t Port, const FString &WorkerId,
  ...
  AsyncTask(ENamedThreads::AnyBackgroundThreadNormalTask,
            [TimeoutMillis, Hostname, Port, WorkerId, Params,
...
void FWorkerConnection::ConnectToLocatorAsync(
  ...
  if (!CanCreateNewConnection()) {
    UE_LOG(LogSpatialOS, Error,
           TEXT("Can not connect to SpatialOS; a connection already "
                "exists. Call Disconnect first."));
    return;
  }

  auto QueueStatusWrapper = [QueueStatusCallback](
  ...
  bIsConnecting = true;
  AsyncTask(ENamedThreads::AnyBackgroundThreadNormalTask,
            [ProjectName, LocatorHost, LoginToken, TimeoutMillis, DeploymentId,
             Params, QueueStatusWrapper, OnConnectedCallback, this]() {
              WaitForConnectionFuture(
                  TimeoutMillis,
                  CreateLocator(ProjectName, LocatorHost, LoginToken)
...
void FWorkerConnection::SendLogMessage(ELogVerbosity::Type Level,
  ...
  Connection->SendLogMessage(WorkerLogLevel, kLoggerName,
...
worker::Metrics &FWorkerConnection::GetMetrics() { return *Metrics; }
...
SpatialOSLocator FWorkerConnection::CreateLocator(const FString &ProjectName,
  ...
  LocatorParams.LoginToken =
      worker::LoginTokenCredentials{TCHAR_TO_UTF8(*LoginToken)};
...
void FWorkerConnection::WaitForConnectionFuture(
  ...
  if (ConnectionFuture.Wait(TimeoutMillis)) {
    auto WorkerConnection = TSharedPtr<SpatialOSConnection>(
        new SpatialOSConnection{ConnectionFuture.Get()});
    if (WorkerConnection->IsConnected()) {
      ...
      AsyncTask(ENamedThreads::GameThread, [OnConnectedCallback]() {
