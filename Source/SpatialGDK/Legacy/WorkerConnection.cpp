...
void FWorkerConnection::ConnectToReceptionistAsync(const FString& Hostname, std::uint16_t Port, const FString& WorkerId, const worker::ConnectionParameters& Params, FOnConnectedDelegate OnConnectedCallback, std::uint32_t TimeoutMillis)
{
	...
	bIsConnecting = true;
	AsyncTask(ENamedThreads::AnyBackgroundThreadNormalTask, [TimeoutMillis, Hostname, Port, WorkerId, Params, OnConnectedCallback, this]() {
		WaitForConnectionFuture(TimeoutMillis, SpatialOSConnection::ConnectAsync(improbable::unreal::Components{}, std::string{TCHAR_TO_UTF8(*Hostname)}, Port, std::string{TCHAR_TO_UTF8(*WorkerId)}, Params), OnConnectedCallback);
	});
}
