 ...
 * object. Callbacks can be deregistered manually using
 * DeregisterAllCallbacks().
 */
class FScopedViewCallbacks {
public:
  FScopedViewCallbacks() { View.Reset(); }

  FScopedViewCallbacks(const TWeakPtr<SpatialOSView> &InView) : View(InView) {}

  ~FScopedViewCallbacks() { Reset(); }

  /**
  * Initialise the FScopedViewCallbacks. Calling Init() more than once results
  * in an error.
  */
  ...
  /**
  * Reset the FScopedViewCallbacks to its initial state. Init() may be called
  * again after this
  * method is called.
  */
  void Reset() {
