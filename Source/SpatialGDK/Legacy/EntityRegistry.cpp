	...
	for (auto& Path : BlueprintPaths)
	{
		...
		if (EngineUtils::FindOrLoadAssetsByPath(Path, Assets, EngineUtils::ATL_Class))
		{
			for (auto Asset : Assets)
			{
				...
				{
					UE_LOG(LogEntityRegistry, Warning, TEXT("Found asset in the EntityBlueprints folder which is not a blueprint: %s"), *(Asset->GetFullName()));
				}
			}
		}
		else
...
void UEntityRegistry::RegisterEntityClass(const FString& ClassName, UClass* ClassToSpawn)
{
	if (ClassToSpawn == nullptr)
	{
		UE_LOG(LogEntityRegistry, Error, TEXT("UEntityRegistry::RegisterEntityClass: ClassToSpawn argument is null."));
		return;
	}
