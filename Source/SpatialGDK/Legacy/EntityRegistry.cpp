...
void UEntityRegistry::RegisterEntityBlueprints(
    const TArray<FString> &BlueprintPaths) {
  for (auto &Path : BlueprintPaths) {
    TArray<UObject *> Assets;
    if (EngineUtils::FindOrLoadAssetsByPath(Path, Assets,
      ...
      for (auto Asset : Assets) {
        ...
        if (BlueprintGeneratedClass != nullptr) {
          ...
          UE_LOG(LogEntityRegistry, Warning,
                 TEXT("Found asset in the EntityBlueprints "
      ...
      UE_LOG(LogEntityRegistry, Warning,
             TEXT("No assets found in EntityBlueprints folder."));
    }
  }
}
...
UClass **UEntityRegistry::GetRegisteredEntityClass(const FString &ClassName) {
  ...
  if (ClassToSpawn == nullptr) {
    UE_LOG(LogEntityRegistry, Log,
           TEXT("No UClass is associated with ClassName('%s')."), *ClassName);
    return nullptr;
  }
...
void UEntityRegistry::RegisterEntityClass(const FString &ClassName,
  ...
  if (ExistingClass != nullptr) {
    UE_LOG(LogEntityRegistry, Error,
           TEXT("ClassName '%s' has already been registered to '%s'."),
