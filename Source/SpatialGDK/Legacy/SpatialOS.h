...
/**
 * This class provides an interface for customising a worker, connecting and
 * disconnecting from
 * SpatialOS.
 */
UCLASS()
...
* If multiple worker instances are started, worker configurations are applied
* to the first
* worker
* instances based on the "PIEInstance" field of their respective
* "WorldContext".
* If there are more worker instances than worker configurations, no worker
* configuration is
* applied to the remaining worker instances.
*
* Remarks:
* Only call this method for worker instances started from the Unity Editor.
* Calling this method yields no effect if InWorldContext.WorldType !=
* EWorldType::PIE or
* InWorldContext.PIEInstance == -1.
* Do not call this method if any of the following editor settings assumes a
* certain value:
...
*
* Remarks:
* It is invalid to call this method before ApplyConfiguration has been
* called.
* It is invalid to call this multiple times between matching calls to
* Disconnect.
*/
  UFUNCTION(BlueprintCallable, Category = "SpatialOS")
  void Connect();
...
*
* Remarks:
* If a connection attempt has failed, call this method to receive log
* messages and
* disconnection
* events.
*/
...
* Remarks:
* Your worker must be connected before before access to this property is
* valid.
*/
...
*/
  ...
  /**
* Returns the raw entity, if it is available on this worker, or nullptr
* otherwise.
*/
  worker::Entity* GetLocalEntity(const worker::EntityId& InEntityId);

  /**
* This delegate is triggered when a connection to SpatialOS has been
* successfully made.
*/
  ...
  /**
* This delegate is triggered when the connection to SpatialOS is terminated
* for any reason.
*/
  UPROPERTY(BlueprintAssignable, Category = "SpatialOS")
  ...
  /**
* This delegate is triggered when a connection to SpatialOS has not been
* successfully made.
*/
  UPROPERTY(BlueprintAssignable, Category = "SpatialOS")
