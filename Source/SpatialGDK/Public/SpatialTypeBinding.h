	...
	: SubobjectProperty(false), Offset(0)
	{
		...
		for (FName PropertyName : PropertyNames)
		{
			...
			else
			{
				...
				{
					// We should only encounter a non-container style property if this is the final
					// property
					// in the chain.
	...
	FORCEINLINE uint8* GetPropertyData(uint8* Container) const
	{
		if (SubobjectProperty)
		{
			...
			for (int i = 0; i < PropertyChain.Num(); ++i)
			{
				...
				{
					// Migratable property chains can cross into subobjects, so we will need to deal
					// with
