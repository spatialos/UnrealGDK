...
enum EReplicatedPropertyGroup { GROUP_SingleClient, GROUP_MultiClient };

FORCEINLINE EReplicatedPropertyGroup
GetGroupFromCondition(ELifetimeCondition Condition) {
  switch (Condition) {
...
public:
  FRepHandleData(UClass *Class, TArray<FName> PropertyNames,
                 ELifetimeCondition Condition,
                 ELifetimeRepNotifyCondition RepNotifyCondition)
      : Condition(Condition), RepNotifyCondition(RepNotifyCondition),
        Offset(0) {
    ...
    for (FName PropertyName : PropertyNames) {
      ...
      UProperty *CurProperty =
          CurrentContainerType->FindPropertyByName(PropertyName);
    ...
    for (FName PropertyName : PropertyNames) {
      ...
      if (StructProperty) {
        ...
        if (ObjectProperty) {
          ...
          SubobjectProperty =
              true; // We are now recursing into a subobjects properties.
          Offset = 0;
        } else {
      ...
      for (int i = 0; i < PropertyChain.Num(); ++i) {
        ...
        if (i < (PropertyChain.Num() - 1)) {
          ...
          UObjectProperty *ObjectProperty =
              Cast<UObjectProperty>(PropertyChain[i]);
          if (ObjectProperty) {
            UObject *PropertyValue =
                ObjectProperty->GetObjectPropertyValue(Data);
...
public:
  virtual const FRepHandlePropertyMap &GetRepHandlePropertyMap() const
      PURE_VIRTUAL(USpatialTypeBinding::GetRepHandlePropertyMap,
                   ...
                   return Map;);
  virtual const FMigratableHandlePropertyMap &
  ...
  CreateActorEntity(const FString &ClientWorkerId, const FVector &Position,
