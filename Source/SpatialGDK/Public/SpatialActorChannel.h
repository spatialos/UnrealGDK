...
public:
  ...
  // SpatialOS Entity ID.
  FORCEINLINE FEntityId GetEntityId() const { return ActorEntityId; }
  ...
  FORCEINLINE bool
  IsClientAutonomousProxy(worker::ComponentId ServerRPCsComponentId) {
    ...
    if (View.Get()) {
      ...
      // out the
      // entity.
      auto &EntityAuthority =
          View->ComponentAuthority[ActorEntityId.ToSpatialEntityId()];
  ...
  // UChannel interface
  virtual void Init(UNetConnection *connection, int32 channelIndex,
                    bool bOpenedLocally) override;
