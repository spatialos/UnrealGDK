...
// Stores the result of an attempt to call an RPC sender function. Either we
// have an unresolved
// object which needs
// to be resolved before we can send this RPC, or we successfully sent a command
// request.
...
// Function storing a command request operation, capturing all arguments by
// value.
using FRPCCommandRequestFunc = TFunction<FRPCCommandRequestResult()>;

// Stores the result of attempting to receive an RPC command. We either return
// an unresolved object
// which needs
// to be resolved before the RPC implementation can be called successfully, or
// nothing, which
// indicates that
// the RPC implementation was called successfully.
using FRPCCommandResponseResult =
    TOptional<improbable::unreal::UnrealObjectRef>;

// Function storing a command response operation, capturing the command request
// op by value.
...
using FPendingOutgoingProperties = TPair<
    TArray<uint16>,
    TArray<uint16>>; // Pending incoming properties (replicated and migratable).
using FPendingIncomingProperties = TPair<TArray<const FRepHandleData *>,
                                         TArray<const FMigratableHandleData *>>;

// Map types for pending objects/RPCs. For pending updates, they store a map
// from an unresolved
// object to a map of channels to properties
// within those channels which depend on the unresolved object. For pending
// RPCs, they store a map
// from an unresolved object to a list of
// RPC functor objects which need to be re-executed when the object is resolved.
using FPendingOutgoingObjectUpdateMap =
    TMap<UObject *, TMap<USpatialActorChannel *, FPendingOutgoingProperties>>;
using FPendingOutgoingRPCMap =
    TMap<UObject *, TArray<TPair<FRPCCommandRequestFunc, bool>>>;
using FPendingIncomingObjectUpdateMap =
    TMap<FHashableUnrealObjectRef,
         TMap<USpatialActorChannel *, FPendingIncomingProperties>>;
using FPendingIncomingRPCMap =
    TMap<FHashableUnrealObjectRef, TArray<FRPCCommandResponseFunc>>;

// Helper function to write incoming replicated property data to an object.
FORCEINLINE void
ApplyIncomingReplicatedPropertyUpdate(const FRepHandleData &RepHandleData,
                                      UObject *Object, const void *Value,
                                      TArray<UProperty *> &RepNotifies) {
  uint8 *Dest =
      RepHandleData.GetPropertyData(reinterpret_cast<uint8 *>(Object));
  ...
  if (BoolProperty) {
    // We use UBoolProperty::SetPropertyValue here explicitly to ensure that
    // packed boolean
    // properties
    // are de-serialized correctly without clobbering neighboring boolean values
    // in memory.
...
FORCEINLINE void ApplyIncomingMigratablePropertyUpdate(
    const FMigratableHandleData &MigratableHandleData, UObject *Object,
    const void *Value) {
  uint8 *Dest =
      MigratableHandleData.GetPropertyData(reinterpret_cast<uint8 *>(Object));

  // Write value to destination.
  UBoolProperty *BoolProperty =
      Cast<UBoolProperty>(MigratableHandleData.Property);
  if (BoolProperty) {
    // We use UBoolProperty::SetPropertyValue here explicitly to ensure that
    // packed boolean
    // properties
...
// The system which is responsible for converting and sending Unreal updates to
// SpatialOS, and
// receiving updates from SpatialOS and
...
public:
  ...
  void Init(USpatialOS *Instance, USpatialNetDriver *Driver,
  ...
  worker::RequestId<worker::CreateEntityRequest> SendCreateEntityRequest(
      USpatialActorChannel *Channel, const FVector &Location,
      const FString &PlayerWorkerId, const TArray<uint16> &RepChanged,
      const TArray<uint16> &MigChanged);
  void SendSpatialPositionUpdate(const FEntityId &EntityId,
                                 const FVector &Location);
  void SendSpatialUpdate(USpatialActorChannel *Channel,
                         const TArray<uint16> &RepChanged,
                         const TArray<uint16> &MigChanged);
  void InvokeRPC(AActor *TargetActor, const UFunction *const Function,
                 FFrame *const Frame);
  ...
  // Called by USpatialPackageMapClient when a UObject is "resolved" i.e. has a
  // unreal object ref.
  // This will dequeue pending object ref updates and RPCs which depend on this
  // UObject existing
  // in
  // the package map.
  ...
  // Called by USpatialInteropPipelineBlock when an actor channel is opened on
  // the client.
  void AddActorChannel(const FEntityId &EntityId,
  ...
  GetActorChannelByEntityId(const FEntityId &EntityId) const;

  // RPC handlers. Used by generated type bindings.
  void SendCommandRequest_Internal(FRPCCommandRequestFunc Function,
                                   bool bReliable);
  void SendCommandResponse_Internal(FRPCCommandResponseFunc Function);
  void HandleCommandResponse_Internal(const FString &RPCName,
                                      FUntypedRequestId RequestId,
                                      const FEntityId &EntityId,
                                      const worker::StatusCode &StatusCode,
                                      const FString &Message);

  // Used to queue incoming/outgoing object updates/RPCs. Used by generated type
  // bindings.
  void
  QueueOutgoingObjectRepUpdate_Internal(UObject *UnresolvedObject,
                                        USpatialActorChannel *DependentChannel,
                                        uint16 Handle);
  void
  QueueOutgoingObjectMigUpdate_Internal(UObject *UnresolvedObject,
                                        USpatialActorChannel *DependentChannel,
                                        uint16 Handle);
  void QueueOutgoingRPC_Internal(UObject *UnresolvedObject,
  ...
  void QueueIncomingRPC_Internal(
      const improbable::unreal::UnrealObjectRef &UnresolvedObjectRef,
      FRPCCommandResponseFunc Responder);

  // Accessors.
  USpatialOS *GetSpatialOS() const { return SpatialOSInstance; }
...
private:
  ...
  TMap<FUntypedRequestId, TSharedPtr<FOutgoingReliableRPC>>
      OutgoingReliableRPCs;
...
private:
  ...
  void SendComponentInterests(USpatialActorChannel *ActorChannel,
                              const FEntityId &EntityId);

  void ResolvePendingOutgoingObjectUpdates(UObject *Object);
  void ResolvePendingOutgoingRPCs(UObject *Object);
  void ResolvePendingIncomingObjectUpdates(
      UObject *Object, const improbable::unreal::UnrealObjectRef &ObjectRef);
  void ResolvePendingIncomingRPCs(
