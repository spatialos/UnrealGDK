...
// Stores the result of an attempt to call an RPC sender function. Either we have an unresolved
// object which needs
...
// Stores the result of attempting to receive an RPC command. We either return an unresolved object
// which needs
// to be resolved before the RPC implementation can be called successfully, or nothing, which
// indicates that
...
// Map types for pending objects/RPCs. For pending updates, they store a map from an unresolved
// object to a map of channels to properties
// within those channels which depend on the unresolved object. For pending RPCs, they store a map
