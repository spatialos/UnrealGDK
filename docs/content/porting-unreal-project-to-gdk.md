...
**NOTE:** This workflow is very subject to change. We’re aware that it’s not optimized, and one of our priorities is to improve iteration times.
### Prerequisites
...
> Tip: To speed up build times, install IncrediBuild, FastBuild, or another build distributor.
...
1. Create symlinks between your Unreal project and the Unreal GDK:
    1. Open another instance of **File Explorer** and locate the Unreal GDK directory (but don’t go into it).
...
2. In your project’s uproject file, add the following to the `Modules` section:
    ``` csharp
    {
        "Name": "SpatialGDK",
        "Type": "Runtime",
...
1. In `<GameRoot>\Config`, open `DefaultEngine.ini` and add:
    ...
    Note: These lines ensure that SpatialOS Unreal GDK can override Unreal’s network replication.
...
### Generate code for SpatialOS C++ workers, and build your project
...
### Run the Interop Code Generator, and set up the UE4 Editor
...
1. Compile and run again with the same build configuration (new source files that require compilation will have been generated when you ran the Interop Code Generator).
    ...
    Note: You may now have compilation errors in your *TypeBinding generated files from missing `#include`s. Identify what header file each class is missing, and add it to the DefaultEditorSpatialGDK.ini config file.
