                                             ...
                                             const TArray<TArray<FName>>& MigratableProperties)
{
  ...
  for (TFieldIterator<UProperty> It(Type); It; ++It)
  {
    ...
    //		are in fact owned by the character, and can be stored in the same entity as the character
    ...
    //		the owner of the property value is equal to itself. As structs don't have CDOs, we assume
    ...
    //      might be a property which stores a MovementComponent pointer, so we'd need to somehow
    //      figure out the real type being stored there
    //		during runtime. This is determined by getting the CDO of this class to determine what is
    ...
    if (Value)
    {
      // If this is an editor-only property, skip it. As we've already added to the property list at
  ...
  // ownership. However, the rep layout will recurse into structs and allocate rep handles for their
  ...
  for (int CmdIndex = 0; CmdIndex < RepLayout.Cmds.Num(); ++CmdIndex)
  {
    ...
    // Parents array.
    // The Cmds array is an expanded version of the Parents array. This usually maps 1:1 with the
    ...
    // multiple Cmds map to the structs properties, but they all have the same ParentIndex (which
    // points to the root replicated property
    // which contains them.
    //
    // This might be problematic if we have a property which is inside a struct, nested in another
    ...
    //	}
    //
    // The parents array will contain "Bar", and the cmds array will contain "Nested", but we have
...
TArray<TSharedPtr<FUnrealProperty>> GetFlatRPCParameters(TSharedPtr<FUnrealRPC> RPCNode)
{
  ...
  VisitAllProperties(RPCNode,
                     [&ParamList](TSharedPtr<FUnrealProperty> Property) {
                       ...
                       // If the RepType is not a generic struct, such as Vector3f or Plane, add to
