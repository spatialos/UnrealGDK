											 ...
											 const TArray<TArray<FName>>& MigratableProperties)
{
	...
	for (TFieldIterator<UProperty> It(Type); It; ++It)
	{
		...
		//	 1) Determine whether this property is a strong or weak reference to the object. Some
		// subobjects (such as the CharacterMovementComponent)
		//		are in fact owned by the character, and can be stored in the same entity as the
		// character
		// itself. Some subobjects (such as the Controller
		//		field in AActor) is a weak reference, and should just store a reference to the real
		// object. We inspect the CDO to determine whether
		//		the owner of the property value is equal to itself. As structs don't have CDOs, we
		// assume
		// that all object properties in structs are
		//		weak references.
		//
		//   2) Obtain the concrete object type stored in this property. For example, the property
		//   containing the CharacterMovementComponent
		//      might be a property which stores a MovementComponent pointer, so we'd need to
		//      somehow
		//      figure out the real type being stored there
		//		during runtime. This is determined by getting the CDO of this class to determine what
		//is
		...
		if (Value)
		{
			// If this is an editor-only property, skip it. As we've already added to the property
			// list at
	...
	// Set up replicated properties by reading the rep layout and matching the properties with the
	// ones in the type node.
	// Based on inspection in InitFromObjectClass, the RepLayout will always replicate object
	// properties using NetGUIDs, regardless of
	// ownership. However, the rep layout will recurse into structs and allocate rep handles for
	// their
	// properties, unless the condition
	// "Struct->StructFlags & STRUCT_NetSerializeNative" is true. In this case, the entire struct is
	...
	for (int CmdIndex = 0; CmdIndex < RepLayout.Cmds.Num(); ++CmdIndex)
	{
		...
		// In a FRepLayout, all the root level replicated properties in a class are stored in the
		// Parents array.
		// The Cmds array is an expanded version of the Parents array. This usually maps 1:1 with
		// the
		// Parents array (as most properties
		// don't contain other properties). The main exception are structs which don't have a native
		// serialize function. In this case
		// multiple Cmds map to the structs properties, but they all have the same ParentIndex
		// (which
		// points to the root replicated property
		// which contains them.
		//
		// This might be problematic if we have a property which is inside a struct, nested in
		// another
		...
		//	}
		//
		// The parents array will contain "Bar", and the cmds array will contain "Nested", but we
		// have
		...
		{
			// Here, the Cmd is some property inside the Parent property. We need to find it in the
			// AST.
					...
					if (Property->Property == Cmd.Property)
					{
						checkf(!PropertyNode.IsValid(),
							   TEXT("We've already found a previous property node with the "
									"same property. This indicates that we have a 'diamond of "
...
FUnrealFlatRepData GetFlatRepData(TSharedPtr<FUnrealType> TypeInfo)
{
	...
	RepData[REP_SingleClient].KeySort([](uint16 A, uint16 B) { return A < B; });
	return RepData;
}
...
TArray<TSharedPtr<FUnrealProperty>> GetFlatRPCParameters(TSharedPtr<FUnrealRPC> RPCNode)
{
	...
	VisitAllProperties(RPCNode,
					   [&ParamList](TSharedPtr<FUnrealProperty> Property) {
						   ...
						   // If the RepType is not a generic struct, such as Vector3f or Plane, add
						   // to
...
TArray<TSharedPtr<FUnrealProperty>> GetPropertyChain(TSharedPtr<FUnrealProperty> LeafProperty)
{
	...
	// As we started at the leaf property and worked our way up, we need to reverse the list at the
	// end.
