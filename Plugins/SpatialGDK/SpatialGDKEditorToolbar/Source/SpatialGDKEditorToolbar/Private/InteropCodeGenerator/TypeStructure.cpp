...
FString GetLifetimeConditionAsString(ELifetimeCondition Condition) {
  const UEnum *EnumPtr =
      FindObject<UEnum>(ANY_PACKAGE, TEXT("ELifetimeCondition"), true);
  if (!EnumPtr) {
...
FString
GetRepNotifyLifetimeConditionAsString(ELifetimeRepNotifyCondition Condition) {
  switch (Condition) {
...
void VisitAllObjects(TSharedPtr<FUnrealType> TypeNode,
  ...
  for (auto &PropertyPair : TypeNode->Properties) {
    if (bShouldRecurseFurther && PropertyPair.Value->Type.IsValid()) {
      ...
      if (bRecurseIntoSubobjects ||
          PropertyPair.Value->Property->IsA<UStructProperty>()) {
        VisitAllObjects(PropertyPair.Value->Type, Visitor,
                        bRecurseIntoSubobjects);
      }
    }
  }
}
...
CreateUnrealTypeInfo(UStruct *Type,
  ...
  for (TFieldIterator<UProperty> It(Type); It; ++It) {
    ...
    if (Property->IsA<UMulticastDelegateProperty>()) {
      UE_LOG(LogSpatialGDKInteropCodeGenerator, Warning,
             TEXT("%s - multicast delegate property, skipping"),
             *Property->GetName());
      continue;
    }
    ...
    //		are in fact owned by the character, and can be stored in the same
    //entity as the
    // character
    ...
    //		the owner of the property value is equal to itself. As structs don't
    //have CDOs, we
    // assume
    ...
    if (Value) {
      ...
      if (Value->GetOuter() == ContainerCDO) {
        ...
        PropertyNode->Type =
            CreateUnrealTypeInfo(ObjectProperty->PropertyClass, {});
        PropertyNode->Type->ParentProperty = PropertyNode;
      } else {
        // The values outer is not us, store as weak reference.
        UE_LOG(LogSpatialGDKInteropCodeGenerator, Warning,
               TEXT("%s - %s weak reference (outer not this)"),
  ...
  for (TFieldIterator<UFunction> RemoteFunction(Class); RemoteFunction;
       ++RemoteFunction) {
    if (RemoteFunction->FunctionFlags & FUNC_NetClient ||
      ...
      RPCNode->bReliable =
          (RemoteFunction->FunctionFlags & FUNC_NetReliable) != 0;
      ...
      for (TFieldIterator<UProperty> It(*RemoteFunction); It; ++It) {
        ...
        TSharedPtr<FUnrealProperty> PropertyNode =
            MakeShared<FUnrealProperty>();
        ...
        if (StructParameter) {
          PropertyNode->Type =
              CreateUnrealTypeInfo(StructParameter->Struct, {});
          PropertyNode->Type->ParentProperty = PropertyNode;
        }
      }
    }
  ...
  for (int CmdIndex = 0; CmdIndex < RepLayout.Cmds.Num(); ++CmdIndex) {
    ...
    if (Parent.RoleSwapIndex != -1) {
      ...
      RepDataNode->RoleSwapHandle =
          (int32)RepLayout.Cmds[SwappedCmdIndex].RelativeHandle;
    } else {
  ...
  for (const TArray<FName> &PropertyNames : MigratableProperties) {
    ...
    for (FName PropertyName : PropertyNames) {
      ...
      MigratableProperty =
          CurrentTypeNode->Properties.FindChecked(NextProperty);
...
TMap<uint16, TSharedPtr<FUnrealProperty>>
GetFlatMigratableData(TSharedPtr<FUnrealType> TypeInfo) {
...
TArray<TSharedPtr<FUnrealProperty>>
GetFlatRPCParameters(TSharedPtr<FUnrealRPC> RPCNode) {
...
TArray<TSharedPtr<FUnrealProperty>>
GetPropertyChain(TSharedPtr<FUnrealProperty> LeafProperty) {
  ...
  while (CurrentProperty.IsValid()) {
    ...
    if (CurrentProperty->ContainerType.IsValid()) {
      TSharedPtr<FUnrealType> EnclosingType =
          CurrentProperty->ContainerType.Pin();
