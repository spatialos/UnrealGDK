...
{
    // Transform the property chain into a chain of C++ names, joined by either -> or . (UObject or
    ...
    for (int i = 0; i < Chain.Num(); ++i)
    {
        ...
        if (i < (Chain.Num() - 1))
        {
            // If this property is a UObject, use the arrow operator. Otherwise, use the dot
            // operator.
                                      ...
                                      TFunction<void(const FString&)> ObjectResolveFailureGenerator)
{
		...
		else
		{
			...
			Writer.Printf(R"""(
				...
				%s(std::string(reinterpret_cast<char*>(ValueData.GetData()), ValueData.Num()));)""",
                          *Property->GetCPPType(), *Property->GetCPPType(), *PropertyValue,
                          *Update);
        }
    }
    else if (Property->IsA(UBoolProperty::StaticClass()))
    ...
    else if (Property->IsA(UArrayProperty::StaticClass()))
    {
        ...
        GenerateUnrealToSchemaConversion(Writer, "List.emplace_back", ArrayProperty->Inner,
    ...
    const FString& PropertyValue, TFunction<void(const FString&)> ObjectResolveFailureGenerator)
{
		...
		else
		{
			Writer.Printf(R"""(
				...
				%s::StaticStruct()->SerializeBin(ValueDataReader, reinterpret_cast<void*>(&%s));)""",
                          *Update, *PropertyType, *PropertyValue);
        }
    }
    else if (Property->IsA(UBoolProperty::StaticClass()))
    {
    ...
    else if (Property->IsA(UByteProperty::StaticClass()))
    {
        ...
        // as TEnumAsByte<...> only has a uint8 constructor, we need to cast the SpatialOS value
        // into uint8 first which causes "uint8(uint8(...))" to be generated for non enum bytes.
                                ...
                                FString& StructName)
{
    ...
    StructName = FString::Printf(TEXT("%s_event%s_Parms"), *RPCOwnerClass->GetName(),
                                 *RPC->Function->GetName());
    if (RPCOwnerClass->ClassGeneratedBy)
    {
        // This RPC is generated from a blueprint class, so we need to generate the parameters
        // struct based on the RPC arguments
        ...
        for (UProperty* Prop : TFieldRange<UProperty>(RPC->Function))
        {
            ...
            bool bEmitConst =
                Prop->HasAnyPropertyFlags(CPF_ConstParm) && Prop->IsA<UObjectProperty>();

            //@TODO: UCREMOVAL: This is awful code duplication to avoid a double-const
            {
                // export 'const' for parameters
                const bool bIsConstParam =
                    (Prop->IsA(UInterfaceProperty::StaticClass()) &&
                     !Prop->HasAllPropertyFlags(
                         CPF_OutParm));  //@TODO: This should be const once that flag exists
                const bool bIsOnConstClass =
                    (Prop->IsA(UObjectProperty::StaticClass()) &&
                     static_cast<UObjectProperty*>(Prop)->PropertyClass != NULL &&
                     static_cast<UObjectProperty*>(Prop)->PropertyClass->HasAnyClassFlags(
                         CLASS_Const));
    ...
    else
    {
        ...
        Writer.Printf(
            "// This struct is declared in %s.generated.h (in a macro that is then put in %s.h "
            "UCLASS macro)",
            *RPCOwnerClass->GetName(), *RPCOwnerClass->GetName());
    }
}
                               ...
                               const TSharedPtr<FUnrealType> TypeInfo)
{
    ...
    for (EReplicatedPropertyGroup Group : GetAllReplicatedPropertyGroups())
    {
        BuildComponentUpdateSignature.NameAndParams += FString::Printf(
            TEXT("\n\timprobable::unreal::generated::%s::Update& %sUpdate,\n\tbool& "
                 "b%sUpdateChanged,"),
            *SchemaReplicatedDataName(Group, Class), *GetReplicatedPropertyGroupName(Group),
            *GetReplicatedPropertyGroupName(Group), *GetReplicatedPropertyGroupName(Group));
    }
    BuildComponentUpdateSignature.NameAndParams +=
        FString::Printf(TEXT("\n\timprobable::unreal::generated::%s::Update& "
    ...
    {
        HeaderWriter.Printf(
            "void ServerSendUpdate_%s(const uint8* RESTRICT Data, int32 Handle, UProperty* "
            "Property, USpatialActorChannel* Channel, improbable::unreal::generated::%s::Update& "
            "OutUpdate) const;",
            *GetReplicatedPropertyGroupName(Group), *SchemaReplicatedDataName(Group, Class));
    }
    HeaderWriter.Printf(
        "void ServerSendUpdate_Migratable(const uint8* RESTRICT Data, int32 Handle, UProperty* "
        "Property, USpatialActorChannel* Channel, improbable::unreal::generated::%s::Update& "
        "OutUpdate) const;",
        *SchemaMigratableDataName(Class));
    for (EReplicatedPropertyGroup Group : GetAllReplicatedPropertyGroups())
    {
        HeaderWriter.Printf(
            ...
            "improbable::unreal::generated::%s::Update& Update) const;",
            *GetReplicatedPropertyGroupName(Group), *SchemaReplicatedDataName(Group, Class));
    }
    HeaderWriter.Printf(
	...
	HeaderWriter.Printf("void ReceiveUpdate_NetMulticastRPCs(worker::EntityId EntityId, "
		"const improbable::unreal::generated::%s::Update& Update);",*SchemaRPCComponentName(RPC_NetMulticast, Class));
    ...
    for (auto Group : GetRPCTypes())
    {
        ...
        {
			HeaderWriter.Printf(
				"void %s_SendCommand(worker::Connection* const Connection, void* Parameters, "
    ...
    for (auto Group : GetRPCTypes())
    {
        for (auto& RPC : RPCsByType[Group])
        {
			...
			{
				HeaderWriter.Printf(
					"void %s_OnEvent(const worker::EntityId EntityId, const "
					"improbable::unreal::generated::%s& EventData);",
					*RPC->Function->GetName(), *SchemaRPCRequestType(RPC->Function));
			}
			else
			{
				HeaderWriter.Printf(
					"void %s_OnCommandRequest(const "
					"worker::CommandRequestOp<improbable::unreal::generated::%s::Commands::%s>& Op);",
					*RPC->Function->GetName(), *SchemaRPCComponentName(Group, Class),
    ...
    for (auto Group : GetRPCTypes())
    {
        ...
        {
            HeaderWriter.Printf(
                "void %s_OnCommandResponse(const "
                "worker::CommandResponseOp<improbable::unreal::generated::%s::Commands::%s>& Op);",
                *RPC->Function->GetName(), *SchemaRPCComponentName(Group, Class),
                *CPPCommandClassName(Class, RPC->Function));
        }
    }
...
void GenerateFunction_GetBoundClass(FCodeWriter& SourceWriter, UClass* Class)
{
    ...
    if (Class->ClassGeneratedBy)
    {
        ...
        SourceWriter.Printf("return FindObject<UClass>(ANY_PACKAGE, TEXT(\"%s\"));",
                            *Class->GetName());
    }
    else
    {
...
void GenerateFunction_Init(FCodeWriter& SourceWriter, UClass* Class,
                           const FUnrealRPCsByType& RPCsByType, const FUnrealFlatRepData& RepData,
                           const FCmdHandlePropertyMap& MigratableData)
{
    SourceWriter.BeginFunction(
    ...
    for (auto Group : GetRPCTypes())
    {
        for (auto& RPC : RPCsByType[Group])
        {
            SourceWriter.Printf("RPCToSenderMap.Emplace(\"%s\", &%s::%s_SendCommand);",
    ...
    if (RepData.Num() > 0 || MigratableData.Num() > 0)
    {
        // Get class.
        SourceWriter.PrintNewLine();
        SourceWriter.Printf("UClass* Class = FindObject<UClass>(ANY_PACKAGE, TEXT(\"%s\"));",
                            *Class->GetName());
    }
    ...
    if (RepData.Num() > 0)
    {
        ...
        for (auto& RepProp : ReplicatedProperties)
        {
            ...
            Algo::Transform(GetPropertyChain(RepProp.Value), PropertyChainNames,
                            [](const TSharedPtr<FUnrealProperty>& PropertyInfo) -> FString {
                                return TEXT("\"") + PropertyInfo->Property->GetFName().ToString() +
                                    TEXT("\"");
                            });
            ...
            {
                SourceWriter.Printf(
                    "RepHandleToPropertyMap.Add(%d, FRepHandleData(Class, {%s}, %s, %s, %d));",
                    RepProp.Value->ReplicationData->Handles[i], *PropertyChainInitList,
    ...
    if (MigratableData.Num() > 0)
    {
        ...
        for (auto& MigratableProp : MigratableData)
        {
            ...
            FString PropertyChainInitList;
            TArray<FString> PropertyChainNames;
            Algo::Transform(GetPropertyChain(MigratableProp.Value), PropertyChainNames,
            ...
            // Add the handle data to the map.
            SourceWriter.Printf(
                "MigratableHandleToPropertyMap.Add(%d, FMigratableHandleData(Class, {%s}));",
                                 ...
                                 const FUnrealRPCsByType& RPCsByType)
{
    ...
    {
        SourceWriter.Printf(
            "ViewCallbacks.Add(View->OnComponentUpdate<improbable::unreal::generated::%s>([this](",
            *SchemaReplicatedDataName(Group, Class));
        SourceWriter.Indent();
        SourceWriter.Printf(
            "const worker::ComponentUpdateOp<improbable::unreal::generated::%s>& Op)",
            *SchemaReplicatedDataName(Group, Class));
    ...
    for (auto Group : GetRPCTypes())
    {
        ...
        if (RPCsByType.Contains(Group) && RPCsByType[Group].Num() > 0)
        {
            SourceWriter.PrintNewLine();
            SourceWriter.Printf(
                "using %sRPCCommandTypes = improbable::unreal::generated::%s::Commands;",
                *GetRPCTypeName(Group), *SchemaRPCComponentName(Group, Class));
            for (auto& RPC : RPCsByType[Group])
            {
                SourceWriter.Printf(
                    "ViewCallbacks.Add(View->OnCommandRequest<%sRPCCommandTypes::%s>(std::bind(&%s:"
                    ":%s_OnCommandRequest, this, std::placeholders::_1)));",
                    *GetRPCTypeName(Group), *CPPCommandClassName(Class, RPC->Function),
                    *TypeBindingName(Class), *RPC->Function->GetName());
            }
            for (auto& RPC : RPCsByType[Group])
            {
                SourceWriter.Printf(
                    "ViewCallbacks.Add(View->OnCommandResponse<%sRPCCommandTypes::%s>(std::bind(&%"
                    "s::%s_OnCommandResponse, this, std::placeholders::_1)));",
                    *GetRPCTypeName(Group), *CPPCommandClassName(Class, RPC->Function),
                    *TypeBindingName(Class), *RPC->Function->GetName());
            }
        }
...
{
    SourceWriter.BeginFunction({"worker::Entity",
    ...
    for (EReplicatedPropertyGroup Group : GetAllReplicatedPropertyGroups())
    {
        SourceWriter.Printf("improbable::unreal::generated::%s::Data %sData;",
                            *SchemaReplicatedDataName(Group, Class),
                            *GetReplicatedPropertyGroupName(Group));
        SourceWriter.Printf("improbable::unreal::generated::%s::Update %sUpdate;",
                            *SchemaReplicatedDataName(Group, Class),
                            *GetReplicatedPropertyGroupName(Group));
        SourceWriter.Printf("bool b%sUpdateChanged = false;",
                            *GetReplicatedPropertyGroupName(Group));
    }
    SourceWriter.Printf("improbable::unreal::generated::%s::Data MigratableData;",
                        *SchemaMigratableDataName(Class));
    SourceWriter.Printf("improbable::unreal::generated::%s::Update MigratableDataUpdate;",
                        *SchemaMigratableDataName(Class));
    ...
    {
        BuildUpdateArgs.Add(
            FString::Printf(TEXT("%sUpdate"), *GetReplicatedPropertyGroupName(Group)));
        BuildUpdateArgs.Add(
            FString::Printf(TEXT("b%sUpdateChanged"), *GetReplicatedPropertyGroupName(Group)));
    }
    BuildUpdateArgs.Add("MigratableDataUpdate");
    BuildUpdateArgs.Add("bMigratableDataUpdateChanged");
    SourceWriter.Printf("BuildSpatialComponentUpdate(InitialChanges, Channel, %s);",
                        *FString::Join(BuildUpdateArgs, TEXT(", ")));
    for (EReplicatedPropertyGroup Group : GetAllReplicatedPropertyGroups())
    {
        SourceWriter.Printf("%sUpdate.ApplyTo(%sData);", *GetReplicatedPropertyGroupName(Group),
                            *GetReplicatedPropertyGroupName(Group));
    }
    ...
    SourceWriter.Printf(
        ".AddComponent<improbable::unreal::generated::%s>(MigratableData, WorkersOnly)",
        *SchemaMigratableDataName(Class));
    SourceWriter.Printf(
        ".AddComponent<improbable::unreal::generated::%s>(improbable::unreal::generated::%s::Data{}"
        ", OwningClientOnly)",
    ...
    SourceWriter.Printf(
        ".AddComponent<improbable::unreal::generated::%s>(improbable::unreal::generated::%s::Data{}"
        ", WorkersOnly)",
        *SchemaRPCComponentName(ERPCType::RPC_NetMulticast, Class),
        *SchemaRPCComponentName(ERPCType::RPC_NetMulticast, Class));
...
{
    SourceWriter.BeginFunction({"void",
    ...
    for (EReplicatedPropertyGroup Group : GetAllReplicatedPropertyGroups())
    {
        ...
        SourceWriter.BeginScope();
        SourceWriter.Printf(
            "Connection->SendComponentUpdate<improbable::unreal::generated::%s>(EntityId."
            "ToSpatialEntityId(), %sUpdate);",
            *SchemaReplicatedDataName(Group, Class), *GetReplicatedPropertyGroupName(Group));
        SourceWriter.End();
    }
    ...
    SourceWriter.Printf(
        "Connection->SendComponentUpdate<improbable::unreal::generated::%s>(EntityId."
        "ToSpatialEntityId(), MigratableDataUpdate);",
        *SchemaMigratableDataName(Class));
    SourceWriter.End();
...
{
    SourceWriter.BeginFunction({"void",
                                "SendRPCCommand(UObject* TargetObject, const UFunction* const "
...
void GenerateFunction_ReceiveAddComponent(FCodeWriter& SourceWriter, UClass* Class)
{
    ...
    for (EReplicatedPropertyGroup Group : GetAllReplicatedPropertyGroups())
    {
        SourceWriter.Printf(
			...
			if (%sAddOp)
			{
				auto Update = improbable::unreal::generated::%s::Update::FromInitialData(*%sAddOp->Data.data());
    ...
    SourceWriter.Printf(R"""(
		...
		{
			auto Update = improbable::unreal::generated::%s::Update::FromInitialData(*MigratableDataAddOp->Data.data());
...
void GenerateFunction_GetInterestOverrideMap(FCodeWriter& SourceWriter, UClass* Class)
{
    ...
    SourceWriter.Printf(R"""(
		...
		if (bIsClient)
		{
			if (!bAutonomousProxy)
			{
				Interest.emplace(improbable::unreal::generated::%s::ComponentId, worker::InterestOverride{false});
			}
			Interest.emplace(improbable::unreal::generated::%s::ComponentId, worker::InterestOverride{false});
		}
		return Interest;)""",
...
void GenerateFunction_BuildSpatialComponentUpdate(FCodeWriter& SourceWriter, UClass* Class)
{
    ...
    for (EReplicatedPropertyGroup Group : GetAllReplicatedPropertyGroups())
    {
        ...
        SourceWriter.Indent();
        SourceWriter.Printf(
            "ServerSendUpdate_%s(Data, HandleIterator.Handle, Cmd.Property, Channel, %sUpdate);",
            *GetReplicatedPropertyGroupName(Group), *GetReplicatedPropertyGroupName(Group));
        SourceWriter.Printf("b%sUpdateChanged = true;", *GetReplicatedPropertyGroupName(Group));
                                               ...
                                               EReplicatedPropertyGroup Group)
{
    FFunctionSignature ServerSendUpdateSignature{
        "void",
        FString::Printf(TEXT("ServerSendUpdate_%s(const uint8* RESTRICT Data, int32 Handle, "
                             "UProperty* Property, USpatialActorChannel* Channel, "
                             "improbable::unreal::generated::%s::Update& OutUpdate) const"),
                        *GetReplicatedPropertyGroupName(Group),
                        *SchemaReplicatedDataName(Group, Class))};
    ...
    if (RepData[Group].Num() > 0)
    {
        ...
        for (auto& RepProp : RepData[Group])
        {
            ...
            for (int i = 0; i < RepProp.Value->ReplicationData->Handles.Num(); ++i)
            {
                GenerateBody_SendUpdate_RepDataProperty(
                    SourceWriter, RepProp.Value->ReplicationData->Handles[i], RepProp.Value,
        ...
        SourceWriter.Indent();
        SourceWriter.Print(
            "checkf(false, TEXT(\"Unknown replication handle %d encountered when creating a "
            "SpatialOS update.\"));");
        SourceWriter.Print("break;");
...
void GenerateBody_SendUpdate_RepDataProperty(FCodeWriter& SourceWriter, uint16 Handle,
                                                      ...
                                                      const FCmdHandlePropertyMap& MigratableData)
{
    ...
    FFunctionSignature ServerSendUpdateSignature{
        "void",
        FString::Printf(TEXT("ServerSendUpdate_Migratable(const uint8* RESTRICT Data, int32 "
    ...
    if (MigratableData.Num() > 0)
    {
        ...
        for (auto& MigProp : MigratableData)
        {
            ...
            // Get unreal data by deserialising from the reader, convert and set the corresponding
            // field in the update object.
            FString PropertyValueName = TEXT("Value");
            FString PropertyCppType = Property->GetClass()->GetFName().ToString();
            FString PropertyValueCppType = Property->GetCPPType();
            FString PropertyName = TEXT("Property");
            // todo-giray: The reinterpret_cast below is ugly and we believe we can do this more
            ...
            GenerateUnrealToSchemaConversion(
                ...
                [&SourceWriter, Handle](const FString& PropertyValue) {
                    SourceWriter.Printf(
                        "Interop->QueueOutgoingObjectMigUpdate_Internal(%s, Channel, %d);",
        ...
        SourceWriter.Indent();
        SourceWriter.Print(
            "checkf(false, TEXT(\"Unknown migration property handle %d encountered when creating a "
            "SpatialOS update.\"));");
        SourceWriter.Print("break;");
                                            ...
                                            EReplicatedPropertyGroup Group)
{
    FFunctionSignature ReceiveUpdateSignature{
        "void",
        FString::Printf(TEXT("ReceiveUpdate_%s(USpatialActorChannel* ActorChannel, const "
                             "improbable::unreal::generated::%s::Update& Update) const"),
                        *GetReplicatedPropertyGroupName(Group),
                        *SchemaReplicatedDataName(Group, Class))};
    ...
    if (RepData[Group].Num() > 0)
    {
        SourceWriter.Printf(R"""(
				...
				const bool bAutonomousProxy = ActorChannel->IsClientAutonomousProxy(improbable::unreal::generated::%s::ComponentId);
        ...
        for (auto& RepProp : RepData[Group])
        {
            ...
            for (int i = 0; i < RepProp.Value->ReplicationData->Handles.Num(); ++i)
            {
                GenerateBody_ReceiveUpdate_RepDataProperty(
                    SourceWriter, RepProp.Value->ReplicationData->Handles[i], RepProp.Value,
                    RepProp.Value->ReplicationData->Handles.Num() == 1 ? -1 : i);
            }
        }
...
void GenerateBody_ReceiveUpdate_RepDataProperty(FCodeWriter& SourceWriter, uint16 Handle,
                                                TSharedPtr<FUnrealProperty> PropertyInfo,
                                                const int ArrayIdx)
{
    UProperty* Property = PropertyInfo->Property;
			...
			UE_LOG(LogSpatialOSInterop, Log, TEXT("%s: Received unresolved object property. Value: %s. actor %s (%lld), property %s (handle %d)"),
				...
				*RepData->Property->GetName(),
				Handle);)""");
                                           ...
                                           SourceWriter.Print(
                                               "Interop->QueueIncomingObjectRepUpdate_Internal("
    ...
    SourceWriter.Print(
        "ApplyIncomingReplicatedPropertyUpdate(*RepData, ActorChannel->Actor, static_cast<const "
                                                   ...
                                                   const FCmdHandlePropertyMap& MigratableData)
{
    ...
    FFunctionSignature ReceiveUpdateSignature{
        "void",
        FString::Printf(TEXT("ReceiveUpdate_Migratable(USpatialActorChannel* ActorChannel, const "
                             "improbable::unreal::generated::%s::Update& Update) const"),
                        *SchemaMigratableDataName(Class))};
    SourceWriter.BeginFunction(ReceiveUpdateSignature, TypeBindingName(Class));
    ...
    if (MigratableData.Num() > 0)
    {
        ...
        for (auto& MigProp : MigratableData)
        {
            ...
            SourceWriter.Printf(
                "uint8* PropertyData = "
                "MigratableData->GetPropertyData(reinterpret_cast<uint8*>(ActorChannel->Actor));");
            if (Property->IsA<UBoolProperty>())
            {
                SourceWriter.Printf(
            ...
            GeneratePropertyToUnrealConversion(
                ...
                [&SourceWriter](const FString& PropertyValue) {
                    ...
                    SourceWriter.Print("bWriteObjectProperty = false;");
                    SourceWriter.Print(
                        "Interop->QueueIncomingObjectMigUpdate_Internal(ObjectRef, ActorChannel, "
                        "MigratableData);");
                });
            ...
            SourceWriter.Print(
                "ApplyIncomingMigratablePropertyUpdate(*MigratableData, ActorChannel->Actor, "
...
void GenerateFunction_ReceiveUpdate_MulticastRPCs(FCodeWriter& SourceWriter, UClass* Class,
                                                  const TArray <TSharedPtr<FUnrealRPC>> RPCs)
{
    FFunctionSignature ReceiveUpdateSignature{
        "void",
        FString::Printf(TEXT("ReceiveUpdate_NetMulticastRPCs(worker::EntityId EntityId, const "
    ...
    for (auto RPC : RPCs)
    {
        SourceWriter.Printf(R"""(
			...
			{
				%s_OnEvent(EntityId, event);
			}
			)""",
                            *SchemaCommandName(Class, RPC->Function), *RPC->Function->GetName());
        SourceWriter.PrintNewLine();
    }
                                     ...
                                     const TSharedPtr<FUnrealRPC> RPC)
{
    FFunctionSignature SendCommandSignature{
        "void",
        FString::Printf(TEXT("%s_SendCommand(worker::Connection* const Connection, void* "
                             "Parameters, UObject* TargetObject)"),
                        *RPC->Function->GetName())};
    ...
    if (RPC->Function->NumParms > 0)
    {
        ...
        SourceWriter.Printf("%s StructuredParams = *static_cast<%s*>(Parameters);",
                            *ParametersStructName, *ParametersStructName);
        SourceWriter.PrintNewLine();
    }
    ...
    SourceWriter.Printf(
        "auto Sender = [this, Connection, %s]() mutable -> FRPCCommandRequestResult",
    ...
    SourceWriter.Printf("Interop->SendCommandRequest_Internal(Sender, %s);",
                        RPC->bReliable ? TEXT("/*bReliable*/ true") : TEXT("/*bReliable*/ false"));

    SourceWriter.End();
}

void GenerateFunction_RPCSendEvent(FCodeWriter& SourceWriter, UClass* Class,
                                   const TSharedPtr<FUnrealRPC> RPC)
{
    FFunctionSignature SendCommandSignature{
        "void",
    ...
    SourceWriter.PrintNewLine();
    SourceWriter.Print("// Build request.");
    SourceWriter.Printf("improbable::unreal::generated::%s EventData;",
                        *SchemaRPCRequestType(RPC->Function));

    TArray<TSharedPtr<FUnrealProperty>> RPCParameters = GetFlatRPCParameters(RPC);
    for (auto Param : RPCParameters)
    {
        FString SpatialValueSetter = TEXT("EventData.set_") + SchemaFieldName(Param);
                                          ...
                                          const TSharedPtr<FUnrealRPC> RPC)
{
    ...
    auto ObjectResolveFailureGenerator = [&SourceWriter, &RPC, Class](const FString& PropertyName,
        ...
        SourceWriter.Printf(R"""(
			UE_LOG(LogSpatialOSInterop, Log, TEXT("%%s: %s_OnCommandRequest: %s %%s is not resolved on this worker."),
				...
				*ObjectRefToString(%s));
			return {%s};)""",
                            *RPC->Function->GetName(), *PropertyName, *ObjectRef, *ObjectRef,
                            *ObjectRef);
    };
    ...
    if (RPC->Parameters.Num() > 0)
    {
        ...
        for (auto Param : GetFlatRPCParameters(RPC))
        {
            FString SpatialValue =
                FString::Printf(TEXT("%s.%s()"), TEXT("Op.Request"), *SchemaFieldName(Param));

            GeneratePropertyToUnrealConversion(
                SourceWriter, SpatialValue, Param->Property,
                FString::Printf(TEXT("Parameters.%s"), *CPPFieldName(Param)),
		...
		else
		{
			UE_LOG(LogSpatialOSInterop, Error, TEXT("%%s: %s_OnCommandRequest: Function not found. Object: %%s, Function: %s."),
				...
				*TargetObject->GetFullName());
                        ...
                        *RPC->Function->GetName(), *RPCParametersStruct, *RPC->Function->GetName(),
                        *RPC->Function->GetName());
    SourceWriter.PrintNewLine();
    ...
    SourceWriter.Print(
        ...
        "Interop->GetSpatialOS()->GetConnection().Pin();");
    SourceWriter.Printf(
        "Connection->SendCommandResponse<improbable::unreal::generated::%s::Commands::%s>(Op."
        "RequestId, {});",
        *SchemaRPCComponentName(RPC->Type, Class), *CPPCommandClassName(Class, RPC->Function));
...
void GenerateFunction_RPCOnEvent(FCodeWriter& SourceWriter, UClass* Class,
                                 const TSharedPtr<FUnrealRPC> RPC)
{
    FString RequestFuncName =
        FString::Printf(TEXT("%s_OnEvent(const worker::EntityId EntityId, "
                             "const improbable::unreal::generated::%s& EventData)"),
                        *RPC->Function->GetName(), *SchemaRPCRequestType(RPC->Function));
    ...
    if (RPC->Parameters.Num() > 0)
    {
        ...
        {
            FString SpatialValue =
                FString::Printf(TEXT("%s.%s()"), TEXT("EventData"), *SchemaFieldName(Param));
                                           ...
                                           const TSharedPtr<FUnrealRPC> RPC)
{
    ...
    SourceWriter.Printf(
        "Interop->HandleCommandResponse_Internal(TEXT(\"%s\"), Op.RequestId.Id, Op.EntityId, "
