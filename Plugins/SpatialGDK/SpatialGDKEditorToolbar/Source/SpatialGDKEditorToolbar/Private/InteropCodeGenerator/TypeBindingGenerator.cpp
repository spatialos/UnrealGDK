    ...
    TFunction<void(const FString&)> ObjectResolveFailureGenerator)
{
  ...
  else if (Property->IsA(UByteProperty::StaticClass()))
  {
    ...
    // TEnumAsByte<...> or uint8. However, as TEnumAsByte<...> only has a uint8 constructor, we need
                               ...
                               const TSharedPtr<FUnrealType> TypeInfo)
{
  ...
  {
    HeaderWriter.Printf(
        "void ServerSendUpdate_%s(const uint8* RESTRICT Data, int32 Handle, UProperty* Property, "
  ...
  HeaderWriter.Printf(
      ...
      "Property, USpatialActorChannel* Channel, improbable::unreal::%s::Update& OutUpdate) const;",
      *SchemaMigratableDataName(Class));
  for (EReplicatedPropertyGroup Group : GetAllReplicatedPropertyGroups())
  {
    HeaderWriter.Printf(
                                 ...
                                 const FUnrealRPCsByType& RPCsByType)
{
  ...
  for (auto Group : GetRPCTypes())
  {
    // Ensure that this class contains RPCs of the type specified by group (eg, Server or Client) so
    ...
    if (RPCsByType.Contains(Group) && RPCsByType[Group].Num() > 0)
    {
      ...
      {
        SourceWriter.Printf(
            "ViewCallbacks.Add(View->OnCommandRequest<%sRPCCommandTypes::%s>(std::bind(&%s::%s_"
      ...
      {
        SourceWriter.Printf(
            "ViewCallbacks.Add(View->OnCommandResponse<%sRPCCommandTypes::%s>(std::bind(&%s::%s_"
                                               ...
                                               EReplicatedPropertyGroup Group)
{
  FFunctionSignature ServerSendUpdateSignature{
      "void",
      FString::Printf(
          TEXT("ServerSendUpdate_%s(const uint8* RESTRICT Data, int32 Handle, UProperty* Property, "
               "USpatialActorChannel* Channel, improbable::unreal::%s::Update& OutUpdate) const"),
          *GetReplicatedPropertyGroupName(Group), *SchemaReplicatedDataName(Group, Class))};
  ...
  if (RepData[Group].Num() > 0)
  {
    ...
    for (auto& RepProp : RepData[Group])
    {
      ...
      // Get unreal data by deserialising from the reader, convert and set the corresponding field
    ...
    SourceWriter.Indent();
    SourceWriter.Print(
        "checkf(false, TEXT(\"Unknown replication handle %d encountered when creating a SpatialOS "
        "update.\"));");
    SourceWriter.Print("break;");
  ...
  if (MigratableData.Num() > 0)
  {
    ...
    for (auto& MigProp : MigratableData)
    {
      ...
      GenerateUnrealToSchemaConversion(
          ...
          [&SourceWriter, Handle](const FString& PropertyValue) {
            SourceWriter.Printf("Interop->QueueOutgoingObjectMigUpdate_Internal(%s, Channel, %d);",
                                *PropertyValue, Handle);
          });
      SourceWriter.Print("break;");
      SourceWriter.End();
                                     ...
                                     const TSharedPtr<FUnrealRPC> RPC)
{
  ...
  {
    // Note that macros returned by GeneratePropertyReader require this FFrame variable to be named
    // "Stack"
  ...
  SourceWriter.Printf("auto Sender = [this, Connection, %s]() mutable -> FRPCCommandRequestResult",
  ...
  for (auto Param : RPCParameters)
  {
    ...
    GenerateUnrealToSchemaConversion(
        ...
        [&SourceWriter, &RPC](const FString& PropertyValue) {
          SourceWriter.Printf(
              "UE_LOG(LogSpatialOSInterop, Log, TEXT(\"%%s: RPC %s queued. %s is unresolved.\"), "
              "*Interop->GetSpatialOS()->GetWorkerId());",
