...
void GenerateUnrealToSchemaConversion(
	FCodeWriter& Writer, const FString& Update, UProperty* Property, const FString& PropertyValue, const bool bIsUpdate, TFunction<void(const FString&)> ObjectResolveFailureGenerator)
{
	// Get result type.
	...
	FCodeWriter& Writer, const FString& Update, const UProperty* Property, const FString& PropertyValue, const bool bIsUpdate, TFunction<void(const FString&)> ObjectResolveFailureGenerator)
{
	...
	else if (Property->IsA(UArrayProperty::StaticClass()))
	{
		...
		GeneratePropertyToUnrealConversion(
			Writer, "List[i]", ArrayProperty->Inner, FString::Printf(TEXT("%s[i]"), *PropertyValue), true, ObjectResolveFailureGenerator);
		Writer.End();
		Writer.End();
	}
	else
	...
	FCodeWriter& SourceWriter, UClass* Class, const TMap<uint16, TSharedPtr<FUnrealProperty>>& MigratableData)
{
	...
	if (MigratableData.Num() > 0)
	{
		...
		for (auto& MigratableProp : MigratableData)
		{
			...
			TArray<FString> PropertyChainNames;
			Algo::Transform(
				GetPropertyChain(MigratableProp.Value), PropertyChainNames, [](const TSharedPtr<FUnrealProperty>& Property) -> FString {
...
void GenerateFunction_ServerSendUpdate_RepData(
	FCodeWriter& SourceWriter, UClass* Class, const FUnrealFlatRepData& RepData, EReplicatedPropertyGroup Group)
{
	...
	if (RepData[Group].Num() > 0)
	{
		...
		for (auto& RepProp : RepData[Group])
		{
			...
			GenerateUnrealToSchemaConversion(
				SourceWriter, SpatialValueSetter, RepProp.Value->Property, PropertyValueName, true, [&SourceWriter, Handle](const FString& PropertyValue) {
					SourceWriter.Printf(
	...
	FCodeWriter& SourceWriter, UClass* Class, const TMap<uint16, TSharedPtr<FUnrealProperty>>& MigratableData)
{
	...
	if (MigratableData.Num() > 0)
	{
		...
		for (auto& MigProp : MigratableData)
		{
			...
			GenerateUnrealToSchemaConversion(
				SourceWriter, SpatialValueSetter, MigProp.Value->Property, PropertyValueName, true, [&SourceWriter, Handle](const FString& PropertyValue) {
					SourceWriter.Printf(
	...
	if (RepData[Group].Num() > 0)
	{
		...
		for (auto& RepProp : RepData[Group])
		{
			...
			FString SpatialValue =
				FString::Printf(TEXT("(*%s.%s().data())"), TEXT("Update"), *SchemaFieldName(RepProp.Value));

			GeneratePropertyToUnrealConversion(
				SourceWriter, SpatialValue, RepProp.Value->Property, PropertyValueName, true, [&SourceWriter](const FString& PropertyValue) {
					SourceWriter.Print(R"""(
	...
	FCodeWriter& SourceWriter, UClass* Class, const TMap<uint16, TSharedPtr<FUnrealProperty>>& MigratableData)
{
	...
	if (MigratableData.Num() > 0)
	{
		...
		for (auto& MigProp : MigratableData)
		{
			...
			FString SpatialValue =
				FString::Printf(TEXT("(*%s.%s().data())"), TEXT("Update"), *SchemaFieldName(MigProp.Value));

			GeneratePropertyToUnrealConversion(
				SourceWriter, SpatialValue, MigProp.Value->Property, PropertyValueName, true, [&SourceWriter](const FString& PropertyValue) {
					SourceWriter.Print(R"""(
					UE_LOG(LogSpatialOSInterop, Log, TEXT("%s: Received unresolved object property. Value: %s. actor %s (%lld), property %s (handle %d)"),
						*Interop->GetSpatialOS()->GetWorkerId(),
						*ObjectRefToString(ObjectRef),
...
void GenerateFunction_RPCSendCommand(FCodeWriter& SourceWriter, UClass* Class, const TSharedPtr<FUnrealRPC> RPC)
{
	...
	for (auto Param : RPCParameters)
	{
		...
		GenerateUnrealToSchemaConversion(
			SourceWriter, SpatialValueSetter, Param->Property, CPPFieldName(Param), false, [&SourceWriter, &RPC](const FString& PropertyValue) {
				SourceWriter.Printf(
