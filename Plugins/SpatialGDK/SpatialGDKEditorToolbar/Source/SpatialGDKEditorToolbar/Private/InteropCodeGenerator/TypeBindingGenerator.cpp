...
void GenerateUnrealToSchemaConversion(FCodeWriter& Writer, const FString& Update, UProperty* Property, const FString& PropertyValue, const bool bIsUpdate, TFunction<void(const FString&)> ObjectResolveFailureGenerator)
{
	...
	if (UEnumProperty* EnumProperty = Cast<UEnumProperty>(Property))
	{
		...
		// return GenerateUnrealToSchemaConversion(Writer, EnumProperty->GetUnderlyingProperty(),
	...
	if (Property->IsA(UStructProperty::StaticClass()))
	{
		...
		{
			// A generic struct, RepMovement or UniqueNetIdRepl needs to be replicated using
			// NetSerialize.
			Writer.BeginScope();
			Writer.Printf(R"""(
	...
	else if (Property->IsA(UArrayProperty::StaticClass()))
	{
		...
		GenerateUnrealToSchemaConversion(Writer, "const auto& ElementData = ", ArrayProperty->Inner,
	...
	FCodeWriter& Writer, const FString& Update, const UProperty* Property, const FString& PropertyValue, const bool bIsUpdate, TFunction<void(const FString&)> ObjectResolveFailureGenerator)
{
	...
	else if (Property->IsA(UByteProperty::StaticClass()))
	{
		...
		// TEnumAsByte<...> or uint8. However, as TEnumAsByte<...> only has a uint8 constructor, we
		// need
...
void GenerateTypeBindingHeader(FCodeWriter& HeaderWriter, FString SchemaFilename, FString InteropFilename, UClass* Class, const TSharedPtr<FUnrealType> TypeInfo)
{
	...
	for (EReplicatedPropertyGroup Group : GetAllReplicatedPropertyGroups())
	{
		HeaderWriter.Printf(
			...
			"Property, "
			"USpatialActorChannel* Channel, improbable::unreal::%s::Update& OutUpdate) const;",
	...
	HeaderWriter.Printf(
		...
		"Property, USpatialActorChannel* Channel, improbable::unreal::%s::Update& OutUpdate) "
		"const;",
		*SchemaMigratableDataName(Class));
	for (EReplicatedPropertyGroup Group : GetAllReplicatedPropertyGroups())
	{
	...
	for (auto Group : GetRPCTypes())
	{
		...
		{
			HeaderWriter.Printf(
				"void %s_SendCommand(worker::Connection* const Connection, struct FFrame* const "
	...
	for (auto Group : GetRPCTypes())
	{
		for (auto& RPC : RPCsByType[Group])
		{
			HeaderWriter.Printf(
				...
				"worker::CommandRequestOp<improbable::unreal::%s::Commands::%s>& Op);",
	...
	for (auto Group : GetRPCTypes())
	{
		...
		for (auto& RPC : RPCsByType[Group])
		{
			HeaderWriter.Printf(
				...
				"worker::CommandResponseOp<improbable::unreal::%s::Commands::%s>& Op);",
				*RPC->Function->GetName(), *SchemaRPCComponentName(Group, Class),
				*CPPCommandClassName(RPC->Function));
		}
	}
...
void GenerateFunction_BindToView(FCodeWriter& SourceWriter, UClass* Class, const FUnrealRPCsByType& RPCsByType)
{
	...
	{
		SourceWriter.Printf(
			"ViewCallbacks.Add(View->OnComponentUpdate<improbable::unreal::%s>([this](",
			*SchemaReplicatedDataName(Group, Class));
		SourceWriter.Indent();
...
void GenerateFunction_CreateActorEntity(FCodeWriter& SourceWriter, UClass* Class)
{
	...
	// Set up initial data.
	SourceWriter.Print(
		TEXT("checkf(GetRepHandlePropertyMap().Num() >= InitialChanges.RepChanged.Num() - 1, "
			 "TEXT(\"Attempting to replicate more properties than typebinding is aware of. Have "
	...
	SourceWriter.Printf(".AddComponent<improbable::unreal::%s>(MigratableData, WorkersOnly)",
						*SchemaMigratableDataName(Class));
	SourceWriter.Printf(
		".AddComponent<improbable::unreal::%s>(improbable::unreal::%s::Data{}, OwningClientOnly)",
		*SchemaRPCComponentName(ERPCType::RPC_Client, Class),
		*SchemaRPCComponentName(ERPCType::RPC_Client, Class));
	SourceWriter.Printf(
		".AddComponent<improbable::unreal::%s>(improbable::unreal::%s::Data{}, WorkersOnly)",
...
void GenerateFunction_SendComponentUpdates(FCodeWriter& SourceWriter, UClass* Class)
{
	...
	for (EReplicatedPropertyGroup Group : GetAllReplicatedPropertyGroups())
	{
		...
		SourceWriter.BeginScope();
		SourceWriter.Printf(
			"Connection->SendComponentUpdate<improbable::unreal::%s>(EntityId.ToSpatialEntityId(), "
			"%sUpdate);",
			*SchemaReplicatedDataName(Group, Class), *GetReplicatedPropertyGroupName(Group));
		SourceWriter.End();
	}
...
void GenerateFunction_ServerSendUpdate_RepData(FCodeWriter& SourceWriter, UClass* Class, const FUnrealFlatRepData& RepData, EReplicatedPropertyGroup Group)
{
	FFunctionSignature ServerSendUpdateSignature{
		"void",
		FString::Printf(
			TEXT("ServerSendUpdate_%s(const uint8* RESTRICT Data, int32 Handle, UProperty* "
				 "Property, "
				 "USpatialActorChannel* Channel, improbable::unreal::%s::Update& OutUpdate) const"),
			*GetReplicatedPropertyGroupName(Group), *SchemaReplicatedDataName(Group, Class))};
	...
	if (RepData[Group].Num() > 0)
	{
		...
		for (auto& RepProp : RepData[Group])
		{
			...
			else if (Property->IsA<UArrayProperty>())
			{
				UArrayProperty* ArrayProperty = Cast<UArrayProperty>(Property);
				SourceWriter.Printf(
					"const TArray<%s>& %s = *(reinterpret_cast<TArray<%s> const*>(Data));",
			...
			GenerateUnrealToSchemaConversion(
				...
				[&SourceWriter, Handle](const FString& PropertyValue) {
					SourceWriter.Printf(
						"Interop->QueueOutgoingObjectRepUpdate_Internal(%s, Channel, %d);",
...
void GenerateFunction_ReceiveUpdate_RepData(FCodeWriter& SourceWriter, UClass* Class, const FUnrealFlatRepData& RepData, EReplicatedPropertyGroup Group)
{
	...
	if (RepData[Group].Num() > 0)
	{
		...
		for (auto& RepProp : RepData[Group])
		{
			...
			SourceWriter.Print(
				"ApplyIncomingReplicatedPropertyUpdate(*RepData, ActorChannel->Actor, "
				"static_cast<const "
...
void GenerateFunction_RPCSendCommand(FCodeWriter& SourceWriter, UClass* Class, const TSharedPtr<FUnrealRPC> RPC)
{
	...
	{
		// Note that macros returned by GeneratePropertyReader require this FFrame variable to be
		// named
		// "Stack"
...
void GenerateFunction_RPCOnCommandRequest(FCodeWriter& SourceWriter, UClass* Class, const TSharedPtr<FUnrealRPC> RPC)
{
	...
	SourceWriter.Print(
		...
		"Interop->GetSpatialOS()->GetConnection().Pin();");
	SourceWriter.Printf(
		"Connection->SendCommandResponse<improbable::unreal::%s::Commands::%s>(Op.RequestId, {});",
