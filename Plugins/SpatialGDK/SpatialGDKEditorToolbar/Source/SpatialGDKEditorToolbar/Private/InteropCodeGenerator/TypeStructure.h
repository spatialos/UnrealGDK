...
This file contains functions to generate an abstract syntax tree which is used by the code
generating in
SchemaGenerator.cpp and TypeBindingGenerator.cpp. The AST follows a structure which is similar to a
UClass/UProperty
structure, but also contains additional metadata such as replication data. One main difference is
that the AST
structure will recurse into object properties if it's determined that the container type holds a
strong reference
to that subobject (such as a character owning its movement component).

The AST is built by recursing through the UClass/UProperty structure generated by UHT. Afterwards,
the rep layout
is generated with FRepLayout::InitFromObjectClass, and the replication handle / etc is stored in
FUnrealRepData
...
// instead try
// to emulate it by separating all replicated properties into two groups: properties which are meant
// for just one
...
// RPC Type. This matches the tag specified in UPROPERTY. For example, RPC_Client means RPCs which
// are destined for
...
// Given an AST, this applies the function 'Visitor' to all FUnrealType's contained transitively
// within the properties. bRecurseIntoObjects will control
// whether this function will recurse into a UObject's properties, which may not always be
...
// The list of replicated properties will all have the ReplicatedData field set to a valid
// FUnrealRepData node which contains
// data such as the handle or replication condition.
//
// This function will _not_ traverse into subobject properties (as the replication system deals with
// each object separately).
FUnrealFlatRepData GetFlatRepData(TSharedPtr<FUnrealType> TypeInfo);

// Traverses an AST, and generates a flattened list of migratable properties. The list of migratable
// properties will all have
// the MigratableData field set to a value FUnrealMigratableData node which contains data such as
...
// Traverses an AST fully (including subobjects) and generates a list of all RPCs which would be
...
// Given an AST, traverses all its parameters (and properties within structs) and generates a
...
// Given a property, traverse up to the root property and create a list of properties needed to
