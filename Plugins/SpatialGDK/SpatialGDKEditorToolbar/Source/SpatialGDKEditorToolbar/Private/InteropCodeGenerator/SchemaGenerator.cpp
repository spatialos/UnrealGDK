...
{
    return FString::Printf(TEXT("Unreal%s%sRepData"), *UnrealNameToSchemaTypeName(Type->GetName()),
...
{
    return FString::Printf(TEXT("Unreal%s%sRPCs"), *UnrealNameToSchemaTypeName(Type->GetName()),
...
FString SchemaFieldName(const TSharedPtr<FUnrealProperty> Property,
                        const int FixedArrayIndex /*=-1*/)
{
    // Transform the property chain into a chain of names.
    TArray<FString> ChainNames;
    Algo::Transform(GetPropertyChain(Property), ChainNames,
                    [](const TSharedPtr<FUnrealProperty>& Property) -> FString {
                        // Note: Removing underscores to avoid naming mismatch between how schema
                        // compiler and interop generator process schema identifiers.
                        return Property->Property->GetName().ToLower().Replace(TEXT("_"), TEXT(""));
                    });

    // Prefix is required to disambiguate between properties in the generated code and
    // UActorComponent/UObject properties which the generated code extends :troll:.
...
FString SchemaCommandName(UClass* Class, UFunction* Function)
{
    ...
    // Note: Removing underscores to avoid naming mismatch between how schema compiler and interop
...
void WriteSchemaRepField(FCodeWriter& Writer, const TSharedPtr<FUnrealProperty> RepProp,
                         const FString& PropertyPath, const int FieldCounter, const int ArrayIdx)
{
    Writer.Printf("%s %s = %d; // %s // %s", *PropertyToSchemaType(RepProp->Property),
                  *SchemaFieldName(RepProp, ArrayIdx), FieldCounter,
                  *GetLifetimeConditionAsString(RepProp->ReplicationData->Condition),
                  *PropertyPath);
}

void WriteSchemaMigratableField(FCodeWriter& Writer,
                                const TSharedPtr<FUnrealProperty> MigratableProp,
                                const int FieldCounter, const int ArrayIdx)
{
    Writer.Printf("%s %s = %d;", *PropertyToSchemaType(MigratableProp->Property),
                  *SchemaFieldName(MigratableProp, ArrayIdx), FieldCounter);
}

void WriteSchemaRPCField(TSharedPtr<FCodeWriter> Writer, const TSharedPtr<FUnrealProperty> RPCProp,
                         const int FieldCounter, const int ArrayIdx)
{
    Writer->Printf("%s %s = %d;", *PropertyToSchemaType(RPCProp->Property),
                   *SchemaFieldName(RPCProp, ArrayIdx), FieldCounter);
}
                              ...
                              TSharedPtr<FUnrealType> TypeInfo, FString SchemaPath)
{
    ...
    for (EReplicatedPropertyGroup Group : GetAllReplicatedPropertyGroups())
    {
        ...
        for (auto& RepProp : RepData[Group])
        {
            ...
            // This loop will add the owner class of each field in the component. Meant for
            ...
            while (UnrealProperty->ContainerType != nullptr)
            {
                ...
                {
                    TSharedPtr<FUnrealProperty> ParentProperty =
                        ContainerType->ParentProperty.Pin();
                    if (ParentProperty.IsValid())
                    {
                        PropertyPath +=
                            FString::Printf(TEXT("%s::%s"), *ContainerType->Type->GetName(),
            ...
            {
                FieldCounter++;
                WriteSchemaRepField(Writer, RepProp.Value, PropertyPath, FieldCounter,
    ...
    for (auto& Prop : GetFlatMigratableData(TypeInfo))
    {
        ...
        {
            FieldCounter++;
            WriteSchemaMigratableField(Writer, Prop.Value, FieldCounter,
    ...
    for (auto& RPCTypeOwner : RPCTypeOwners)
    {
        Writer.Printf("import \"improbable/unreal/generated/Unreal%sTypes.schema\";",
                      *RPCTypeOwner);
    ...
    for (auto Group : GetRPCTypes())
    {
        ...
        for (auto& RPC : RPCsByType[Group])
        {
            ...
            FString RPCOwnerName =
                UnrealNameToSchemaTypeName(*RPC->Function->GetOuter()->GetName());
            ...
            FieldCounter = 1;
            // TODO: Support fixed size arrays in RPCs. The implementation below is incomplete.
            // UNR-283.
            for (auto& Param : ParamList)
            {
                ...
                {
                    FieldCounter++;
                    WriteSchemaRPCField(RPCTypeOwnerSchemaWriter, Param, FieldCounter,
    ...
    for (auto& RPCTypeOwner : RPCTypeOwners)
    {
        ...
        RPCTypeOwnerSchemaWriter->WriteToFile(
            FString::Printf(TEXT("%s%s.schema"), *SchemaPath, *RPCTypeOwnerSchemaFilename));
    }

    // Generate RPC components
    for (auto Group : GetRPCTypes())
    {
