...
FString SchemaFieldName(const TSharedPtr<FUnrealProperty> Property) {
  ...
  Algo::Transform(GetPropertyChain(Property), ChainNames,
                  [](const TSharedPtr<FUnrealProperty> &Property) -> FString {
                    ...
                    // identifiers.
                    return Property->Property->GetName().ToLower().Replace(
                        TEXT("_"), TEXT(""));
                  });
...
int GenerateTypeBindingSchema(FCodeWriter &Writer, int ComponentId,
                              UClass *Class, TSharedPtr<FUnrealType> TypeInfo) {
