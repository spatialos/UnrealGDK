...
// Given a class or function name, generates the name used for naming schema components and types.
// Currently, this just removes underscores.
FString UnrealNameToSchemaTypeName(const FString& UnrealName);

// Given a replicated property group and Unreal type, generates the name of the corresponding
// component.
...
// Given an unreal type, generates the name of the component which stores worker to worker
// replication data.
// For example: UnrealCharacterWorkerRepData
FString SchemaMigratableDataName(UStruct* Type);

// Given an RPC type and Unreal type, generates the name of the corresponding RPC container
// component.
...
// Given a UFunction, generates the schema command name. Currently just returns the function name in
// lowercase.
FString SchemaCommandName(UFunction* Function);

// Given a UFunction, generates the c++ command name. Identical to the schema name with the first
// letter being uppercase.
FString CPPCommandClassName(UFunction* Function);

// Given a RepLayout cmd type (a data type supported by the replication system). Generates the
// corresponding
// type used in schema.
