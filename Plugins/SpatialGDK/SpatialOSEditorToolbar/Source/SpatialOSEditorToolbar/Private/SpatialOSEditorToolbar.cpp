...
FSpatialOSEditorToolbarModule::FSpatialOSEditorToolbarModule()
...
void FSpatialOSEditorToolbarModule::StartupModule() {
  ...
  OnPropertyChangedDelegateHandle =
      FCoreUObjectDelegates::OnObjectPropertyChanged.AddRaw(
          this, &FSpatialOSEditorToolbarModule::OnPropertyChanged);
...
void FSpatialOSEditorToolbarModule::RegisterSettings() {
  if (ISettingsModule *SettingsModule =
    ...
    ISettingsContainerPtr SettingsContainer =
        SettingsModule->GetContainer("Project");
    ...
    if (SettingsSection.IsValid()) {
      SettingsSection->OnModified().BindRaw(
          this, &FSpatialOSEditorToolbarModule::HandleSettingsSaved);
    }
  }
}
...
bool FSpatialOSEditorToolbarModule::HandleSettingsSaved() {
  USpatialOSEditorToolbarSettings *Settings =
      GetMutableDefault<USpatialOSEditorToolbarSettings>();
  Settings->SaveConfig();
...
void FSpatialOSEditorToolbarModule::MapActions(
    TSharedPtr<class FUICommandList> PluginCommands) {
  PluginCommands->MapAction(
      FSpatialOSEditorToolbarCommands::Get().StartSpatialOSStackAction,
      FExecuteAction::CreateRaw(
          this, &FSpatialOSEditorToolbarModule::StartSpatialOSButtonClicked),
      FCanExecuteAction::CreateRaw(
          this, &FSpatialOSEditorToolbarModule::StartSpatialOSStackCanExecute));

  PluginCommands->MapAction(
      FSpatialOSEditorToolbarCommands::Get().StopSpatialOSStackAction,
      FExecuteAction::CreateRaw(
          this, &FSpatialOSEditorToolbarModule::StopSpatialOSButtonClicked),
      FCanExecuteAction::CreateRaw(
          this, &FSpatialOSEditorToolbarModule::StopSpatialOSStackCanExecute));

  PluginCommands->MapAction(
      FSpatialOSEditorToolbarCommands::Get().LaunchInspectorWebPageAction,
      FExecuteAction::CreateRaw(
          this,
          &FSpatialOSEditorToolbarModule::LaunchInspectorWebpageButtonClicked),
      FCanExecuteAction());
}

void FSpatialOSEditorToolbarModule::SetupToolbar(
    TSharedPtr<class FUICommandList> PluginCommands) {
...
void FSpatialOSEditorToolbarModule::AddToolbarExtension(
...
void FSpatialOSEditorToolbarModule::StartSpatialOSButtonClicked() {
  ...
  const FString ExecuteAbsolutePath = FPaths::ConvertRelativePathToFull(
      SpatialOSToolbarSettings->ProjectRootFolder.Path);
  ...
  SpatialOSStackProcHandle = FPlatformProcess::CreateProc(
      *(CmdExecutable), *SpatialCmdArgument, true, false, false,
      &SpatialOSStackProcessID, 0, *ExecuteAbsolutePath, nullptr, nullptr);

  FNotificationInfo Info(
      SpatialOSStackProcHandle.IsValid() == true
          ? FText::FromString(TEXT("SpatialOS Starting..."))
          : FText::FromString(TEXT("Failed to start SpatialOS")));
  Info.ExpireDuration = 3.0f;
  Info.bUseSuccessFailIcons = true;
  auto NotificationItem =
      FSlateNotificationManager::Get().AddNotification(Info);

  if (!SpatialOSStackProcHandle.IsValid()) {
    NotificationItem->SetCompletionState(SNotificationItem::CS_Fail);
    const auto LogPath = SpatialOSToolbarSettings->ProjectRootFolder.Path +
                         FString(TEXT("/logs/spatial.log"));
    UE_LOG(LogSpatialOSEditor, Error,
           TEXT("Failed to start SpatialOS, please refer to log file `%s` for "
                "more information."),
           *LogPath);
  } else {
...
void FSpatialOSEditorToolbarModule::OnPropertyChanged(
