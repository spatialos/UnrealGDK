...
call :MarkStartOfBlock "Unpack dependencies"
    powershell -Command "Expand-Archive -Path \"%CORE_SDK_DIR%\cpp-static-x86_64-msvc_mtd-win32.zip\"       -DestinationPath \"%CORE_SDK_DIR%\cpp-src\" -Force; "^
                        ...
                        "Expand-Archive -Path \"%~dp0\..\Build\Programs\Improbable.Unreal.CodeGeneration.Test\NUnit\NUnit.zip\" -DestinationPath \"%PACKAGE_TARGET_DIR%\" -Force"
...
call :MarkStartOfBlock "Build CodeGeneration"
    %MSBUILD_EXE% /nologo /verbosity:minimal ..\Build\Programs\Improbable.Unreal.CodeGeneration\UnrealCodeGeneration.sln /property:Configuration=Release
